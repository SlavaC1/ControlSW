...
#define LOGS_DIRECTORY			LOG4CPLUS_TEXT("Logs")
	...
	TestManager::TestManager()
		: m_testsFolder(TESTS_FOLDER), m_status(new int), m_volumes(new unsigned int[IDT_COUNT]), m_materialInfo(new unsigned char*[IDT_COUNT]), m_isBusy(false), m_bStop(false)
	{
		memset(m_volumes, 0, IDT_COUNT * sizeof(unsigned int));
		memset(m_materialInfo, 0, IDT_COUNT * sizeof(unsigned char*));
	...
	TestManager::~TestManager()
	{
		...
		if (m_materialInfo != NULL)
		{
			for (int i = 0; i < IDT_COUNT; i++)
			{
				if (m_materialInfo[i] != NULL)
				{
					delete[] m_materialInfo[i];
					m_materialInfo[i] = NULL;
				}
			}
