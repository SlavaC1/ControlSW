// -----------------------------------------------------------------------------
// VaultIC_100_FipsPersonaliseSampleCode.cpp : 
// -----------------------------------------------------------------------------
// Defines a procedure for demonstrating how to personalise the VaultIC 100
// for authentication in FIPS mode.  FIPS mode adds an additional step to the 
...
#endif
{
    ...
    try
    {
        ...
        //---------------------------------------------------------------------
        // 3. Select the B-283-R1 Preloaded Domain Params File
        //---------------------------------------------------------------------
        ...
        //---------------------------------------------------------------------
        // 4. B-283-R1 key strength is strong enough, enable FIPS mode.
        //---------------------------------------------------------------------
        ...
        //---------------------------------------------------------------------
        // 7: Store the Host's Public Key in the Secure Object file system.
        //---------------------------------------------------------------------
        ...
        if (VLT_OK != ( usActualSW = theBaseApi->VltPutKey( (VLT_U8)enHostKeyGroup, 
                ...
                &KeyObj ) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltPutKey VLT_KEY_ECDSA_GF2M_PUB");
        }

        //---------------------------------------------------------------------
        // 8: Store the Device's Private Key in the Secure Object file system.
        //---------------------------------------------------------------------
        ...
        if (VLT_OK != ( usActualSW = theBaseApi->VltPutKey( (VLT_U8)enDeviceKeyGroup, 
                ...
                &KeyObj  ) ) )
        {
             CloseAndExit(hMod, usActualSW, "VltPutKey VLT_KEY_ECDSA_GF2M_PRIV");
        }

        //---------------------------------------------------------------------
        // 9: Store the Device's Public Key in the Secure Object file system.
        //---------------------------------------------------------------------
        KeyObj.u8KeyID = VLT_KEY_ECDSA_GF2M_PUB;
        KeyObj.data.EcdsaPubKey.u16QLen = ucBinaryFieldLength;
        ...
        //---------------------------------------------------------------------
        // 10 Transition to the ACTIVE state.
        //---------------------------------------------------------------------
        if (VLT_OK != ( usActualSW = theBaseApi->VltSetStatus( VLT_OPERATIONAL_ACTIVE ) ) )
        {
           CloseAndExit(hMod, usActualSW, "VltSetStatus VLT_OPERATIONAL_ACTIVE");
        }

        //---------------------------------------------------------------------
        // 11: Log the manufacturer out of the VaultIC Secure Object.
        //---------------------------------------------------------------------
		...
		CloseAndExit(hMod, usActualSW, "Example code complete, VaultIC 1XX is personalised for FIPS");             
    }
	catch(...)
	{
