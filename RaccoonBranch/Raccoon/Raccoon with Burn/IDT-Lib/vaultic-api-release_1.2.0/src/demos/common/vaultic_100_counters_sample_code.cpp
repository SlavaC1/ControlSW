...
#endif
{
    ...
    try
    {
       ...
       //
        ...
        //---------------------------------------------------------------------
        // 3. Select the B283 pre-defined curves
        //---------------------------------------------------------------------
		unsigned char ucCurve = VLT_ECDSA_PARAMS_PRE_LOADED_B283_CHOICE;

        if ( VLT_OK != ( usActualSW = theBaseApi->VltSetConfig( VLT_ECDSA_DOMAIN_PARAM_CHOICE, 
            VLT_ECDSA_PARAMS_CHOICE_SZ, &ucCurve ) ) )
        {
             CloseAndExit(hMod, usActualSW, "VltSetConfig VLT_ECDSA_PARAMS_PRE_LOADED_B283_CHOICE");
        }
		...
		if ( VLT_OK != ( usActualSW = theBaseApi->VltSetConfig( VLT_COUNTER_MODE_PARAM, 
            VLT_COUNTER_MODE_SZ, &ucCounterMode ) ) )
        {
             CloseAndExit(hMod, usActualSW, "VltSetConfig VLT_COUNTER_MODE_PARAM failed");
        }
		printf("VltSetConfig VLT_COUNTER_MODE_PARAM to VLT_DIRECT_MODE succeeded\n");

		//---------------------------------------------------------------------
        // 11: Set initials counter value
        //---------------------------------------------------------------------+
	
		//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		//	Warning: AMOUNT VALUE IS IN LSB MODE
		//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		printf("\n****************************************\n");
		printf("* Warning: AMOUNT VALUE IS IN LSB MODE *\n");
		printf("****************************************\n\n");
		VLT_U8 pCounterValue1[] = {0x10,0x00,0x00,0x00}; //Set value to 16
		VLT_U8 pCounterValue2[] = {0x00,0xFF,0x00,0x00};
		VLT_U8 pCounterValue3[] = {0x00,0x00,0xFF,0x00};
		VLT_U8 pCounterValue4[] = {0xFF,0xFF,0xFF,0xFF}; //Set Value to max

		if (VLT_OK != ( usActualSW = theBaseApi->VltSetSecureCounters( 0x01, 0x01,sizeof(pCounterValue1), pCounterValue1 ) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltSetSecureCounters (1,1) failed");
        }
		printf("VltSetSecureCounters (1,1) to 0x10 succeeded\n");

		if (VLT_OK != ( usActualSW = theBaseApi->VltSetSecureCounters( 0x01, 0x02,sizeof(pCounterValue2), pCounterValue2 ) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltSetSecureCounters (1,2) failed");
        }
		printf("VltSetSecureCounters (1,2) to 0x00FF succeeded\n");
		if (VLT_OK != ( usActualSW = theBaseApi->VltSetSecureCounters( 0x07, 0x01,sizeof(pCounterValue3), pCounterValue3 ) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltSetSecureCounters (7,1) failed");
        }
		printf("VltSetSecureCounters (7,1) to 0x0000FF succeeded\n");
		if (VLT_OK != ( usActualSW = theBaseApi->VltSetSecureCounters( 0x08, 0x02,sizeof(pCounterValue4), pCounterValue4 ) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltSetSecureCounters (8,2) failed");
        }
		printf("VltSetSecureCounters (8,2) to 0xFFFFFFFF succeeded\n");
		...
		printf("VltSetConfig VLT_COUNTER_MODE_PARAM to VLT_COUNTER_MODE succeeded\n");

		//---------------------------------------------------------------------
        // 12: Transition to the ACTIVE state.
        //---------------------------------------------------------------------
        if (VLT_OK != ( usActualSW = theBaseApi->VltSetStatus( VLT_OPERATIONAL_ACTIVE ) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltSetStatus (OPERATIONAL ACTIVE) failed");
        }

		//---------------------------------------------------------------------
        // 13: Log the manufacturer out of the VaultIC Secure Object.
        //---------------------------------------------------------------------
		...
		//---------------------------------------------------------------------
        // 14: Generate Random to get a nounce, not mandatory to use generate random command
        //---------------------------------------------------------------------
		VLT_COUNTER_DATA counterData;

		if (VLT_OK != ( usActualSW = theBaseApi->VltGenerateRandom(VLT_NOUNCE_LENGTH, counterData.nounce.u8Nounce) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltGenerateRandom failed");
        }

		//---------------------------------------------------------------------
        // 15: Decrement counter (1,1).
        //---------------------------------------------------------------------
		counterData.amount.u8AmountLength = VLT_COUNTER_LENGTH;
		memset(counterData.amount.u8Amount,0x00,counterData.amount.u8AmountLength);
		counterData.amount.u8Amount[0] = 0x05;
		counterData.nounce.u8NounceLength = VLT_NOUNCE_LENGTH;

		VLT_COUNTER_RESPONSE counterResponse;
		counterResponse.pu8Signature = (VLT_PU8)malloc(0x200);

		/* Update secure counter 1,1 */
		if (VLT_OK != ( usActualSW = theBaseApi->VltDecrementCounter(0x01,0x01,&counterData,&counterResponse) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltDecrementCounter (1,1) failed");
        }
		printf("VltSetSecureCounters (1,1) decremented of 1 succeeded\n");
		printf("New Value of SecureCounter (1,1) : ");
		for (unsigned int j=0;j<VLT_COUNTER_LENGTH;j++)
		{
			printf("%.2X",counterResponse.u8CntValue[j]);
		}
		printf("\n");

		//---------------------------------------------------------------------
        // 16: Generate Random to get a nounce, not mandatory to use generate random command
        //---------------------------------------------------------------------
		if (VLT_OK != ( usActualSW = theBaseApi->VltGenerateRandom(VLT_NOUNCE_LENGTH, counterData.nounce.u8Nounce) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltGenerateRandom failed");
		...
		//---------------------------------------------------------------------
        // 17: Decrement counter (1,2).
        //---------------------------------------------------------------------
		/* Update secure counter 1,2 */
		if (VLT_OK != ( usActualSW = theBaseApi->VltDecrementCounter(0x01,0x02,&counterData,&counterResponse) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltDecrementCounter (1,2) failed");
        }
		printf("VltSetSecureCounters (1,2) decremented of 5 succeeded\n");
		printf("New Value of SecureCounter (1,2) : ");
		for (unsigned int j=0;j<VLT_COUNTER_LENGTH;j++)
		{
			printf("%.2X",counterResponse.u8CntValue[j]);
		}
		...
		//---------------------------------------------------------------------
        // 18: Generate Random to get a nounce, not mandatory to use generate random command
        //---------------------------------------------------------------------
		if (VLT_OK != ( usActualSW = theBaseApi->VltGenerateRandom(VLT_NOUNCE_LENGTH, counterData.nounce.u8Nounce) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltGenerateRandom failed");
		...
		//---------------------------------------------------------------------
        // 17: Increment counter (1,2).
        //---------------------------------------------------------------------
		counterData.amount.u8Amount[0] = 0x05;
		//Create signature
		VLT_U8 dataToSign[VLT_NOUNCE_LENGTH + VLT_COUNTER_LENGTH];
		memcpy(dataToSign, counterData.nounce.u8Nounce,VLT_NOUNCE_LENGTH);
		memcpy(dataToSign + VLT_NOUNCE_LENGTH,counterData.amount.u8Amount,VLT_COUNTER_LENGTH);

		VLT_U32 u32SignatureLength = 0x200;
		VLT_PU8 aucSignature = (VLT_PU8)malloc(u32SignatureLength);

		VLT_ECDSA_PRIVATE_KEY hostPrivKey;
		hostPrivKey.u16DLen = sizeof(auc_Host_D);
		hostPrivKey.pu8D = auc_Host_D;

		VLT_ECDSA_DOMAIN_PARAMS ecdsaDomParams; 
		ecdsaDomParams.u16QLen = ucBinaryFieldLength;
		ecdsaDomParams.pu8Q = auc_Q;
		ecdsaDomParams.pu8Gx = auc_Gx;
		ecdsaDomParams.pu8Gy = auc_Gy;
		ecdsaDomParams.pu8Gz = auc_Gz;
		ecdsaDomParams.pu8A = auc_A;
		ecdsaDomParams.pu8B = auc_B;
		ecdsaDomParams.u16NLen = ucBinaryFieldLength;
		ecdsaDomParams.pu8N = auc_N;
		ecdsaDomParams.u32H = us_H;
		...
		*/
		...
		/* Update secure counter 1,2 */
		if (VLT_OK != ( usActualSW = theBaseApi->VltIncrementCounter(0x01,0x02,&counterData,u32SignatureLength,aucSignature,&counterResponse) ) )
        {
            CloseAndExit(hMod, usActualSW, "VltIncrementCounter (1,2) failed");
        }
		printf("VltSetSecureCounters (1,2) incremented of 5 succeeded\n");
		printf("New Value of SecureCounter (1,2) : ");
		for (unsigned int j=0;j<VLT_COUNTER_LENGTH;j++)
		{
			printf("%.2X",counterResponse.u8CntValue[j]);
		...
		FREE(counterResponse.pu8Signature);

		CloseAndExit(hMod, usActualSW, "Example code complete");             
    }
	catch(...)
	{
