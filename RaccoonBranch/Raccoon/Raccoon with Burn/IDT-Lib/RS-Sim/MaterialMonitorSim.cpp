...
#define COUNTER_FILE		"counter.txt"
	...
	int MaterialMonitorSim::InitHW(unsigned char *pucCartridgesCount)
	{
	...
	///
	int MaterialMonitorSim::BurnIDC(unsigned char ucCartridgeNum, const unsigned char *aucMaterialInfo, unsigned short usMaterialInfoLength, const unsigned char *aucPrvKS, const unsigned char *aucPubKS)
	{
		UniqueLock uniqueLock(m_mtx);
		...
		{
			int returnValue = 0;
			int behaviorReturnValue = GetBurnIDCBehavior(ucCartridgeNum, aucMaterialInfo, usMaterialInfoLength, aucPrvKS, aucPubKS);
			...
			try
			{
				// sign certificate with PrvKS and append signature:
				prvKS.Load(ArraySource(aucPrvKS, PRIVATE_KEY_SIZE, true).Ref());

				ECDSA<EC2N, SHA256>::Signer signer(prvKS);
				string signature(signer.MaxSignatureLength(), 0x00);
				signer.SignMessage(prng, ucCertificate, offset, (byte *)signature.data());

				memcpy(ucCertificate + offset, signature.data(), signature.length());
	...
	int MaterialMonitorSim::GetBurnIDCBehavior(unsigned char ucCartridgeNum, const unsigned char *aucMaterialInfo, unsigned short usMaterialInfoLength, const unsigned char *aucPrvKS, const unsigned char *aucPubKS)
	{
		if (m_behavior == NULL)
		{
			return 0;
		...
		buffer = new char[2 * PRIVATE_KEY_SIZE + 1];
		ByteArrayToHexString(aucPrvKS, PRIVATE_KEY_SIZE, buffer);
		parameters.push_back(pair<string, string>("PrvKS", buffer));
		delete[] buffer;

		buffer = new char[2 * PUBLIC_KEY_SIZE + 1];
