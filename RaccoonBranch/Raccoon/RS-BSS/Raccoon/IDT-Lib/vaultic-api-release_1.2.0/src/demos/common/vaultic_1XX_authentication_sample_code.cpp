...
#endif
{
    ...
    try
    {
        ...
        {
           CloseAndExit(hMod, usActualSW, "USER NOT AUTHENTICATED");
        }


        //---------------------------------------------------------------------
        // 3: Check we are logged in by accessing a restricted command:
        ...
        //---------------------------------------------------------------------
        // 4: Check the vaultic crypto engine was setup for a SHA-256 bit hash.
        //---------------------------------------------------------------------
        ...
        if ( VLT_OK != ( usActualSW = theStrongAuth->VltStrongClose( ) ) )
        {
		 ...
		 CloseAndExit(hMod, usActualSW, "Example code complete, VaultIC file system remains personalised.");               
