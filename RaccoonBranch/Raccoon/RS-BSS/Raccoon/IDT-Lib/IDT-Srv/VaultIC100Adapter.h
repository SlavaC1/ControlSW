...
#if (ENABLE_BURNING == 1)
		virtual int Login(const unsigned char* acPassword, unsigned char usPasswordLength);
		virtual int Configure();
		virtual int SetKeys(PubKey* pubKS);
		virtual int SetCounter(unsigned int uiCouterValue);
		virtual int WriteMaterialInfo(unsigned char* aucMaterialInfo, unsigned short usMaterialInfoLength);
		virtual int Logout();
		virtual int WriteSignature(unsigned char* aucSignature, unsigned short usSignatureLength);
#endif
...
#endif
		int GetPaddedLength(int length, int padding) { return (int)(ceilf(length / (float)padding) * padding); }
