...
/**
 * \fn VltUnInitializeAlgorithm();
 *
 * \brief Uninitializes a cryptographic algorithm.
 *
 * \par Description:
 *
 */
...
/**
 * \fn VltGenerateAssuranceMessage( VLT_U8 u8KeyGroup,
    VLT_U8 u8KeyIndex,
 ...
 * assurance.
 *
 * \param u8KeyGroup        [IN]     The public key group that will be used in 
 *                                   the verify operation.
 * \param u8KeyIndex        [IN]     The public key index that will be used in 
 *                                   the verify operation.
...
VLT_STS VltUpdateMessageDigest( VLT_U32 u32MessageLength,
    const VLT_U8 *pu8PartMessage);
/**
 * \fn VLT_STS VltComputeMessageDigestFinal( VLT_PU8 u32MessagePartLength,
 *   VLT_PU8 pu8Digest );
 *
 ...
 *
 * \brief Derive a key using the DH agreement specified on pKeyDerivation.
 *
 * \par Description:
 *
 * \param u8keyGroup			[IN]	Group index of derived key.
 * \param u8keyIndex			[IN]	Index of derived key.
 * \param pKeyFilePrivileges	[IN]	Privileges of derived key.
 * \param u8DerivatedKeyType	[IN]	Derived key type.
 * \param u16WDerivatedKeyLen	[IN]	Privileges of derived key.
 * \param pKeyDerivation		[IN]	Key derivation parameters.
 *
 * \return Upon successful completion a VLT_OK status will be returned otherwise
 * the appropriate error code will be returned. Please note, status values 
 * larger than VLT_OK are errors that have originated in the API library while 
...
VLT_STS VltDeriveKey(VLT_U8 u8keyGroup,
	...
	const VLT_KEY_DERIVATION *pKeyDerivation );
/**
 * \fn VLT_STS VltConstructDHAgreement( 
 *	VLT_U8 u8resultKeyGroup,
 *  VLT_U8 u8resultKeyIndex,
 *  const VLT_FILE_PRIVILEGES *pKeyFilePrivileges,
 *	const VLT_KEY_MATERIAL *pKeyMaterial );
 *
 * \brief Construct a DH agreement.
 *
 * \par Description:
 *
 * \param u8resultKeyGroup		[IN]	Group index of result key.
 * \param u8resultKeyIndex		[IN]	Index of result key.
 * \param pKeyFilePrivileges	[IN]	Privileges of result key.
 * \param pKeyMaterial			[IN]	Key material for DH agreement construction.
 *
 * \return Upon successful completion a VLT_OK status will be returned otherwise
 * the appropriate error code will be returned. Please note, status values 
 * larger than VLT_OK are errors that have originated in the API library while 
