 ...
 *
 * $Id: hasp_api.h 1.2 2015/12/23 15:29:00IST Zaks, Michael (michaelz) Exp  $
 */
...
enum hasp_error_codes
{
    ...
    /** Vm engine is monopolized by other clients */
    HASP_VM_USED_ERR = 701,

    /** Vm engine internal error */
    HASP_VM_INT_ERR = 702,

    /** Heap in vm engine execution overflow */
    HASP_VM_HEAP_OVERFLOW = 703,

    /** Invliad vm code in the vm resource */
    HASP_VM_INVALID_CODE = 704,

    /** Type or size is not correct in vm io buffer */
    HASP_VM_TYPE_ERR = 705,

    /** Method id is not found in the vm resource */	
    HASP_VM_METHOD_NOT_FOUND = 706,
	
	/** Exception is triggered */
    HASP_VM_METHOD_EXCEPTION_OCCUR = 707,
...
/** Sentinel VM method ID */
typedef hasp_u32_t hasp_methodid_t;
 ...
 *
 * <b>Sentinel Admin API Usage Notes</b><br>
 * This function establishes a context to a Sentinel License Manager
 *
 * \param admin_context  NULL; unused im Embedded LM, just kept for compatibility
...
hasp_status_t HASP_CALLCONV hasp_embadmin_context_new (
                              void         *admin_context,
                              char         *host,
                              hasp_u16_t    port,
                              char         *password);
 ...
 *
 * Does nothing in Embedded LM, just kept for compatibility
 * an AdminAPI context.
 *
 * \param admin_context       NULL; unused im Embedded LM, just kept for compatibility
 *
 ...
 *
 */
hasp_status_t HASP_CALLCONV hasp_embadmin_context_delete (
                              void   *admin_context);
 ...
 * \param scope            Definition of the data that is to be searched, in XML
 *                           format. For more information, see the accompanying
 *                           Sentinel Admin API help documentation
 * \param format           Definition of the format in which the data is to be
 *                           displayed, in XML format. For more information, see the
 *                           accompanying Sentinel Admin API help documentation
 * \param info             Pointer to the information that is retrieved, in XML
 *                           format, including status information
 * \return                 <ul>
...
hasp_status_t HASP_CALLCONV hasp_embadmin_get (
                              void          *admin_context,
                              const char    *scope,
                              const char    *format,
                              char         **info);
 ...
 *
 * \param admin_context    NULL; unused im Embedded LM, just kept for compatibility
 * \param action           Definition of the actions/settings to be done, in XML format.
 *                           For more information, see the accompanying Sentinel Admin API
 *                           help documentation
...
hasp_status_t HASP_CALLCONV hasp_embadmin_set (
                              void          *admin_context,
                              const char    *input,
                              char         **status );
...
/**
 * \brief Initializes the vm resource file, and will monopolize the token until hasp_vm_close is called.
 *
 * \param handle      Handle for the session
 * \param vm_id       Input for vm resource file id
 *
 * \return            <ul>
 *                      <li>HASP_STATUS_OK</li> Request was successfully
 *                        completed
 ...
 *
 */
hasp_status_t HASP_CALLCONV hasp_vm_init(hasp_handle_t      handle,
                                         hasp_fileid_t      vm_id);

/**
 * \brief Invokes the method in VM resource file via method id.
 *
 * \param handle      Handle for the session
 * \param vm_id       Input for method id in vm resource file
 * \param buffer      Pointer to the io buffer of vm method
 * \param length      Pointer to the io buffer length
 * \param returncode  Pointer to the vm method return code
 *
 * \return            <ul>
 *                      <li>HASP_STATUS_OK</li> Request was successfully
 *                        completed
 ...
 *
 */
hasp_status_t HASP_CALLCONV hasp_vm_invoke(hasp_handle_t         handle,
                                             hasp_u16_t          method_id,
                                             void *              buffer,
                                             hasp_size_t *       length,
                                             hasp_s32_t *        returncode);


/**
 * \brief Rleases vm context, and will free the token.
 *
 * \param handle      Handle for the session
 *
 * \return            <ul>
...
hasp_status_t HASP_CALLCONV hasp_vm_dyn_execute(hasp_handle_t handle,
    const void* vm_image,
    hasp_size_t vm_image_size,
    hasp_u32_t method_id,
    const void* input,
    hasp_size_t input_size,
    void* output,
    hasp_size_t* output_size,
