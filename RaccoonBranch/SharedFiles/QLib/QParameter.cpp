...
void AddSpecificObserver(TParamChangeEvent Event,TGenericCockie Cockie = 0, TObserverType Type = 0);    
...
{
   return (m_ValuesStack.size() == 0);
}

void CQParameterBase::CollapseStack(void)
{
   while(m_ValuesStack.size() > 0)
     m_ValuesStack.pop();
}

void CQParameterBase::ResetStack(void)
{
   bool StackEmpty = m_ValuesStack.size() == 0;
   QString Value;
   while(m_ValuesStack.size() > 0)
   {
