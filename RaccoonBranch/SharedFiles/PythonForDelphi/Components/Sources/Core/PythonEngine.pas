...
type
  ...
  TPyMemberFlag = (mfDefault, mfReadOnly, mfReadRestricted, mfRestricted);
{$ENDIF}
...
type
  ...
  TPyCFunctionWithExtraArg = function(ExtraArg : pointer; self, args:PPyObject): PPyObject; cdecl;
...
type
  ...
  public
    ...
    procedure  ExecStringUseDict(const command : String; DictToUse : PPyObject);
    ...
    function   EvalStringAsStrUseDict(const command : String; DictToUse : PPyObject) : String;
...
{$ENDIF}
    ...
    procedure ClearModuleDictionary;
...
//-------------------------------------------------------
  ...
  TMethodsContainer = class(TEngineClient)
    ...
    public
      ...
      function AddMethodWithExtraArg(MethodName  : PChar;
                                     MethodWithExtraArg: TPyCFunctionWithExtraArg;
                                     ExtraArg : pointer;
                                     DocString : PChar ) : PPyMethodDef;
...
function TPythonEngine.EvalStringAsStrUseDict(const command : String; DictToUse : PPyObject) : String;
var
  OldDict : PPyObject;
begin
  try
    OldDict := FLocalVars;
    FLocalVars := DictToUse;
    Result := Run_CommandAsString( command, eval_input );
  finally
    FLocalVars := OldDict;
  end;
...
procedure TPythonEngine.ExecStringUseDict(const command : String; DictToUse : PPyObject);
var
  OldDict : PPyObject;
begin
  try
...
function  TMethodsContainer.AddMethodWithExtraArg(MethodName  : PChar;
                                                  MethodWithExtraArg: TPyCFunctionWithExtraArg;
                                                  ExtraArg : pointer;
                                                  DocString : PChar ) : PPyMethodDef;
var
  Method : TMethod;
begin
  Method.Data := ExtraArg;
  Method.Code := pointer(@MethodWithExtraArg);

  Result := AddMethod( MethodName,
                       GetOfObjectCallBack(TCallBack(Method),2,ctCDECL),
                       DocString);
end;
...
procedure TPythonEngine.ClearModuleDictionary;
var
  i,l : integer;
  Keys : PPyObject;
  Key : PPyObject;
...
begin
  ...
  l := PyDict_Size(ModuleDict);

  if l > 0 then
  begin
    Keys := PyDict_Keys(ModuleDict);

    for i := 0 to l - 1 do
    begin
      Key := PyList_GetItem(Keys,i);
      KeyStr := PyObjectAsString(Key);

      if (KeyStr <> 'sys') and
         (KeyStr <> 'DebugOutput') and
         (KeyStr <> '__builtins__') and
         (KeyStr <> '__name__') and
         (KeyStr <> '__doc__') and
         (KeyStr <> 'pyio') then
        PyDict_DelItem(ModuleDict,Key);
    end;
