...
(* Sub-Version: 0.5                 4721 rue Brebeuf                      *)
(*                                  H2J 3L2 MONTREAL (QC)                 *)
(*                                  CANADA                                *)
(*                                  e-mail: mmm@free.fr                   *)
(*                                                                        *)
...
function VarIsSame( A, B : Variant ) : Boolean; // checks if 2 variants share the same Python object.
function VarIsSameType( A, B : Variant ) : Boolean; // checks if 2 variants are of the same Python type.
...
function len( AValue : Variant ) : Integer; // return the length of a Python collection.
function _type( AValue : Variant ) : Variant; // return the type object of a Python object.
...
function VarIsSame( A, B : Variant ) : Boolean;
begin
  Result := ExtractPythonObjectFrom(A) = ExtractPythonObjectFrom(B);
end;

function VarIsSameType( A, B : Variant ) : Boolean;
var
...
function len( AValue : Variant ) : Integer;
begin
  if VarIsPython(AValue) then
...
function _type( AValue : Variant ) : Variant;
begin
  if VarIsPython(AValue) then
  ...
  begin
    ...
    begin
      with LStrings[LStrCount] do
        if LArgByRef then
        begin
          //BStr := StringToOleStr(System.PString(ParamPtr^)^);
          BStr := System.Copy(System.PString(LParamPtr^)^, 1, MaxInt);
          PStr := System.PString(LParamPtr^);
...
begin
  ...
  LIdent := String(PChar(@CallDesc^.ArgTypes[LArgCount]));
...
{$ENDIF}
  ...
  function GetObjectLength( AObject : PPyObject ) : PPyObject;
  begin
    with GetPythonEngine do
    begin
      PyErr_Clear;
      Result := PyInt_FromLong( PyObject_Length(AObject) );
      CheckError;
    end; // of with
  end; // of function
      ...
      try
        ...
        begin
          ...
          if (Length(Arguments) = 1) and SameText(AName, 'GetItem') then
            ...
            Result := GetObjectLength(_container);
        end; // of if
      finally
      ...
      if Assigned(_obj) then
        ...
        begin
          ...
          try
            ...
            Result := PyObject_CallObject(_obj, _Args);
            CheckError;
          finally
            Py_XDecRef(_Args);
          end; // of try
