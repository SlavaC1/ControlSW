...
// This lookup table converted index 0 to value 0 and index 1 to value 1 etc'
// So basically this lookup table is redundant, so i marked it out

//BYTE xdata MSCCardLookUpTable[NUM_OF_MSC_CARDS] = {
//                                     FIRSTMSCCARD,
//									 SECONDMSCCARD
//								   };
...
typedef struct {
	BYTE DestId;
	BYTE TransactionId;
    }TGetPowerSuppliesTaskParams;//Elad
...
void EdenMessagesDecode(BYTE *Message) 
{
  ...
  // Reset the CommunicationLoss timer, since we got a message with OCB as destination.
  ...
  TimerSetTimeout (&CommunicationLossTimer, TIMER0_MS_TO_TICKS(COMMUNICATION_LOSS_TIMEOUT));
    ...
    case SET_HEADS_FILLING_PARAMS_MSG:
    {
      ...
      SetHeadsFillingParamsTaskParams.DestId = SrcId;
 	  ...
 	  SwapUnsignedShortArray(&(SetHeadsFillingParamsMsg->TimePumpOn), &(SetHeadsFillingParamsMsg->TimePumpOn), 15); 
...
void GetDoorStatusTask(BYTE Arg)
{
  ...
  Msg.DoorRequest = DoorGetLastRequest(); 
  Msg.DoorStatus = DoorGetStatus();
...
void IsLiquidTanksInsertedTask(BYTE Arg)
{
  ...
  LiquidTanksGetStatus(&Msg.ModelTank1Status);
...
void GetLiquidTanksWeightTask(BYTE Arg)
{
  ...
  LiquidTanksGetAverageWeights(&Msg.ModelTank1Weight);
#ifdef OBJET_MACHINE
  SwapUnsignedLongArray((unsigned long*)&Msg.ModelTank1Weight,(unsigned long*)&Msg.ModelTank1Weight,9);
#else
  SwapUnsignedLongArray((unsigned long*)&Msg.ModelTank1Weight,(unsigned long*)&Msg.ModelTank1Weight,7);
#endif
...
#else
  HeadsFillingGetLiquidsLevel(&Msg.Model1_Level, &Msg.Model2_Level, &Msg.SupportLevel);
#endif
...
void GetUVLampsValueExTask(BYTE Arg)
{
  ...
  Msg.UVNumOfReadings = UVLampsGetUVNumOfReadings(); //EncoderData
...
void GetSWVersionTask(BYTE Arg)
{
  ...
  Msg.MSC1HardwareVersion = I2CA2D_GetReading(LAST_BYTE, /*MSCCardLookUpTable[FIRSTMSCCARD]*/ FIRSTMSCCARD )& 0x01;//Elad, I2C Version
  Msg.MSC1FirmwareVersion = I2CA2D_GetReading(LAST_BYTE, /*MSCCardLookUpTable[FIRSTMSCCARD]*/ FIRSTMSCCARD )>>0x03;
  Msg.MSC2HardwareVersion = I2CA2D_GetReading(LAST_BYTE, /*MSCCardLookUpTable[SECONDMSCCARD]*/ SECONDMSCCARD )& 0x01;
  Msg.MSC2FirmwareVersion = I2CA2D_GetReading(LAST_BYTE, /*MSCCardLookUpTable[SECONDMSCCARD]*/ SECONDMSCCARD )>>0x03;

	if (EdenProtocolSend((BYTE*)&Msg,sizeof(TOCBSoftwareVersionMsg),GetSWVersionTaskParams.DestId,OCB_ID,
 			GetSWVersionTaskParams.TransactionId,TRUE,EDEN_OCB_PROTOCOL) == EDEN_PROTOCOL_NO_ERROR)
		SchedulerSuspendTask(-1);
	else
		SchedulerLeaveTask(Arg);	
}
...
#ifdef OCB_SIMULATOR
	XilinxRead(OHDB_XILINX_DATA__ENCODER_ADDR);
	XilinxGetReceivedData((BYTE*)&EncoderData);
...
void GetPowerSuppliesVTask(BYTE Arg)	   //Elad
{
	...
	I2CA2D_GetBlockReading(Msg.FirstI2CA2DInput,FIRSTMSCCARD); 
	I2CA2D_GetBlockReading(Msg.SecondI2CA2DInput,SECONDMSCCARD);

	SwapUnsignedShortArray(&Msg.Vs_Volt,&Msg.Vs_Volt,12);
