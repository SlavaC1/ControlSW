...
CHeadVacuumBase::CHeadVacuumBase(const QString& Name,CPurgeBase *PurgeInstance) : CQComponent(Name)
{
  ...
  m_MutexDataBase.WaitFor();
     ...
     for(int i=0;i<MAX_VACUUM_HISTORY_VALUES;i++)
        m_VacuumSensorValue[i].AssignFromA2D(0);
     m_CurrentVacuumIndex=-1;
     m_AverageIndex=0;
  ...
  m_VacuumSamplesToAverage = m_ParamsMgr->VacuumSamplesToAverage.Value();
  ResetStatistics();
}
...
{
   int MaxVacuumIndex = m_VacuumSamplesToAverage;

   if(m_AverageIndex < MaxVacuumIndex)
      m_AverageIndex++;

   m_MutexDataBase.WaitFor();

      //First handle the average:
      if(++m_CurrentVacuumIndex==MaxVacuumIndex)
          m_CurrentVacuumIndex=0;

      //Instead of every time count each element of vacuum table
      //we replace the new with the old one
      m_VacuumLastNValues += A2DValue - m_VacuumSensorValue[m_CurrentVacuumIndex].A2DValue();
      m_VacuumAverage = (float)m_VacuumLastNValues/(float)m_AverageIndex;
      //Now keep the new value
      m_VacuumSensorValue[m_CurrentVacuumIndex].AssignFromA2D(A2DValue);
   m_MutexDataBase.Release();
}
...
float CHeadVacuumBase::GetLastVacuumCmH20Value(void)
{
   ...
   m_MutexDataBase.WaitFor();
     Value = m_VacuumSensorValue[m_CurrentVacuumIndex].CmH2OValue();
   m_MutexDataBase.Release();
...
int CHeadVacuumBase::GetLastVacuumA2DValue(void)
{
   ...
   m_MutexDataBase.WaitFor();
     Value =  m_VacuumSensorValue[m_CurrentVacuumIndex].A2DValue();
   m_MutexDataBase.Release();
...
bool CHeadVacuumBase::CheckVacuumSensorValue(void)
{
   ...
   bool CriticalOutOfRange;
   bool AverageOutOfRange;
   int Average;
   int CurrA2DValue;

   m_MutexDataBase.WaitFor();
     CurrA2DValue = m_VacuumSensorValue[m_CurrentVacuumIndex].A2DValue();

     CriticalOutOfRange =
      (m_VacuumSensorValue[m_CurrentVacuumIndex] > m_ParamsMgr->VacuumHighCriticalLimit ||
       m_VacuumSensorValue[m_CurrentVacuumIndex] < m_ParamsMgr->VacuumLowCriticalLimit);

     Average = static_cast<int>(m_VacuumAverage);
     AverageOutOfRange = (Average > m_ParamsMgr->VacuumHighLimit || Average < m_ParamsMgr->VacuumLowLimit );
   m_MutexDataBase.Release();

   // check and report high/low reading that are not critical:
   if (CurrA2DValue > m_ParamsMgr->VacuumHighLimit || CurrA2DValue < m_ParamsMgr->VacuumLowLimit)
   {
       CQLog::Write(LOG_TAG_HEAD_VACUUM,"VACUUM OUT OF RANGE (Non-critical). A2D value: %d",
                    (int)CurrA2DValue);
   }
   ...
   if(CriticalOutOfRange && AllowVacuumErrorDetection && m_EnableErrorHandling)
   {
     if(m_VaccumLastStatus == VACUUM_STATUS_OK)
     {
       m_VacuumTotalOutOfRangeCounter++;
       m_ErrorHandlerClient->ReportError("Vacuum out of range: ", Q2RT_HEAD_VACUUM_OUT_OF_RANGE,
                                          m_VacuumSensorValue[m_CurrentVacuumIndex].CmH2OValue(), CurrA2DValue);

       CQLog::Write(LOG_TAG_HEAD_VACUUM,"VACUUM OUT OF RANGE. A2D value: %d OutOfRange counter: %d",
                    (int)CurrA2DValue, (int)m_VacuumTotalOutOfRangeCounter);
       m_VaccumLastStatus = VACUUM_STATUS_WRONG;
     }
     return false;
   }

   if(AverageOutOfRange && AllowVacuumErrorDetection && m_EnableErrorHandling)
   {
     if(m_VaccumLastStatus == VACUUM_STATUS_OK)
     {
       m_VacuumAverageOutOfRange++;
       if(++m_VacuumOutOfRangeCounter >= m_ParamsMgr->VacuumSamplesToMarkError)
       {
         m_VacuumOutOfRangeCounter=0;
         CQLog::Write(LOG_TAG_HEAD_VACUUM,"VACUUM AVERAGE OUT OF RANGE");
         m_ErrorHandlerClient->ReportError("Vacuum average out of range: ", Q2RT_HEAD_VACUUM_AVERAGE_OUT_OF_RANGE, Average);
         m_VaccumLastStatus = VACUUM_STATUS_WRONG;
       }
     }
     return false;
