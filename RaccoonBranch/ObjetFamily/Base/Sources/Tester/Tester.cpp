...
// Constructor
CTester::CTester(void) : CQComponent("Tester")
{
...
BYTE CTester::ReadDataFromLotus(WORD Address)
{
  ...
  if (ProtocolClient->SendInstallWaitReply(&Msg,
	  ...
	  ETester("Lotus didn't get ack for \"ReadDataFromLotus\" message");
      }

  // Check if we really got ack
  if(!m_ReadXilinxAckOk)
	ETester("Tester:ReadDataFromLotus Invalid reply from Lotus");
...
int CTester::WriteDataToLotus(WORD Address, int Data)
{
  ...
  if (ProtocolClient->SendInstallWaitReply(&Msg,
	  ...
	  ETester("Lotus didn't get ack for \"WriteDataToLotus\" message");
      }

  // Check if we really got ack
  if(!m_WriteXilinxAckOk)
    ETester("Tester:\"WriteDataToLotus\" Invalid reply from Lotus");
