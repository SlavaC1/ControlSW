...
// Constructor
CRandomScatter::CRandomScatter(int HeadPrintWidth,int NozzlesSeqOffset) : CScatterGenerator() 
{
...
//  ScatterPlusInterlace -= (NozzleGapMinusOne - ParamsMgr->YSecondaryInterlaceNoOfPixels + TrimmedInterlace);
  ...
  NoOfPasses = CRaccoon::Instance()->R_CNOP(SliceHeight,SliceSizeType,SliceYOffset,s_HeadPrintWidth,ScatterPlusEvenOdd,CoveredHeight); //ITAMAR RACCOON
  YScatterOffset = CRaccoon::Instance()->R_CYSO(SliceHeight,SliceSizeType,SliceYOffset,s_HeadPrintWidth,g_NozzlesGapInPixels,NoOfPasses,ScatterPlusEvenOdd,ParamsMgr->YInterlaceNoOfPixels,ParamsMgr->YSecondaryInterlaceNoOfPixels);//ITAMAR RACCOON
}
