...
void CSerialReceiverThread::DisplayUnreadData(void)
{
   QString DataString;
  ...
  CQLog::Write(LOG_TAG_GENERAL,"Wrong Bytes received : %s",DataString);
}
                                                    ...
                                                    PVOID Data,unsigned DataLength,TGenericCockie Cockie)
{
  ...
  if(!InstancePtr->ProcessPacket(PacketHeader,Data,DataLength))
    ...
    if(!ReRoutePacket(PacketHeader,Data,DataLength,InstancePtr))
      ...
      RouteError(Q2RT_SERIAL_ROUTE_PACKET_ERR,QFormatStr("Route error (Source = %d , Dest = %d)",
                 PacketHeader->SourceID,PacketHeader->DestID));
}
