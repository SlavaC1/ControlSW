...
void COCBStatusSender::Execute(void)
{
	...
	const int DELAY_BETWEEN_EACH_OCB_STATUS_VERIFICATION_MIL = 500;
	//Delay : How much time to ask time TIME * DELAY_BETWEEN_EACH_OCB_STATUS_VERIFICATION_MIL
	const int TIME_BETWEEN_FILLING_STATUS = 2 *FACTOR_TIME_BETWEEN;   //
	const int TIME_BETWEEN_POWER_STATUS = 4 *FACTOR_TIME_BETWEEN;
	const int TIME_BETWEEN_TRAY_HEATER_STATUS = 8 *FACTOR_TIME_BETWEEN;
	const int TIME_BETWEEN_UV_LAMPS_STATUS = 1 *FACTOR_TIME_BETWEEN;
	const int TIME_BETWEEN_ACTUATOR_STATUS = 1 *FACTOR_TIME_BETWEEN;
	const int TIME_BETWEEN_RSS_STATUS = 1 * FACTOR_TIME_BETWEEN; //RSS, itamar
	const int TIME_BETWEEN_MODEL_SENSOR = 2 * FACTOR_TIME_BETWEEN;
	const int TIME_BETWEEN_WEIGHT_STATUS = 1 * FACTOR_TIME_BETWEEN;
	// todo -oNobody -cNone: improper usage of clock based functions - effected by clock changes
	time_t CurrentTime = QGetCurrentTime();;
	time_t NextFillingStatusTime = CurrentTime;
	time_t NextPowerStatusTime = CurrentTime;
	time_t NextTrayHeaterStatusTime = CurrentTime;
	time_t NextUVStatusTime = CurrentTime;
	time_t NextActuatorTime = CurrentTime;
	time_t NextRSSTime = CurrentTime;//RSS, itamar
	//time_t NextModelSensorTime = CurrentTime;
	time_t NextWeightStatusTime = CurrentTime;
	...
	while(!Terminated)
	{
		try
		{
			...
			if((m_timeCounter%(ONE_SECOND_PERIOD*4))==6)
			{
				...
				m_PowerPtr->GetPowerSuppliersStatus();//Elad
...
void COCBStatusSender::GetOCBSWVersionAck(int TransactionID,PVOID Data,unsigned DataLength,TGenericCockie Cockie)
{
	...
	if(DataLength == sizeof(TOCBDriverSoftwareVersionResponse))
	{
		...
		if(Message->MessageID == OCB_SW_VERSION)
		{
			...
			InstancePtr->m_MSC1_HW_Version  =  Message->MSC1HardwareVersion;
			InstancePtr->m_MSC1_FW_Version  =  Message->MSC1FirmwareVersion;
			InstancePtr->m_MSC2_HW_Version  =  Message->MSC2HardwareVersion;
			InstancePtr->m_MSC2_FW_Version  =  Message->MSC2FirmwareVersion;
...
TQErrCode COCBStatusSender::DisplayOCBVersion(void)
{
	...
	CQLog::Write(LOG_TAG_GENERAL,"MSC1 HW version = %d",m_MSC1_HW_Version);
	CQLog::Write(LOG_TAG_GENERAL,"MSC1 FW version = %d",m_MSC1_FW_Version);
	CQLog::Write(LOG_TAG_GENERAL,"MSC2 HW version = %d",m_MSC2_HW_Version);
	CQLog::Write(LOG_TAG_GENERAL,"MSC2 FW version = %d",m_MSC2_FW_Version);
...
int COCBStatusSender::GetMSC1HWVersion(void)
{
	return m_MSC1_HW_Version ;
}

int COCBStatusSender::GetMSC1FWVersion(void)
{
	return m_MSC1_FW_Version;
}

int COCBStatusSender::GetMSC2HWVersion(void)
{
	return m_MSC2_HW_Version;
}

int COCBStatusSender::GetMSC2FWVersion(void)
{
	return m_MSC2_FW_Version;
}
