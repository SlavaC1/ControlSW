...
const int THERMISTOR_FILLING_OPEN_VALUE  = 4000; 
...
CHeadFillingBase::CHeadFillingBase(const QString& Name) : CQComponent(Name)
{
   ...
   INIT_METHOD(CHeadFillingBase,GetMaterialSensorStatus); 
...
TQErrCode CHeadFilling::UpdateActiveThermistors()
{
   ...
   model = BackEnd->IsModelMaterial(m_ParamsMgr->TypesArrayPerChamber[TYPE_CHAMBER_MODEL3]);
   ...
   {
	 m_activeThermistors[SUPPORT_M3_CHAMBER_THERMISTOR] = 1;
	 m_activeThermistors[M1_M2_CHAMBER_THERMISTOR] = 1;
	 m_ParamsMgr->ThermistorsOperationMode = HIGH_THERMISTORS_MODE;
   }
...
TQErrCode CHeadFilling::GetMaterialSensorStatus(void)
{
   ...
   if (m_FlagGetMaterialStatusComUnderUse)  //m_FlagGetMaterialStatusComUnderUse)
	  QMonitor.Print("WARNING:HeadFiling:GetMaterialSensorStatus:two message send in the same time");

   m_FlagGetMaterialStatusComUnderUse=true;

   TOHDBGetMaterialLevelSensorStatusMessage GetMsg;

   GetMsg.MessageID = static_cast<BYTE>(OHDB_GET_MATERIAL_LEVEL_SENSORS_STATUS);

   // Send a Turn ON request
   if (m_OHDBClient->SendInstallWaitReply(&GetMsg,sizeof(TOHDBGetMaterialLevelSensorStatusMessage),
                                         MaterialSensorStatusAckResponse,
                                         reinterpret_cast<TGenericCockie>(this)) != QLib::wrSignaled)
   {
      m_FlagGetMaterialStatusComUnderUse=false;
      CQLog::Write(LOG_TAG_HEAD_FILLING,"OCB didn't get ack for  \"GetMaterialSensorStatus\" message");
      throw EHeadFilling("OCB didn't get ack for \"GetMaterialSensorStatus\" message");
   }
...
void CHeadFilling::MaterialSensorStatusAckResponse(int TransactionId,PVOID Data,
                                                   unsigned DataLength,TGenericCockie Cockie)
{
   // Head Fillings material sensor status message
   TOHDBMaterialLevelSensorStatusResponse *StatusMsg =
      static_cast<TOHDBMaterialLevelSensorStatusResponse *>(Data);

   // Get a pointer to the instance
   CHeadFilling *InstancePtr = reinterpret_cast<CHeadFilling *>(Cockie);

   //Verify size of message
   if(DataLength != sizeof(TOHDBMaterialLevelSensorStatusResponse))
   {
      FrontEndInterface->NotificationMessage("Head Filling \"MaterialSensorStatusAckResponse\" length error");
      CQLog::Write(LOG_TAG_HEAD_FILLING,"Head Filling \"MaterialSensorStatusAckResponse\" length error");
      return;
   }

   // Update DataBase before Open the Semaphore/Set event.
   if (static_cast<int>(StatusMsg->MessageID) != OHDB_MATERIAL_SENSOR_STATUS)
   {
      FrontEndInterface->NotificationMessage("Head Filling \"MaterialSensorStatusAckResponse\" message id error");
      CQLog::Write(LOG_TAG_HEAD_FILLING,"Head Filling \"MaterialSensorStatusAckResponse\" message id error (0x%X)",
                                       (int)StatusMsg->MessageID);
      return;
   }
   //for(int i = LAST_CHAMBER_TYPE-1; i >= FIRST_CHAMBER_TYPE; i--) //itamar objet check!!! no iteration on i
   InstancePtr->UpdateCurrentThermistorFull(TYPE_CHAMBER_MODEL1 , static_cast<int>(StatusMsg->ModelFullLevel));
   InstancePtr->UpdateCurrentThermistorFull(TYPE_CHAMBER_SUPPORT, static_cast<int>(StatusMsg->SupportFullLevel));
#if defined CONNEX_MACHINE || defined OBJET_MACHINE
   InstancePtr->UpdateCurrentThermistorFull(TYPE_CHAMBER_MODEL2 , static_cast<int>(StatusMsg->Model2FullLevel));
#endif
#ifdef OBJET_MACHINE
   InstancePtr->UpdateCurrentThermistorFull(TYPE_CHAMBER_MODEL3 , static_cast<int>(StatusMsg->Model3FullLevel));
#endif
}
                                ...
                                unsigned DataLength,TGenericCockie Cockie)
{
   ...
   switch (FillingsError)
   {
      ...
      case ERROR_HeadsTemperatureTooLow : // OCB ref.: HEADS_TEMPERATURE_TOO_LOW
         ErrText += "Temperature is out of range.";
         CQLog::Write(LOG_TAG_HEAD_FILLING, ErrText);
      ...
      case ERROR_SupportEOL : // OCB ref.: SUPPORT_FILLING_TIMEOUT
		 ErrText += "support end of liquid. (Support Filling-timeout reached)";
		 CQLog::Write(LOG_TAG_HEAD_FILLING, ErrText);
      ...
      case ERROR_Model3EOL :  // OCB ref.: MODEL3_FILLING_TIMEOUT
		 ...
		 break;
	  case ERROR_S_M3_EOL:
		 ErrText += "model end of liquid. (Support&Model3 Filling-timeout reached)";
		 CQLog::Write(LOG_TAG_HEAD_FILLING, ErrText);
		 Q2RTError = Q2RT_HEADS_FILLING_MONITORING_ERROR;
		 break;
...
{
   int ResinFilterCounterID[NUMBER_OF_PUMPS] = // Maps each one of the 6 material pumps to a resin filter counter id.
							INIT_MSG_COUNTER_ID_ARRAY(_RESIN_FILTER);
...
// Command to get material level sensor status 
TQErrCode CHeadFillingDummy::GetMaterialSensorStatus(void)
{
   return Q_NO_ERROR;
} 
...
CMonitorActivator::CMonitorActivator(CHeadFillingBase* HeadFilling)
{
   if (m_HeadFilling = HeadFilling)
