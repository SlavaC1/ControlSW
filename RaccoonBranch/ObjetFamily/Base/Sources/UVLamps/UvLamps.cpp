...
#ifdef EDEN_250 // This is only for machines with the only UV Lamp !!!!
const char SZ_UV_LAMPS_TURNING_ON[] = "UV Lamp Turning ON"; // single lamp
const char SZ_UV_LAMPS_RE_IGNITION[] = "UV Lamp re-ignition"; // single lamp
#else
...
void CUvLamps::GetUVValueAckResponseMsg (int TransactionId,PVOID Data,unsigned DataLength,TGenericCockie Cockie)
{
   ...
   USHORT NumOfActualReadings = static_cast<USHORT>(ResponseMsg->NumOfActualReadings);
   ...
   InstancePtr->UpdateUvLampValue(NumOfActualReadings, UVvalueEXT);

   USHORT NumOfSaturatedReadings = static_cast<USHORT>(ResponseMsg->NumOfSaturatedReadings);
   USHORT UVMaxValueEXT = static_cast<USHORT>(ResponseMsg->MaxValueEXT);
   FrontEndInterface->UpdateStatus(FE_CURRENT_UV_LAMP_SENSOR_VALUE, (int)UVMaxValueEXT);
   InstancePtr->UpdateUvLampMaxValue(NumOfSaturatedReadings, UVMaxValueEXT);

   USHORT SaturationFlag = static_cast<USHORT>(ResponseMsg->SaturationFlag);
...
//   FrontEndInterface->UpdateStatus(FE_CURRENT_UV_LAMP_SENSOR_VALUE, (int)UVvalueINT);
   InstancePtr->UpdateUvLampMaxDelta(SaturationFlag, UVMaxDeltaEXT);
