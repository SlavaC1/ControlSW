...
bool CQ2RTAutoWizard::Start(void)
{
  ...
  WriteToLogFile(LOG_TAG_SERVICE,"started");
...
bool CQ2RTAutoWizard::Cancel(void)
{
  ...
  Q2PostRTCleanUp();
  WriteToLogFile(LOG_TAG_SERVICE,"was cancelled");
  return ret;
}
...
bool CQ2RTAutoWizard::End(void)
{
  ...
  Q2RTCleanUp();
  Q2PostRTCleanUp();
  WriteToLogFile(LOG_TAG_SERVICE,"ended");
  }
  __finally
  {
...
unsigned CQ2RTAutoWizard::StartAsyncHeatingCycle(const int NumberOfHeadHeaters, bool IncludePreHeater, int* HeadsTemperatures)
{
  ...
  m_BackEnd->TurnHeadHeaters(true, HeadsTemperatures, TOTAL_NUMBER_OF_HEATERS);
...
void CQ2RTAutoWizard::HeadsHeatingCycle(const int TimeOut,const int NumberOfHeadHeaters, bool IncludePreHeater, int* HeadsTemperatures)
{
  unsigned StartTime = StartAsyncHeatingCycle(NumberOfHeadHeaters, IncludePreHeater, HeadsTemperatures);
...
void CQ2RTAutoWizard::HeadsFillingCycle(unsigned int FillingTimeout)
{
   ...
   YieldWizardThread();
   WriteToLogFile(LOG_TAG_GENERAL,"Filling cycle completed.");   
...
bool CQ2RTPythonAutoWizard::Start(void)
{
  ...
  CQLog::Write(LOG_TAG_SERVICE,GetTitle() + " started");
  return ret;
}
...
bool CQ2RTPythonAutoWizard::Cancel(void)
{
  ...
  CQLog::Write(LOG_TAG_SERVICE,GetTitle() + " was cancelled");
  return ret;
}
...
bool CQ2RTPythonAutoWizard::End(void)
{
  ...
  m_BackEnd->SetMachineActionsDate(m_Title);
#endif
  CQLog::Write(LOG_TAG_SERVICE,GetTitle() + " ended");
  return ret;
}
...
COneCheckBoxWizardPage(Wizard,IDS_PLS_CLOSE_DOOR,IDS_DOOR_CLOSED)
{
  m_Wizard = Wizard;
}
