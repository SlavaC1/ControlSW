...
CQ2RTApplication::CQ2RTApplication(void)
{
  ...
  m_SupportReplacementAllowed = false;
}
...
TMachineType CQ2RTApplication::GetMachineType(void)
{
//  return CAppParams::Instance()->MachineType; //objet_machine
...
QString CQ2RTApplication::LogFileChangeEventHandler(const QString& NewLogFileName,TGenericCockie /*Cockie*/)
{
   return CQ2RTApplication::GetTagDictionary();
}
...
void CQ2RTApplication::ApplyParametersChange(void)
{
	...
	CQLog::EnableDisableTag(LOG_TAG_RACCOON, ParamsMgr->RaccoonLog);

	CQLog::EnableDisableTag(LOG_TAG_OBJET_MACHINE, ParamsMgr->ObjetMachineLog);
	CQLog::EnableDisableTag(LOG_TAG_SERVICE,ParamsMgr->ServiceLog);
...
CTester *CQ2RTApplication::GetTester(void)
{
	return m_Tester;
}
...
void CQ2RTApplication::SetSupportReplacementAllowed(bool allowed)
{
	m_SupportReplacementAllowed = allowed;
}
...
bool CQ2RTApplication::GetSupportReplacementAllowed()
{
#ifndef EMULATION_ONLY
	return m_SupportReplacementAllowed;
#else
	return true;
#endif
}
