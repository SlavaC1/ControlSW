...
void CModesManager::EnumerateMaterialModes()
{
	...
	while((ModeName = GetNextModeName(MODEL_DEFAULT_MATERIALS_MODES_DIR)) != "")
	{
		...
		if(ModeName[0] != '~')
		{
            ...
            if(!Q2RTApplication->GetSupportReplacementAllowed() && ( MaterialIndex==SUP707_RESIN_ID || MaterialIndex== SUP706_RESIN_ID) )
			{
...
{		
  if (!Param)
  {
	for(unsigned j = 0; j < m_Keys.size(); j++)
		if (Param = dynamic_cast<CQParameterBase *>(ParamsContainer->FindProperty(m_Keys[j])))
