...
void CBMPFile::Open(const QString& FileName)
{
  ...
  if((BitmapHeader.BitsPerPixel != 1) ||
     (BitmapHeader.Compression != BMP_NO_COMPRESSION))
  {
    fclose(m_fp);
    throw EBMPFile("Only uncompressed 1BPP BMP files are supported");
  }
  ...
  m_LineSizeInBytes = ((m_Width + 31) / 32) * 4;
