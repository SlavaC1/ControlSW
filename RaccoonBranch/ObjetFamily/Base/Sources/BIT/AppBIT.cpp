        ...
        case TOTAL_PHYSICAL_MEMORY_TEST:
          {
            ...
            DWORDLONG TotalMemInMB = Status.dwTotalPhys / (1024 * 1024);
          ...
          break;
/*
        case AVAIL_PHYSICAL_MEMORY_TEST: //bug6182
          {
            // Get memory info structure and check the amount of physical memory
            MEMORYSTATUS Status;
            ::GlobalMemoryStatus(&Status);

            int AvailMemInMB = Status.dwAvailPhys / (1024 * 1024);
            QString StepErrorStr;

            // Check if we have enough physical memory
            if(AvailMemInMB < MIN_AVAIL_PHYSICAL_MEM)
            {
              StepErrorStr = "Not enough available memory";
              SetResultDescription("Environment warning");
              m_WarnFlag = true;
            }

            SetActualVsRequested(QFormatStr("%d MB",MIN_AVAIL_PHYSICAL_MEM),QFormatStr("%d MB",AvailMemInMB),StepErrorStr);
          }
          break;
*/
        case DISK_FREE_TEST:
