...
TQErrCode CMachineManager::StartPrint(void)
{
	...
	WriteServiceInformationToLogFile();
...
TQErrCode CMachineManager::StopPrint(TQErrCode ErrCode)
{
	...
	if(m_JobOrigin == LocalJob)
  {
		if((ErrCode == Q2RT_HOST_TCP_ERROR) || (ErrCode == Q2RT_HOST_RESPONSE))
     {
			CQLog::Write(LOG_TAG_PRINT, QFormatStr("Receive stop print from host during local job - ignoring (StopReason = %s (%d))", PrintErrorMessage(ErrCode).c_str(), ErrCode));
			return Q_NO_ERROR;
     }
  }
  else
	...
	switch(m_CurrentState)
	{
		...
		case msPaused:
			...
			CQLog::Write(LOG_TAG_PRINT, QFormatStr("Print stop reason = %s (%d)", PrintErrorMessage(ErrCode).c_str(), ErrCode));
		...
		case msPrePrint:
			...
			CQLog::Write(LOG_TAG_PRINT, QFormatStr("Pre-Print stop reason = %s (%d)", PrintErrorMessage(ErrCode).c_str(), ErrCode));
		...
		case msStandby1:
		case msStandby2:
			CQLog::Write(LOG_TAG_PRINT, QFormatStr("Receive stop print during standby, StopReason = %s (%d)", PrintErrorMessage(ErrCode).c_str(), ErrCode));
			FrontEndInterface->EnableDisableControl(FE_STOP,false);
			InformStop(ErrCode);
		...
		case msPurge:
		case msFireAll:
			CQLog::Write(LOG_TAG_GENERAL, QFormatStr("Receive stop, StopReason = %s (%d)", PrintErrorMessage(ErrCode).c_str(), ErrCode));
			FrontEndInterface->EnableDisableControl(FE_STOP,false);
...
#ifdef OBJET_MACHINE  //batch number req.
...
TMachineState CMachineManager::PausingPhase()
{
  ...
  CQLog::Write(LOG_TAG_SERVICE,QFormatStr( "Pausing Print Job: name: %s job id: %d", m_CurrentJob->GetJobName().c_str(),m_CurrentJob->GetJobId() ) );
	try
	{
...
TMachineState CMachineManager::StoppedPhase()
{
	...
	if (m_PrintCompleted )
	{
        CQLog::Write(LOG_TAG_SERVICE, "Print Job Completed" );
		...
		CQLog::Write(LOG_TAG_SERVICE, "Print Job Stopped" );
...
TMachineState CMachineManager::TestPatternPhase(void)
{
	...
	{
		// Get the new command
		QLib::TQWaitResult WaitResult = m_TestPatternQueue->Receive(Msg,
		                                QMsToTicks(TIME_TO_GET_TEST_NUMBER_MS));
		if(WaitResult != QLib::wrSignaled)
		...
		if((Err = m_MachineSequencer->TestPattern(Msg.AskToPrintInCurrentPosition, Msg.MoveAxisZInit)) != Q_NO_ERROR)
		{
			if(Err == Q2RT_STOP_DURING_TESTPATTERN)
			{
				...
				throw EMachineManager("Error during test pattern");
		}
	}
	catch(EQException& err)
	{
...
TQErrCode CMachineManager::ExitStandbyState(void)
{
	...
	throw EMachineManager("standby state can be exit manualy only from standby state");
}
...
#if defined CONNEX_MACHINE || defined OBJET_MACHINE //itamar objet check!!! Is this DM with two materials (add for 3 materials)
// Load a local file print job (return a pointer to the file print job object)
...
/*
// Load a local file print job (return a pointer to the file print job object)
TQErrCode CMachineManager::Load4LocalDoublePrintJob(QString FirstModelFileName, QString FirstSupportFileName,QString FirstModel2FileName,QString FirstModel3FileName,
                                              QString SecondModelFileName,QString SecondSupportFileName,QString SecondModel2FileName,QString SecondModel3FileName,
                                              int BitmapResolution,QString ModeStr,int FirstSliceNum,int SecondSliceNum)
{
    int QualityMode,OperationMode;
    if (GetModeFromShortStr(ModeStr, QualityMode, OperationMode) == false)
    ...
    strcpy((TBMPFileName)FileNames1 + MAX_FILE_PATH*TYPE_CHAMBER_MODEL1 , FirstModelFileName.c_str());
    strcpy((TBMPFileName)FileNames1 + MAX_FILE_PATH*TYPE_CHAMBER_MODEL2 , FirstModel2FileName.c_str());
    strcpy((TBMPFileName)FileNames1 + MAX_FILE_PATH*TYPE_CHAMBER_MODEL3 , FirstModel3FileName.c_str());
    strcpy((TBMPFileName)FileNames2 + MAX_FILE_PATH*TYPE_CHAMBER_SUPPORT, SecondSupportFileName.c_str());
    strcpy((TBMPFileName)FileNames2 + MAX_FILE_PATH*TYPE_CHAMBER_MODEL1 , SecondModelFileName.c_str());
    strcpy((TBMPFileName)FileNames2 + MAX_FILE_PATH*TYPE_CHAMBER_MODEL2 , SecondModel2FileName.c_str());
    strcpy((TBMPFileName)FileNames2 + MAX_FILE_PATH*TYPE_CHAMBER_MODEL3 , SecondModel3FileName.c_str());
    TQErrCode ret = LoadLocalDoublePrintJob(FileNames1, FileNames2, BitmapResolution, QualityMode, OperationMode, FirstSliceNum, SecondSliceNum);
    free (FileNames1);
    free (FileNames2);
    return ret;
        ...
        int               SliceNum2)
{
	...
	try
	{
		...
		m_DoubleFilePrintJob->Init(FormatNames(FlexFileNames1, OperationMode==SINGLE_MATERIAL_OPERATION_MODE),
		                           ...
		                           SliceNum1,
		                           SliceNum2,
		                           m_ParamsMgr->GetLayerHeight_um(DPI[QualityMode]) / 1000.0f,
		                           BitmapResolution,
        ...
        int               SliceNum)
{
	...
	try
	{
		...
		for(int i = FIRST_CHAMBER_TYPE; i < LAST_CHAMBER_TYPE; i++)
			strcpy((TBMPFileName)FlexFileNames + i*MAX_FILE_PATH,
			       Q2RTApplication->FlexibleFileName((TBMPFileName)FileNames + i*MAX_FILE_PATH).c_str());
...
void CMachineManager::EnableDisablePrintTimeCounters(bool Enable)
{
  ...
  BackEnd->EnableDisableMaintenanceCounter(HS_TIME_SINCE_LAST_HOW_COUNTER_ID, Enable);
  BackEnd->EnableDisableMaintenanceCounter(HQ_TIME_SINCE_LAST_HOW_COUNTER_ID, Enable);
  BackEnd->EnableDisableMaintenanceCounter(DM_TIME_SINCE_LAST_HOW_COUNTER_ID, Enable);
  ...
  BackEnd->EnableDisableMaintenanceCounter(WIPER_CLEANING_COUNTER_ID, Enable);
...
// return true for HQ mode
bool CMachineManager::IsHQMode()
{
	if ((HQ_INDEX == m_QualityMode )&&( m_OperationMode ==SINGLE_MATERIAL_OPERATION_MODE))
	{
		return true;
	}
	return false;
...
// return true for HS mode
bool CMachineManager::IsHSMode()
{
	if ((HS_INDEX == m_QualityMode )&&( m_OperationMode ==SINGLE_MATERIAL_OPERATION_MODE))
	{
		return true;
	}
	return false;
...
void CMachineManager::WriteServiceInformationToLogFile()
{
	QString msg, QualityMode;
	if((m_CurrentState == msPaused) || (m_CurrentState == msPausing))
		msg = "Print Job Resumed After Pause:";
	else if(m_CurrentJob->IsContinue())
		msg = "Print Job Resumed:";
	else
		msg = "New Print Job Start:";
	...
	CQLog::Write(LOG_TAG_SERVICE,QFormatStr( "%s name: %s job id: %d Current quality mode: %s", msg.c_str(),
																						m_CurrentJob->GetJobName().c_str(),
																						m_CurrentJob->GetJobId(),
																						QualityMode.c_str() ) );

	CQLog::Write(LOG_TAG_SERVICE,QFormatStr( "%s version: %s Build %s %s. package: %s",  APPLICATION_NAME,
																					QGetAppVersionStr().c_str(),
																					GetVersionDate().c_str(),
																					GetVersionTime().c_str(),																					m_MachineSequencer->GetLicenseManager()->GetLicensedPackagesAsString().c_str() ) );

	CBackEndInterface* BackEnd = CBackEndInterface::Instance();
	msg = 	"OCB SW: " + BackEnd->GetVersion(VERSION_OCB) + ", "	   +
			"OHSB SW: " + BackEnd->GetVersion(VERSION_OHDB_SW) + ", "  +
			"OHSB HW: " + BackEnd->GetVersion(VERSION_OHDB_HW) + ", "  +
			"MSC SW: " + BackEnd->GetVersion(VERSION_MCB_SW) + ", "    +
			"MSC HW: " + BackEnd->GetVersion(VERSION_MCB_HW) + ", "    +
			"EDEN PCI: " + BackEnd->GetVersion(VERSION_EDEN_PCI) + ", " ;

	CQLog::Write(LOG_TAG_SERVICE,msg.c_str());
	CBackEndInterface::Instance()->WriteMaintenanceCountersToCSLog();
}
