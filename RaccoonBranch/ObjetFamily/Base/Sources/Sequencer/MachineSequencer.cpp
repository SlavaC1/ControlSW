...
// Constructor
CMachineSequencer::CMachineSequencer(const QString& Name) :
    CQSequencer(Name),
    m_IgnorePowerByPass(false)
{
    ...
    try
    {
        ...
        m_TrayHeater = new CTrayDummy("TrayHeater");
        ...
        if(!m_ParamsMgr->OCB_Emulation)
        {
            ...
            if(m_ParamsMgr->TrayHeaterAndInsertedBypass)
                m_TrayHeater = new CTrayDummy("TrayHeater");
            else
...
TQErrCode CMachineSequencer::PowerUpSequence(void)
{
  ...
  CBackEndInterface::Instance()->EnableDisableMaintenanceCounter(RESTART_COMPUTER_COUNTER_ID, true);
    ...
    try
    {
		...
		// Initializes the FCB hardware and starts the thread
		QSleep(m_ParamsMgr->HardwareInitDelay_mSec.Value());
		m_Container->InitRaccoon();
...
//  Turn Heads heating OFF
        ...
        //start to take time to turn off valvules
        if((Err = m_Actuator->SetDirtPumpOnOff(false)) != Q_NO_ERROR)
            throw EMachineSequencer("Actuator message error:Dirt pump.");
...
void CMachineSequencer::PrePrintSequence(bool IsContinueJob) //itamarDeleteJob
{
    ...
    UpdateLastError(Q_NO_ERROR);
  bool isPreviousStateStandby1 = (m_CurrentMachineState == msStandby1);
    ...
    if((Err = m_TrayHeater->VerifyTrayInserted()) != Q_NO_ERROR)
    {
  ...
  // In case of electrical power interruption home all axis first (including Z)
		//********************************************************************
		...
		if((Err = m_Motors->MoveToAbsolutePosition(AXIS_Z,m_ParamsMgr->MaxPositionStep[AXIS_Z],muSteps)) != Q_NO_ERROR)
			throw EMachineSequencer(Err);
		...
		// Now it is save to home all axes (Z is the last one)
	}
  ...
  //In certain conditions we need to wait additional time to allow head heaters to stabilize.
  //Such conditions are weight tests, that are very short and need high accuracy.
  if(!isPreviousStateStandby1 && m_ParamsMgr->IsWeightTest)
  {
	FrontEndInterface->UpdateStatus(FE_MACHINE_SUB_STATUS,"Temperature Stabilization");

	unsigned long StartTime = QGetTicks();
	while ((QGetTicks() - StartTime) < QSecondsToTicks(m_ParamsMgr->WeightTestHeadHeatersDelayInSec))
	{
  ...
  //May be one actuator should be turned here
  //Turn pump gifa ON
  if ((Err = m_Actuator->SetRollerPumpOnOff(true) ) != Q_NO_ERROR)
        throw EMachineSequencer(Err);
    ...
    {
	 m_Actuator->SetWastePumpOnOff(true);
...
//This Procedure
TQErrCode CMachineSequencer::ContinueSequence()
{
    TQErrCode Err;
...
TQErrCode CMachineSequencer::Standby2Sequence(void)
{
    ...
    if(m_Actuator->GetActuatorOnOff(ACTUATOR_ID_WASTE_PUMP))
    {
        //Turn pump gifa OFF
...
TQErrCode CMachineSequencer::MotorInitBeforeFirstLayer(void)
{
    ...
    m_SequencerBlock.HalfLayerHeightDeltaInUM = 0;
    m_SequencerBlock.CurrentModelHeight = 0.0f;
    m_SequencerBlock.SingleLayerHeightInUM = m_ParamsMgr->GetLayerHeight_um(m_ParamsMgr->DPI_InXAxis);
...
// After each periodic time we re-initiate the time counting.
   ...
   static time_t WastePumpOffTime = 0;
    time_t CurrentTime;
    TQErrCode Err;
    ...
    if(StartPrinting)
	  ...
	  if (m_ParamsMgr->DoPurgeBeforePrint)
		WastePumpOffTime = CurrentTime + (time_t)m_ParamsMgr->WastePumpOnTimeLimitForPrintInSec;
	  else
        WastePumpOffTime = (time_t)0;
        return Q_NO_ERROR;
    }
    ...
    {   
      ...
      if ((Err = PurgeSequence(PERFORM_HOME, true)) != Q_NO_ERROR)
			if(Err != Q2RT_RACCOON_TAG_WEIGHT_REACHED_ZERO)
         return Err;
	  ...
	  //If waste pump already working, add additional time to timeout
	  if(!WastePumpOffTime)
		WastePumpOffTime =  CurrentTime + (time_t)m_ParamsMgr->WastePumpOnTimeLimitForPrintInSec;
	  else
		WastePumpOffTime+=  (time_t)m_ParamsMgr->WastePumpOnTimeLimitForPrintInSec;
   ...
   if(WastePumpOffTime && CurrentTime>= WastePumpOffTime)
   {
       m_Actuator->SetWastePumpOnOff(false);
	   WastePumpOffTime = 0;
   }
...
TQErrCode CMachineSequencer::Shutdown(void)
{
  ...
  CBackEndInterface::Instance()->EnableDisableMaintenanceCounter(RESTART_COMPUTER_COUNTER_ID, false);
    try
    {
        ...
        m_Roller->BumperEnableDisable(false);
        m_AmbientTemperature->SetMaterialCoolingFanOnOff(FAN_OFF);
        m_Actuator->SetDirtPumpOnOff(false);
        //RSS, itamar added, turn RSS off
        if(m_IsSuctionSystemExist)
        {
...
TQErrCode CMachineSequencer::PrepareOHDBModelConfig(void)
{
    ...
    if(m_ParamsMgr->AmbientFanControlByPass)
        ...
        int SubPassNumber)
{
    ...
    // HEAD_SIZE_BYTE is the amount of bytes required for a single head
    if(m_ParamsMgr->PrintDirection == BIDIRECTION_PRINT_MODE)
        BufferLength = LayerParms->NoOfFires * 2 * 2 * HEAD_SIZE_BYTE;
    else
        BufferLength = LayerParms->NoOfFires * 2 * HEAD_SIZE_BYTE;

    FrontEndInterface->UpdateStatus(FE_MACHINE_SUB_STATUS, "Write Layer Data");

    // 300/150 DPI in Y printing
    if(GetHeadsNum() == 8 && (m_ParamsMgr->DPI_InYAxis != 600))
    {
        if(Q_NO_ERROR != (Err = FIFOPCI_WriteAsync(m_SequencerBlock.BufferLayerPrintPtr + BufferLength * 4 * PassNumber, static_cast<unsigned>(BufferLength))))
            return Err;
    }
    else
    {
        // 600 DPI in Y printing
        if(Q_NO_ERROR != (Err = FIFOPCI_WriteAsync(m_SequencerBlock.BufferLayerPrintPtr + BufferLength * 4 * (PassNumber * 2 + SubPassNumber), static_cast<unsigned>(BufferLength))))
            return Err;
    }
...
//                            OCB
//------------------------------------------------------------
// Commads preformed by OCB during pre-printing
...
// Commads preformed by OCB during printing
...
TQErrCode CMachineSequencer::GotoHeadInspectionPositionSequencer(void)
{
    ...
    if((Err = m_Motors->GoWaitAbsolutePositionSequence(AXIS_Z, CAppParams::Instance()->MaxPositionStep[AXIS_Z],
              MORE_TIME_TO_FINISH_MOVEMENT)) != Q_NO_ERROR)
        return Err;

    long PosInSteps = CONFIG_ConvertXmmToStep(CAppParams::Instance()->XCleanHeadsPosition);
...
TQErrCode CMachineSequencer::GotoTrayRemovalPositionSequencer(void)
{
    ...
    if((Err = m_Motors->GoWaitAbsolutePositionSequence(AXIS_Z,
              CONFIG_ConvertMMToStep(AXIS_Z, m_ParamsMgr->ZAxisEndPrintPosition), MORE_TIME_TO_FINISH_MOVEMENT)) != Q_NO_ERROR)
        return Err;
...
TQErrCode CMachineSequencer::PurgeSequence(bool PerformHome, bool PerformSinglePurgeInSequence)
{
	...
	TQErrCode Err = UpdatePurgeMaterialConsuption(PerformSinglePurgeInSequence);
	...
	if(Err == Q2RT_TEMPERATURE_IS_NOT_OK_IN_OCB || Err == Q2RT_PURGE_ERROR)
    {
...
// Update material consumption in Raccoon module during purge sequence
TQErrCode CMachineSequencer::UpdatePurgeMaterialConsuption(bool PerformSinglePurgeInSequence)
{
	TQErrCode Err = Q_NO_ERROR;

	int purges  = PerformSinglePurgeInSequence ? 1 : m_ParamsMgr->NumOfPurgesInSequence.Value();
	QString str = PerformSinglePurgeInSequence ? "Single purge sequence." : QFormatStr("Sequence of %d purges.", purges).c_str();

	// The value for each active tank. Consumtion is updated in mGr
	unsigned int Weight = GramsToMilliGrams(m_ParamsMgr->RaccoonPurgeConsumptionSingleMaterial_Gr.Value() * purges);

	for(int i = FIRST_CHAMBER_TYPE; i < LAST_CHAMBER_TYPE; i++)
	{
		TTankIndex TankIndex = m_Container->GetActiveTankNum(i);

		CQLog::Write(LOG_TAG_RACCOON, QFormatStr("%s Update purge material consuption: Chamber: %s (%d), Active tank: %s (%d)",
												 str.c_str(), ChamberToStr(static_cast<TChamberIndex>(i)).c_str(),
												 i,           TankToStr(static_cast<TTankIndex>(TankIndex)).c_str(), (int)TankIndex));

		if(TankIndex != NO_TANK)
		{
			if(m_Container->GetTankCurrentWeightInTag(TankIndex) == 0)
				return Q2RT_RACCOON_TAG_WEIGHT_REACHED_ZERO;

			m_Container->UpdateConsumption(TankIndex, Weight);
		}
	}
...
TQErrCode CMachineSequencer::FireAllAfterPurgeSequence(void)
{
    ...
    if((Err = m_Motors->SetMotorEnableDisable(AXIS_T, MOTOR_ENABLE)) != Q_NO_ERROR)
        return Err;
    if((Err = m_Motors->OpenWasteTankEx(true)) != Q_NO_ERROR)  // if this function is called, the tank will open without the basket (of the new purge unit) touching the heads
        return Err;
...
CTrayBase* CMachineSequencer::GetTrayInstance()
{
    return m_TrayHeater;
}
...
void CMachineSequencer::CheckCartridgeExistence(bool& CheckIsDone)
{
    ...
    m_CartridgesCheckState = CHECK_OVERCONSUMPTION;
    return;
}

void CMachineSequencer::CheckOverconsumption(bool& CheckIsDone)
{
	for(int i = FIRST_CHAMBER_TYPE; i < LAST_CHAMBER_TYPE; i++)
	{
		if(m_Container->IsTankInOverconsumption(m_Container->GetActiveTankNum(i)))
		{
			CQLog::Write(LOG_TAG_GENERAL, ChamberToStr((TChamberIndex)i) + " weight is in overconsumption");
			m_ContainerReplacementPromptTime = QGetTicks();
			FrontEndInterface->ShowCartridgeError(FE_REPLACE_CARTRIDGE, (TChamberIndex)i, false);
...
void CMachineSequencer::CheckLiquidsWeight(bool& CheckIsDone, bool& TurnOnUvLamps)
{
    ...
    if(!m_Container->GetWasteWeightStatusDuringPrinting())
    {
        ...
        m_ContainerReplacementPromptTime = QGetTicks();
        FrontEndInterface->ShowCartridgeError(FE_REPLACE_CARTRIDGE, TYPE_CHAMBER_WASTE, false);
        m_CartridgesCheckState = CHECK_UV_LAMPS_TIME_OUT;
        m_Chamber = TYPE_CHAMBER_WASTE;
        return;
    }
    m_CartridgesCheckState = CHECK_CARTRTIDGE_EXISTENCE;
    CheckIsDone = true;
...
void CMachineSequencer::CheckCartridgeDialogTimeOut(TQErrCode& RetVal, bool& CheckIsDone, bool& TurnOnUvLamps)
{
    ...
    {
        if(QGetTicks() > (m_ContainerReplacementPromptTime + QSecondsToTicks(CAppParams::Instance()->CartridgeErrorContinuePrintTime)))
        {
...
TQErrCode CMachineSequencer::CheckCartridgesStatus()
{
    ...
    while(!Canceled && !CheckIsDone)
    {
        switch(m_CartridgesCheckState)
        {
			...
			case CHECK_OVERCONSUMPTION:
				CheckOverconsumption(CheckIsDone); 
				break;
...
{ 
    ...
    do
    {
        ...
        for(int i = FIRST_CHAMBER_TYPE; i < LAST_CHAMBER_TYPE; i++)
        {
			...
			TankOk = m_Container->IsWeightOk((TChamberIndex)i);
			if(! TankOk)
			{
				if((RetValue = FrontEndInterface->ShowCartridgeError(FE_REPLACE_CARTRIDGE, i, true)) != Q_NO_ERROR)
					return RetValue;
			...
			TankOk = (! m_Container->IsTankInOverconsumption(m_Container->GetActiveTankNum((TChamberIndex)i)));
			if(! TankOk)
			{
				if((RetValue = FrontEndInterface->ShowCartridgeError(FE_REPLACE_CARTRIDGE, i, true)) != Q_NO_ERROR)
					return RetValue;
			}// bug: 2330, 2331 Check consumption 0 here
        }

        //Check Waste
        TankOk = m_Container->IsActiveLiquidTankInserted(TYPE_CHAMBER_WASTE);
        if(!TankOk)
        {
            if((RetValue = FrontEndInterface->ShowCartridgeError(FE_INSERT_CARTRIDGE, TYPE_CHAMBER_WASTE, true)) != Q_NO_ERROR)
                return RetValue;
        }

        TankOk = m_Container->IsActiveLiquidTankEnabled(TYPE_CHAMBER_WASTE);
        if(!TankOk)
        {
            if((RetValue = FrontEndInterface->ShowCartridgeError(FE_REPLACE_CARTRIDGE, TYPE_CHAMBER_WASTE, true)) != Q_NO_ERROR)
                return RetValue;
        }

        if(!m_Container->GetWasteWeightStatusBeforePrinting())
        {
            if((RetValue = FrontEndInterface->ShowCartridgeError(FE_REPLACE_CARTRIDGE, TYPE_CHAMBER_WASTE, true)) != Q_NO_ERROR)
                return RetValue;
        }
...
int CMachineSequencer::GetAvgUVValue(int Lamp, int PSLevel)
{
    ...
    for(int i = 0; i < UVNumberOfSamples; i++)
    {
...
void CMachineSequencer::CheckUVLampsTimeout(TQErrCode& RetVal, bool& CheckIsDone) // todo -oNobody -cNone: function name is misleading
{
	...
	if((0 < timeout) && (QLib::wrSignaled == m_CartridgeErrorDlgCloseEvent.WaitFor(timeout)))
    {
...
void CMachineSequencer::WaitForCartridgeReplacement(TQErrCode& RetVal, bool& CheckIsDone, bool& TurnOnUvLamps)
{
    ...
    if((0 < timeout) && (QLib::wrSignaled == m_CartridgeErrorDlgCloseEvent.WaitFor(timeout)))
    {
        ...
        if(m_Roller->SetRollerOnOff(ROLLER_ON) != Q_NO_ERROR)
            throw EMachineSequencer(Q2RT_ROLLER_TURNING_ON);
        TurnOnUvLamps = true;
#ifdef OBJET_MACHINE  //flag for CHECK_CARTRTIDGE_EXISTENCE state to check if update for batch number is needed
        //m_tankReplaced = true;
#endif
        return;
    }
