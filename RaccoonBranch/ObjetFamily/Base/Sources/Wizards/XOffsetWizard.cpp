...
CXOffsetWizard::CXOffsetWizard(void) : CQ2RTAutoWizard(IDS_XO_WIZARD,true,IN_PROCESS_IMAGE_ID)
{
  ...
  { 
	...
	pPage->SubTitle = "Printing may take up to 20 minutes.";
...
void CXOffsetWizard::UpdateParameters()
{
  ...
  for (int i = TOTAL_NUMBER_OF_HEADS_HEATERS - 2; i >= 0; i--)
  {
...
/*#ifdef OBJET_MACHINE
    m_ParamMgr->HeadMapArray[M0 - i] += PIXELS_TO_MICRONS_1200DPI*CorrectionValue;
#else */
