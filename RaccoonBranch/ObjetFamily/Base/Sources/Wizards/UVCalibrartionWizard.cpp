...
IF YOU DO NOT AGREE TO THESE CONDITIONS, CLICK 'CANCEL' AND CONTACT OBJET CUSTOMER SUPPORT. YOU MUST ACCEPT THE CONDITIONS TO CONTINUE.";
...
In case of any failure, or suspected failure, of your printer, request service from Objet Customer Support.";
  ...
  UserAgreementPage->Label5  = "By checking the 'I agree', you indicate your acceptance of the conditions and compliance with the guidelines and instructions, specified in the Objet's User Guide.";
...
//#endif
  ...
  PlaceSensorPage->Strings.Add(LOAD_STRING(IDS_PLACE_SENSOR));
  PlaceSensorPage->Strings.Add(LOAD_STRING(IDS_HOOD_IS_CLOSED));
...
void CUVCalibrationWizard::PageLeave(CWizardPage *WizardPage, TWizardPageLeaveReason LeaveReason)
{
  ...
  switch(WizardPage->GetPageNumber())
  {
    ...
    {
      bool    NeedMoreCapacity;
      bool    ImprovePossible;
      ...
      else
      {
         if (abs(PercentDiff) > PERCENTAGE_THRESHOLD)
         {
            NeedMoreCapacity = (PercentDiff > 0);
            ImprovePossible  = GetNextPSValue(NeedMoreCapacity, PercentDiff, m_UVLampIndex);
...
bool CUVCalibrationWizard::InsertLampValuePageEnter(CWizardPage* WizardPage)
{
  ...
  Page->Refresh();
  unsigned EndTime = QGetTicks() + QSecondsToTicks(USER_ENTRY_RESPONSE_TIMEOUT);
  return true;
}
