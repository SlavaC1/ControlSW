...
const int   A2D_ROOM_TEMPERATURE_FOR_HEADS           = 2000;
const int   A2D_ROOM_TEMPERATURE_FOR_BLOCK           = 2000;
...
bool CQ2RTSHRWizardBase::CoolingDownPageEnter(CWizardPage* WizardPage)
{
    ...
    for(int i = FIRST_HEAD; i < LAST_HEAD; i++)
        HeadsTemperatures[i] = A2D_ROOM_TEMPERATURE_FOR_HEADS;
    // Set the temperatures for the block heaters
    for(int i = FIRST_BLOCK; i < LAST_BLOCK; i++)
        HeadsTemperatures[i] = A2D_ROOM_TEMPERATURE_FOR_BLOCK;
    ...
    SelectedHeadsHeatingCycle(m_ParamMgr->HeadsCoolingTimeoutSec, HeadsTemperatures);
...
void CQ2RTSHRWizardBase::ReplaceHeadPageEnter(CWizardPage* WizardPage, int PageIndex)
{
    ...
    Page->Strings.Clear();
    for(int i = TOTAL_NUMBER_OF_HEADS - 1 ; i >= 0; i--)
        if(m_HeadsToBeReplaced[i])
            Page->Strings.Add(GetHeadName(i) + " replaced");
    Page->Refresh();
    SetNextPage(PageIndex);
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CQ2RTSHRWizardBase::ReplaceHeadPageLeave(CWizardPage* WizardPage)
{
    ...
    FOR_ALL_HEADS(i)
    {
        if(m_HeadsToBeReplaced[i])
        {
            m_BackEnd->ResetMaintenanceCounter(HeadsCounterID[i]);
            //check if support head was replaced
            if(firstEnter && !(m_ParamMgr->IsModelHead(i)))
            {
                firstEnter = false;
            }
        }
...
void CQ2RTSHRWizardBase::ReInsertHeadPageEnter(CWizardPage* WizardPage)
{
    CCheckBoxWizardPage *Page = dynamic_cast<CCheckBoxWizardPage *>(WizardPage);
    Page->Strings.Clear();
    for(int i = TOTAL_NUMBER_OF_HEADS_HEATERS - 1 ; i >= 0; i--)
        if(m_HeadsToBeReplaced[i])
            Page->Strings.Add("Head " + GetHeadName(i) + " was re-inserted");
    Page->Refresh();
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
bool CQ2RTSHRWizardBase::FillBlockPageEnter(CWizardPage* WizardPage, T_AxesTable a_AxesTable)
{
    ...
    unsigned Timeout   = m_ParamMgr->FillingTimeout - 30;
...
void CQ2RTSHRWizardBase::HeadsHeatingCycle(const int TimeOut, int* HeadsTemperatures, bool IncludePreHeater)
{
    CQ2RTAutoWizard::HeadsHeatingCycle(TimeOut, TOTAL_NUMBER_OF_HEATERS, IncludePreHeater, HeadsTemperatures);
}

void CQ2RTSHRWizardBase::HeadsOnlyHeatingCycle(const int TimeOut, int* HeadsTemperatures)
{
    ...
    HeadsHeatingCycle(TimeOut, HeadsTemperatures, false);
}

void CQ2RTSHRWizardBase::SelectedHeadsHeatingCycle(const int TimeOut, int* HeadsTemperatures)
{
    CParamScopeRestorePointArray<bool, TOTAL_NUMBER_OF_HEATERS> Mask(m_ParamMgr->HeatersMaskTable);
    for(int i = 0 ; i < TOTAL_NUMBER_OF_HEADS_HEATERS; i++)
    {
        if(m_HeadsToBeReplaced[i] && i < TOTAL_NUMBER_OF_HEADS_HEATERS)
            m_ParamMgr->HeatersMaskTable[i] = false;
    }
    HeadsHeatingCycle(TimeOut, HeadsTemperatures);
}
...
void CQ2RTSHRWizardBase::OpenHistoryFile()
{
    QString DirName  = WIZARD_HISTORY_FULL_DIR_NAME;
    QString FileName = DirName + LOAD_STRING(IDS_HISTORY_FILE_NAME);

    QString Content  = LOAD_STRING(IDS_QR_HISTORY_RESTRICTIONS);
    CBackEndInterface::Instance()->ForceFileCreation(FileName, Content);
    m_HeadReplacementFile = fopen(FileName.c_str(), "at");
}

void CQ2RTSHRWizardBase::CloseHistoryFile()
{
...
//   }
    ...
    HeadsDrainingCycle(WizardPage, m_ParamMgr->HF_StabilizationTime, true);
    if(CHECK_NOT_EMULATION(m_ParamMgr->OCB_Emulation))
