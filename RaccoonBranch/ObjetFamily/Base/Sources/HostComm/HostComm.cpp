...
#else
#define MAX_INCOMING_DATA_SIZE 160000000 //bug 5811
#define MAX_INCOMING_RESINN_DATA_SIZE 4000000 //bug 5811
#define MAX_SLICE_MEM_SIZE 60000000 //bug 6171
#endif
#ifdef _DEBUG
#define MAX_INCOMING_DATA_SIZE 40000000 //for debug only
#define MAX_INCOMING_RESINN_DATA_SIZE 1000000//bug 5811
#define MAX_SLICE_MEM_SIZE 15000000   
#endif
  ...
  : CQThread(true,"SlicePreProcessorThread",false)
{
  ...
  m_SlicesAllocator.Init((SLICE_QUEUE_SIZE + 2) * NUMBER_OF_CHAMBERS,MAX_SLICE_MEM_SIZE); //bug 6171 
...
bool CSlicePreProcessor::AllocateTmpBuffers(PBYTE TmpBuffers[],ULONG SliceUncompressedSize)
{
  ...
  {
     // Prepare buffers
     TmpBuffers[i] = (PBYTE)m_SlicesAllocator.GetBlock(SliceUncompressedSize);
                                             ...
                                             ULONG SliceUncompressedSize)
{
		...
		if(m_NumOfAddModelResins!=*pNumberOfAdditionalModelResins)
		{
		   ...
		   if(BackEnd!=NULL) 
			 if(!BackEnd->IsWizardRunning()) //prevent message from popping up during wizard (may stuck the wizard)
...
Arr[15] = 1;
const BYTE SUPPORT_MASK = 0x11;
const BYTE CHAMBER1_MASK = 0x22;
const BYTE CHAMBER2_MASK = 0x33;
const BYTE CHAMBER3_MASK = 0x44;

		bool ret = true;
        TBmp* BMPArray[NUMBER_OF_CHAMBERS];

        BMPArray[TYPE_CHAMBER_MODEL1] = (TBmp *)malloc((SlicePackage->model->len)/NUMBER_OF_CHAMBERS);
        BMPArray[TYPE_CHAMBER_MODEL2] = (TBmp *)malloc((SlicePackage->model->len)/NUMBER_OF_CHAMBERS);
        BMPArray[TYPE_CHAMBER_MODEL3] = (TBmp *)malloc((SlicePackage->model->len)/NUMBER_OF_CHAMBERS);
        BMPArray[TYPE_CHAMBER_SUPPORT] = (TBmp *)malloc((SlicePackage->model->len)/NUMBER_OF_CHAMBERS);

        PBYTE bmpPtr = (SlicePackage->model->bmp);
        //unsigned char bmpPtr[4] = {0x30,0x22,0x01,0x32}; //test
        for(unsigned int i=0,k=0,j=8,l=0; i<SlicePackage->model->len;)
        {
          if(j==0)
          {
            j=8;
          ...
          (BMPArray[TYPE_CHAMBER_MODEL1]->bmp[k]) = (Arr[((bmpPtr[l]^~CHAMBER1_MASK)&0xF0)>>4]<<(j-1))|(Arr[(bmpPtr[l]^~CHAMBER1_MASK)&0x0F]<<(j-2))|(BMPArray[TYPE_CHAMBER_MODEL1]->bmp[k]);

          (BMPArray[TYPE_CHAMBER_MODEL2]->bmp[k]) = (Arr[((bmpPtr[l]^~CHAMBER2_MASK)&0xF0)>>4]<<(j-1))|(Arr[(bmpPtr[l]^~CHAMBER2_MASK)&0x0F]<<(j-2))|(BMPArray[TYPE_CHAMBER_MODEL2]->bmp[k]);

          (BMPArray[TYPE_CHAMBER_MODEL3]->bmp[k]) = (Arr[((bmpPtr[l]^~CHAMBER3_MASK)&0xF0)>>4]<<(j-1))|(Arr[(bmpPtr[l]^~CHAMBER3_MASK)&0x0F]<<(j-2))|(BMPArray[TYPE_CHAMBER_MODEL3]->bmp[k]);

          (BMPArray[TYPE_CHAMBER_SUPPORT]->bmp[k]) = (Arr[((bmpPtr[l]^~SUPPORT_MASK)&0xF0)>>4]<<(j-1))|(Arr[(bmpPtr[l]^~SUPPORT_MASK)&0x0F]<<(j-2))|(BMPArray[TYPE_CHAMBER_SUPPORT]->bmp[k]);
        ...
        if (0 != *pNumberOfAdditionalSupportResins)
        {
		   CQLog::Write(LOG_TAG_HOST_COMM, "Invalid number of additional support resins");
           FrontEndInterface->ErrorMessage("Invalid number of additional support resins.\nClose and restart the printer application.");
           return false;
        }
...
void CSlicePreProcessor::ProcessSlicePackage(TSlicePackageType *SlicePackage)
{
  CAppParams* ParamMgr = CAppParams::Instance();
    ...
    try
	{
	  ...
	  {
        //if(!m_SlicesAllocator.IsAllocated()) //bug 6171
          //m_SlicesAllocator.Init((SLICE_QUEUE_SIZE + 2) * NUMBER_OF_CHAMBERS,SliceUncompressedSize); //bug 6171
	  ...
	  CSlice* Slice = new CSlice(TmpBuffers,SlicePackage->header.sliceNumber,SlicePackage->model->rect.width,
							   ...
							   SlicePackage->model->rect.y,SlicePackage->header.bitmapRes);
...
CHostComm::CHostComm() : CQThread(true,"HostCommThread")
{
  ...
  try
  {
     for(int i=0;i<SLICE_QUEUE_SIZE;i++)
        m_IncomingMsgData[i] = new unsigned char[MAX_INCOMING_DATA_SIZE];
  }
  catch (std::bad_alloc& ba)
  {
     for(int i=0;i<SLICE_QUEUE_SIZE;i++)
  ...
  try
  {
     m_IncomingMsgResinData = new unsigned char [MAX_INCOMING_RESINN_DATA_SIZE];
  }
  catch (std::bad_alloc& ba)
  {
      ...
      case MSG_LENGTH:
           ...
           if(iCntr >= sizeof(unsigned long))  // Entire field has been read.
           {
              if(m_IncomingMsgLength>0)      // Message has data.
              {
                 ...
                 //-----------------
                 iCntr = 0;
                 if (m_IncomingMsgLength<MAX_INCOMING_DATA_SIZE)
		 {
...
//  --------------------------------------------
	...
	case CMD_CONTINUE_MODEL:
		 ...
		 m_HostPrintJob->Init(this,
								 ...
								 ((StartModel->PrintMode == HIGH_QUALITY)? HQ_INDEX : HS_INDEX),
								 ((StartModel->NumberOfAdditionalModelResins == 0)? SINGLE_MATERIAL_OPERATION_MODE : DIGITAL_MATERIAL_OPERATION_MODE),
								 StartModel->YMul,
		 ...
		 int BmpsReceivedFromHostArr[NUMBER_OF_CHAMBERS];
		 int k=0, i=0;
		 ...
		 {		   
		   ...
		   for(; i<StartModel->NumberOfAdditionalSupportResins; i++)
		   {
			 ...
			 {
			   if(strcmp(StartModel->AdditionalMaterials[i],gParamsMgr->TypesArrayPerChamber[k].c_str())==0)
			   {
...
//  CAppParams*        ParamsMgr         = CAppParams::Instance();
  ...
  StatusMsg.DoorStatus             = MachineSequencer->GetDoorInstance()->GetDoorStatus();
  StatusMsg.UVLampsStatus          = MachineSequencer->GetUVLampsInstance()->GetUVLampsStatus();CDoorBase * pDoor = MachineSequencer->GetDoorInstance();
  if (pDoor)
  ...
  StatusMsg.NumberOfAdditionalModelResins   = ModelActiveContainersNum()   - MINIMUM_REQUIRED_MODEL_NUM;
  StatusMsg.NumberOfAdditionalSupportResins = SupportActiveContainersNum() - MINIMUM_REQUIRED_SUPPORT_NUM;

  //if(0 != StatusMsg.NumberOfAdditionalModelResins)
  //{
  ...
  Size -= sizeof(TContainerInfo)*((MM_RESIN_NUM-MINIMUM_REQUIRED_RESIN_NUM)-totalNumOfAdditionalContainers);

  CQLog::Write(LOG_TAG_PRINT,"Weight: Model: %d Support: %d Waste: %d",
		StatusMsg.ModelContainer.ContainerWeight,StatusMsg.SupportContainer.ContainerWeight,StatusMsg.WasteContainer.ContainerWeight);
...
void CHostComm::ReplyVersions(void)
{
  ...
  strcpy(PrinterVersionMsg.HostPrinterProtocol,"6");
...
void CHostComm::ReplyPrinterData(BYTE RequestedData)
{
  ...
  PrinterDataMsg.AverageLayer600DPI  = ((m_ParamsMngr->DPI_InXAxis == DPI[HQ_INDEX]) ? m_ParamsMngr->LayerHeightDPI_um : DEFAULT_LAYER_HEIGHT[HQ_INDEX]) ;
  PrinterDataMsg.ModelXResolution    = m_ParamsMngr->DPI_InXAxis;
  ...
  PrinterDataMsg.AverageLayer1200DPI = ((m_ParamsMngr->DPI_InXAxis == DPI[HS_INDEX]) ? m_ParamsMngr->LayerHeightDPI_um : DEFAULT_LAYER_HEIGHT[HS_INDEX]) ;
...
#ifdef OBJET_MACHINE
  for(int i = FIRST_MODEL_CHAMBER_INDEX+1, j=0; i < LAST_MODEL_CHAMBER_INDEX; i++)
  {
  ...
  for(int i = FIRST_SUPPORT_CHAMBER_INDEX+1, j=0; i < LAST_SUPPORT_CHAMBER_INDEX; i++)
  {
...
#else
  map<QString, bool>  ModelMaterials,SupportMaterials;
  for(int i = FIRST_MODEL_CHAMBER_INDEX; i < LAST_MODEL_CHAMBER_INDEX; i++)
	  ModelMaterials[gParamsMgr->MRW_HostTypesPerChamber[i]] = true;
  int i = 0;
  for(map<QString, bool>::iterator Iter = ModelMaterials.begin(); Iter != ModelMaterials.end(); Iter++)
  {
	 if (Iter->first.compare(gParamsMgr->MRW_HostTypesPerChamber[FIRST_MODEL_CHAMBER_INDEX]) != 0)
	 {
		PrinterDataMsg.NumberOfAdditionalModelResins++;
		strcpy (PrinterDataMsg.AdditionalMaterials[i++], Iter->first.c_str());
        Size += sizeof(TResinName);
     }
  }

  for(int i = FIRST_SUPPORT_CHAMBER_INDEX; i < LAST_SUPPORT_CHAMBER_INDEX; i++)
	  SupportMaterials[gParamsMgr->MRW_HostTypesPerChamber[i]] = true;
  i = 0;
  for(map<QString, bool>::iterator Iter = SupportMaterials.begin(); Iter != SupportMaterials.end(); Iter++)
  {
     if (Iter->first.compare(gParamsMgr->MRW_HostTypesPerChamber[FIRST_SUPPORT_CHAMBER_INDEX]) != 0)
     {
		PrinterDataMsg.NumberOfAdditionalSupportResins++;
        strcpy (PrinterDataMsg.AdditionalMaterials[i++], Iter->first.c_str());
        Size += sizeof(TResinName);
  ...
  for (TLicensedModesList::iterator it = ModesList.begin() ; it < ModesList.end() ; it++)
  {
	  ...
	  switch(it->ModeTag)
	  {
		  case mmHighQualityMode:
			LicensedInfoMsg.OperationModes |= static_cast<BYTE>(pow((double)2, HIGH_QUALITY - 1));
			break;
