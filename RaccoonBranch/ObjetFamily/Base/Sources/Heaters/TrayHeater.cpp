...
// Constructor
//-----------------
CTrayDummy::CTrayDummy(const QString& Name) : CTrayBase(Name)
{
   m_FlagCanSendGetStatusMsg = false;
}
...
bool CTrayBase::IsTrayInserted(void)
{
...
//------------------------------------------------------
TQErrCode CTrayHeater::GetTrayInsertedStatus(void)
{     
  if(!m_ParamsMgr->RemovableTray) //runtime objet
  {
...
// for Eden500 or Eden350 tray is ever IN
...
// -------------------------------------------------------
void CTrayHeater::TrayInsertedStatusResponse(int TransactionId,PVOID Data,
                                 unsigned DataLength,TGenericCockie Cockie)
{
   // Build the Tray Heater turn on message
...
TQErrCode CTrayDummy::SetTrayOnOff(bool OnOff)
{
   UpdateTrayIsOn(OnOff);
...
TQErrCode CTrayDummy::SetTrayTemperatureMsg(int Temperature= CAppParams::Instance()->StartTrayTemperature,
...
TQErrCode CTrayDummy::GetTrayStatus(void)
{
...
TQErrCode CTrayDummy::GetTrayInsertedStatus(void)
{
   UpdateTrayInserted(true);
   return Q_NO_ERROR;
}

TQErrCode CTrayDummy::WaitForTrayInTemperature(void)
{
   return Q_NO_ERROR;
}
...
void CTrayBase::WaitUntilTrayInserted()
{
  if( ! IsTrayInserted() )
  {
     ...
     //QMonitor.AskYesNo("Please insert the tray to continue!");
...
/*
   //Check Tray is IN
   if(!Tray->CheckContinueIfTrayIsOut())
      {
      CQLog::Write(LOG_TAG_MOTORS,"Movement canceled by user");
      return Q_NO_ERROR;
      }
*/
}
...
/*
bool CTrayBase::CheckContinueIfTrayIsOut(void)
{
  if(!IsTrayInserted())
     {
     if (QMonitor.AskYesNo("WARNING Tray is OUT of place. Do you want to continue ?"))
        return true;
     else
        return false;
     }
...
TQErrCode CTrayBase::Enable(bool StartPrintTemperature)
{
  ...
  if((Err = VerifyTrayInserted() ) != Q_NO_ERROR)
     {
     if(Err==Q2RT_TRAY_IS_NOT_INSERTED)
        throw ETrayHeater("TRAY IS OUT. Stop printing");
     else
        return Err;
     }
