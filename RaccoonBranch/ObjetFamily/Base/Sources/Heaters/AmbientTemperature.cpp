...
void CAmbientTemperatureBase::UpdateCurrentAmbientStatus()
{
   ...
   if(!m_ParamsMgr->AmbientTemperatureByPass)
   {
...
void CAmbientTemperature::HoodFanControl()
{
  ...
  if (!m_ParamsMgr->AmbientFanControlByPass)
  {
    ...
    if (CurrentTemperature <= (m_ParamsMgr->AmbientTemperatureFanControl - HYSTERESIS_MARGIN))
    {
      ActuatorPtr->SetHoodFanIdle();
//      CQLog::Write(LOG_TAG_GENERAL,"Ambient: Turning OFF Hoodfan. Current ambient temp.: %d",
...
//      CQLog::Write(LOG_TAG_GENERAL,"Ambient: Turning ON Hoodfan. Current ambient temp.: %d",
//                                     (int)CurrentTemperature);
    }
  }
  else // if (m_ParamsMgr->AmbientFanControlByPass)
  {
    ActuatorPtr->SetHoodFanOnOff(true);
  }
}
...
TQErrCode CAmbientTemperatureBase::SetOHDBAmbientParams(void)
{
    ...
    UpdateFanLimitA2D (m_ParamsMgr->AmbientTemperatureFanControl.Value());
...
{
   UpdateOHDBCurrentTemperatureA2D(m_ParamsMgr->AmbientTemperatureFanControl.Value()+1);
   UpdateCurrentAmbientStatus();
