...
SENSOR_GAIN_OFFSET_ARR = [    
	...
	[  # Desktop machines
		[ # 30
			[ 0.5438, 0 ], # Right lamp - Gain, Offset
		],
		[ # 24
			[ 0.5438, 0 ], # Right lamp - Gain, Offset
		]
...
def GetYstart():
	return FOM.GetLampYPosition_INT(FOM.RIGHT_LAMP)
	
def GetXstart(lamp):
	res = FOM.GetLampXPosition_INT(lamp) - (0.5 * TravPath)
...
def Scan(start, end, VelocityX):
	...
	# read the Sum
	SumValueRead = MachineSequencer.GetUVSumRead( True, False ) # 1st arg => read INT sensor address, 2nd arg => update fresh parameters from OCB / Lotus (this is False because no need to refresh. we read the most updated UV params exactly when we finished scanning in 'FinalizeMeasurement')
...
def UpdateDS():
	...
	FOM.printEx( "Dark Signal (INT) = " + str(DS) )
...
def GetSaturationFlag( ): #no changes for now for INT sensor (seems stable)
	return False
	
def SetSensorCalibrationMode( isSensorCalibrationMode ): #no changes for now for INT sensor (seems stable)
	return	
'''
