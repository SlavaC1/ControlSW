...
# 	Path:		$HeadURL: http://srv-ire-svn/svn/ControlSW/Branches/Slava/RaccoonForObjet260Release/ObjetFamily/Base/PythonScripts/Wizards/UVPCalibration.py $ 
#
################################################################################################
...
PLACE_SENSOR_IMAGE_GNR_EXT      = 115
...
tpOperationPage = GenericCustomWizardPage('UV Intensity Measuring',-1, wpPreviousDisabled | wpNextDisabled | wpHelpNotVisible, wptUVLampsCalibrationWizardPage)
...
tpCorrectionFactorPage.FieldsValues[0] = str( FOM.GetObjetUVSensorCalibratedGain(FOM.RIGHT_LAMP) ) # or Left lamp...
...
tpVerifyGNR_EXTInPlace = CheckBoxWizardPage('Check Printer', PLACE_SENSOR_IMAGE_GNR_EXT, wpNextWhenSelected | wpPreviousDisabled | wpHelpNotVisible)
tpVerifyGNR_EXTInPlace.SubTitle = "Confirm before continuing:"
tpVerifyGNR_EXTInPlace.Strings = [" "]

tpRemoveEXTPage = CheckBoxWizardPage('Check Printer', PLACE_SENSOR_IMAGE_GNR_EXT, wpNextWhenSelected | wpPreviousDisabled | wpHelpNotVisible)
...
# enable + init + home for X, Y, Z, T
def InitMotors(x, y, z, t ):
	if(t):
...
def OnCancel(Page):
	...
	InitMotors(True, True, False, False)
	CleanUp()
...
def OnPageEnter(Page):
	...
	## GNR pages
		...
		if( Debug ):
			WaitMS(1000)
		if( FirstInit ):
			InitMotors(True, True, True, True)
			SimpleMove(AXIS_Z, AppParams.Z_StartPrintPosition + JigHeight)
			FirstInit = False
		else:
			InitMotors(True, True, False, False)  # init again - only X and Y
		GotoNextPage()
	##################
	...
	## INT sensor pages		
		...
		if( FirstInitINT ):
			...
			InitMotors(True, True, True, False)
		GotoNextPage()
