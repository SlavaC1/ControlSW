...
bool CBackEndInterface::IsHQMode()
{
  CMachineManager *Mgr = Q2RTApplication->GetMachineManager();
  return (Mgr->IsHQMode());
}
...
void CBackEndInterface::TurnHeadHeaters(bool On, int* Temperature, int NumOfHeads)
{
...
bool CBackEndInterface::MotorGetAxisRightLimit(TMotorAxis Axis)
{
  CMotorsBase* Motor = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetMotorsInstance();
...
bool CBackEndInterface::MotorGetAxisLeftLimit(TMotorAxis Axis)
{
  CMotorsBase *Motor = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetMotorsInstance();
...
void CBackEndInterface::SetDirtPump(bool WasteOnOff, bool RollerOnOff)
{
  CActuatorBase *Actuators = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetActuatorInstance();
  Actuators->SetDirtPumpOnOff( WasteOnOff,  RollerOnOff);
}

void CBackEndInterface::SetDirtPump(bool OnOff)
{
  SetDirtPump(OnOff,OnOff);
}
...
bool CBackEndInterface::IsActiveLiquidTankInserted(int Chamber)
{
  ...
  ret = Containers->IsActiveLiquidTankInserted(Chamber);
  return ret;
}
...
bool CBackEndInterface::IsTankInOverconsumption(int TankIndex)
{
	CContainerBase* Containers = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetContainerInstance();
 	return Containers->IsTankInOverconsumption(static_cast<TTankIndex>(TankIndex));
}
...
unsigned int CBackEndInterface::GetTankWeightInGram(int Tank) // todo -oNobody -cNone : use TTankIndex here and elsewhere
{
  CContainerBase* Containers = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetContainerInstance();
...
unsigned int CBackEndInterface::GetTankCurrentWeightInTag(TTankIndex TankIndex)
{
  CContainerBase* Containers = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetContainerInstance();
  return Containers->GetTankCurrentWeightInTag(TankIndex);
}
...
void CBackEndInterface::EnableConfirmationBypass(const QString ModeName, QString Dir)
{
  CModesManager::Instance()->EnableConfirmationBypass(ModeName, Dir);
}

// Don't allow bypassing the 'Are you sure you want to update..."
void CBackEndInterface::DisableConfirmationBypass(const QString ModeName, QString Dir)
{
  CModesManager::Instance()->DisableConfirmationBypass(ModeName, Dir);
}
  ...
  switch(WhatToGet)
  {
    ...
    case VERSION_OCB:
      Result = QIntToStr(Seq->GetOCBStatusInstance()->GetOCBSWExternalVersion()) + "."
               + QIntToStr(Seq->GetOCBStatusInstance()->GetOCBSWInternalVersion());
      break;

    case VERSION_OCB_A2D:
      Result = QIntToStr(Seq->GetOCBStatusInstance()->GetOCB_A2D_SWExternalVersion()) + "."
               + QIntToStr(Seq->GetOCBStatusInstance()->GetOCB_A2D_SWInternalVersion());
      break;

    case VERSION_OHDB_SW:
      Result = QIntToStr(Seq->GetHeadStatusInstance()->GetOHDBSWExternalVersion()) + "."
               + QIntToStr(Seq->GetHeadStatusInstance()->GetOHDBSWInternalVersion());
      break;
    ...
    case VERSION_OHDB_A2D:
      Result = QIntToStr(Seq->GetHeadStatusInstance()->GetOHDB_A2D_SWExternalVersion()) + "."
               + QIntToStr(Seq->GetHeadStatusInstance()->GetOHDB_A2D_SWInternalVersion());
      break;
	...
	case HW_FW_VERSION_MSC1:
	  Result = QIntToStr(Seq->GetOCBStatusInstance()->GetMSC1HWVersion()) + "."
			   + QIntToStr(Seq->GetOCBStatusInstance()->GetMSC1FWVersion());
	  break;

	case HW_FW_VERSION_MSC2:
	  Result = QIntToStr(Seq->GetOCBStatusInstance()->GetMSC2HWVersion()) + "."
			   + QIntToStr(Seq->GetOCBStatusInstance()->GetMSC2FWVersion());
	  break;
...
void CBackEndInterface::B_End_SetHeadMinVoltage()
{
  CHeadHeatersBase *HeadHeaters = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetHeadHeatersInstance();

  HeadHeaters->SetHeadMinVoltage();
}

bool CBackEndInterface::IsItNewEEProm(const int& headNum)
{
  CHeadHeatersBase *HeadHeaters = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetHeadHeatersInstance();

  return HeadHeaters->IsItNewEEProm(headNum);
}
void CBackEndInterface::SetAndMeasureHeadVoltage(int HeadNumber, float ReqVoltage)
{
  ...
  if(HeadNumber==0)
  {
   //if it's the first head, the application find the min voltage
    B_End_SetHeadMinVoltage();
  }
...
void CBackEndInterface::WriteMaintenanceCountersToCSLog()
{
  CMaintenanceCounters::GetInstance()->WriteCountersToCSLog();
}
...
QString CBackEndInterface::GetContainerTagID(TTankIndex CartridgeID)
{
  CContainerBase* Containers = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetContainerInstance();
...
bool CBackEndInterface::IsContainerInserted(TTankIndex CartridgeID)
{
  CContainerBase* Containers = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetContainerInstance();
  return Containers->IsContainerInserted(CartridgeID);
}
...
float CBackEndInterface::EstimateHeadVoltage(int HeadNum, int PotVal)
{
    CHeadHeatersBase *HeadHeaters = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetHeadHeatersInstance();
    return HeadHeaters->EstimateHeadVoltage(HeadNum, PotVal);
}

int CBackEndInterface::CalculatePotValue(int HeadNum, float ReqVoltage)
{
    CHeadHeatersBase *HeadHeaters = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetHeadHeatersInstance();
    return HeadHeaters->CalculatePotValue(HeadNum, ReqVoltage);
}
...
bool CBackEndInterface::IsTankWeightOk(int Tank)
{
	CContainerBase* Containers = Q2RTApplication->GetMachineManager()->GetMachineSequencer()->GetContainerInstance();
	return Containers->IsTankWeightOk(static_cast<TTankIndex>(Tank));
}
