...
void __fastcall TBITViewerForm::TestsListViewMouseDown(TObject *Sender,
...
//	CBackEndInterface *BEInterface = CBackEndInterface::Instance();
	...
	if(TestAttributes & taDoorLock)
	   PrepareBitDlg ->AddAttribute(DOOR_CLOSED);
	...
	if(TestAttributes & taTrayInsert)
		...
		if(TestAttributes & taDoorLock)
		{
			 if(BEInterface->CheckIfDoorIsClosed() == Q_NO_ERROR)
			{
				...
				{
					PrepareBitDlg->EnableCheckBox(DOOR_CLOSED,true);
					PrepareBitDlg->SelectCheckBox(DOOR_CLOSED,false);
					checkAgain = true;
				}
				else
				{
				  PrepareBitDlg->EnableCheckBox(DOOR_CLOSED,false);
				  PrepareBitDlg->SelectCheckBox(DOOR_CLOSED,true);
				  PrepareBitDlg->RemoveAttribute(DOOR_CLOSED);
				 }
			}
			else
              {
...
void TBITViewerForm::ShowPrepareBitDialog(bool needToCreateChBx)
{
   ...
   if(PrepareBitDlg->NeedToShow())
   {
	...
	if(PrepareBitDlg->IsAttributeExists(DOOR_CLOSED))
	{
	   if(BEInterface->CheckIfDoorIsClosed() == Q_NO_ERROR)
			{
				if(BEInterface->EnableDoor(true,showDialog) != Q_NO_ERROR)
