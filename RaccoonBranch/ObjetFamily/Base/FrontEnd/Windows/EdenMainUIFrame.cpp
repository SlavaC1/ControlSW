...
{
  delete m_ResinReplacementNewWizard;
  delete m_QuickSHRWizard;
  delete m_BumperCalibrationWizard;
  delete m_XOffsetWizard;
  delete m_UVCalibrationWizard;
  delete m_VacuumCalibrationWizard;
  delete m_SinglePointCalibrationWizard;
  delete m_PrintingPositionWizard;
  delete m_UVReplacementWizard;
  delete m_HeadServiceWizard;
  delete m_ShutdownWizard;
  delete m_WiperCalibrationWizard;
}
...
void TMainUIFrame::HandleStatusUpdateMessage(TMessage &Message)
{
  ...
  switch(Message.WParam)
  {
    ...
    CASE_MESSAGE_ALL_TANKS(FE_TANK_DISABLING_HINT)
    {
...
{
  try {
  	  m_ResinReplacementNewWizard    = new CResinReplacementNewWizard;
  } catch (EQException& Err) {
	  //This wizard is set as the default base for all other wizards (SetWizardForm)
	  //If it can't be initialized - abort everything.
	  //AskOKCancel is used, instead of ErrorMessage, so that the popup appears sync.
	  QMonitor.AskOKCancel("There was a general error during initialization process.\r\n" +
		Err.GetErrorMsg());
	  TerminateProcess(GetCurrentProcess(),-911);
  }
  ...
  m_ShutdownWizard				 = new CShutdownWizard;
  m_WiperCalibrationWizard		 = new CWiperCalibrationWizard;
 
  m_ResinReplacementNewWizard->SetWizardForm(WizardForm);
...
{
	CBackEndInterface::Instance()->SetOnlineOffline(false);
	//MAX_NUMBER_OF_MODEL_CHAMBERS = 3	->   for GUI
	...
	RunWizard(m_ResinReplacementNewWizard); //start off page number 0

  //restart application
  if (m_ResinReplacementNewWizard->IsRestartRequired())
  {
    AppRestartFlag = true;
	MainForm->Close();
  }    
...
{
  OpenGeneralDevicesAction->Checked = !OpenGeneralDevicesAction->Checked;
...
{
  OpenHeadsControlAction->Checked = !OpenHeadsControlAction->Checked;
...
void __fastcall TMainUIFrame::OpenMotorsControlActionExecute(TObject *Sender)
{
  ...
  OpenMotorsControlAction->Checked = !OpenMotorsControlAction->Checked;
...
{
  OpenActuatorsAndSensorsAction->Checked = !OpenActuatorsAndSensorsAction->Checked;
...
void TMainUIFrame::UpdatePrintTime(TDateTime CurrentTime)
{
  int h,m,s;
...
void __fastcall TMainUIFrame::LoadCellCalibrationActionExecute(TObject *Sender)
{
  ...
  WizardForm->RegisterCustomPageViewer(wptStableValueWizardPage, new CStableValuePageViewer);
  #endif
  #ifdef OBJET_MACHINE
