...
void __fastcall TLocalJobForm::OkButtonClick(TObject *Sender)
{
  ...
  for(int i = FIRST_CHAMBER_TYPE; i < LAST_CHAMBER_TYPE; i++)
  {
	  ...
	  {
		FEParamsMgr->LastFileNamesArray1[i] = FName.c_str();
      }
  ...
  FName     = ExtractShortPathName((TBMPFileName) m_FileNamesArray1 + TypeSelector1ComboBox->ItemIndex*MAX_FILE_PATH).c_str();
  FEParamsMgr->LastFileNamesArray1[TypeSelector1ComboBox->ItemIndex] =
               ( (FName.compare("") == 0) ? "?" : FName.c_str() );

  FName     = ExtractShortPathName((TBMPFileName) m_FileNamesArray2 + TypeSelector2ComboBox->ItemIndex*MAX_FILE_PATH).c_str();
  FEParamsMgr->LastFileNamesArray2[TypeSelector2ComboBox->ItemIndex] =
               ( (FName.compare("") == 0) ? "?" : FName.c_str() );
                                   ...
                                   int&               SliceNum)
{
  ...
  {
    FName = ParamsMgr->LastFileNamesArray1[i];
    FName = ( (FName.compare("?") == 0) ? "" : ExpandFileName(FName.c_str()).c_str() );
    strcpy((TBMPFileName)FileNames + MAX_FILE_PATH*i, FName.c_str());
  }
  QualityMode                  = ParamsMgr->LastQualityMode;
  OperationMode                = ParamsMgr->LastOperationMode;
  BitmapResolution             = DPI[QualityMode];
