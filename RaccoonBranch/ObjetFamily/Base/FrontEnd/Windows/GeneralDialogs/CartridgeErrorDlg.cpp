...
void TCartridgeErrorForm::UpdateTanksState(void)
{
	...
	for(int t = FIRST_TANK_INCLUDING_WASTE_TYPE; t < LAST_TANK_INCLUDING_WASTE_TYPE; t++)
	{
		if(m_TanksStatus.find(static_cast<TTankIndex>(t)) != m_TanksStatus.end())
		{
			...
			{
				if(! m_BackEndInterface->IsTankWeightOk(t) || m_BackEndInterface->IsTankInOverconsumption(t))
				{
					// The tank seems to be empty

					// If the tank has been just inserted
					if(QTicksToSeconds(QGetTicks() - m_BackEndInterface->GetLastInsertionTimeInTicks(static_cast<TTankIndex>(t))) <  CAppParams::Instance()->EmptyCartridgeDetectionDelayInSec)
					{
				...
				{
					if(QTicksToSeconds(QGetTicks() - m_BackEndInterface->GetLastInsertionTimeInTicks(static_cast<TTankIndex>(t))) <  CAppParams::Instance()->NonEmptyCartridgeDetectionDelayInSec)
					{
						Status = OldStatus; // Give the user the chance to release the tank, and weight sensor to respond
					}
			...
			{
				m_TanksStatus[static_cast<TTankIndex>(t)] = Status;
				break;
			}
		}
	}
...
{
	int ret = 0;
	for(TTankIndex2TTankStatusMapIter iter =m_TanksStatus.begin();
	        iter!=m_TanksStatus.end(); iter++)
	{
...
//fixme - unneccesary function - 4/6/06 14:29

{
	// IsWasteContainerInserted() looks at the weight (no micro switch)
	if(m_DlgType == FE_INSERT_CARTRIDGE)
	{
		if(m_BackEndInterface->IsTankInAndEnabled(TYPE_TANK_WASTE))
		{
		  ...
		  m_BackEndInterface->SetDirtPump(m_WastePrevOnOff,m_RollerPrevOnOff);
			}
			ReturnValue(mrOk);
		}
	}
	else
	{
		...
		if(m_WasteRemovalDetected && m_BackEndInterface->IsTankInAndEnabled(TYPE_TANK_WASTE) && m_BackEndInterface->IsWeightOk(TYPE_CHAMBER_WASTE))
		{
...
{
  m_WastePrevOnOff = m_BackEndInterface->GetActuatorOnOff(ACTUATOR_ID_WASTE_PUMP);
  m_RollerPrevOnOff = m_BackEndInterface->GetActuatorOnOff(ACTUATOR_ID_ROLLER_PUMP);
	// Turning off the Waste pump and the Roller pump
	m_BackEndInterface->SetDirtPump(false);
