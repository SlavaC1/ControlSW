...
//			#endif
		...
		case SET_PRINTING_HEADS_PARAMS_MSG:
		{
			...
			SetPrintingHeadsTaskParams.HeadDelay = PrintingParamsMsg->HeadDelay;
			SetPrintingHeadsTaskParams.PulseWidth2 = PrintingParamsMsg->PulseWidth2;
			SetPrintingHeadsTaskParams.PulseDelay2 = PrintingParamsMsg->PulseDelay2;
			SetPrintingHeadsTaskParams.PostPulserDelay2	=  PrintingParamsMsg->PostPulserDelay2;
			SetPrintingHeadsTaskParams.HeadDelay2 = PrintingParamsMsg->HeadDelay2;
			SetPrintingHeadsTaskParams.HeadTable = 	PrintingParamsMsg->HeadTable;
...
void SetPrintingHeadsParamsTask (BYTE Arg)
{
	...
	SET_PULSE_WIDTH_AND_DELAY_2,
	WAIT_FOR_PULSE_OP_2_DONE,	
	SET_POST_PULSER_DELAY_2,
	WAIT_FOR_POST_PULSER_OP_2_DONE,

	HEAD_DRIVE_SELECT_TABLE,
	WAIT_FOR_HEAD_DRIVE_SELECT_TABLE_DONE,
	...
	switch (Arg)
	{
		...
		case SET_PULSE_WIDTH_AND_DELAY:
			if (PrintDrv_SetPulseWidthAndDelay(SetPrintingHeadsTaskParams.PulseWidth, SetPrintingHeadsTaskParams.PulseDelay) == PRINT_DRV_NO_ERROR)
		...
		case SET_HEAD_1_DELAY:
			if (PrintDrv_SetHeadDelay(0,SetPrintingHeadsTaskParams.HeadDelay) == PRINT_DRV_NO_ERROR)
		...
		case SET_HEAD_2_DELAY:
			if (PrintDrv_SetHeadDelay(1,SetPrintingHeadsTaskParams.HeadDelay2) == PRINT_DRV_NO_ERROR)
		...
		case WAIT_FOR_HEAD_2_OP_DONE:
			...
			{	
				if (Status == PRINT_DRV_NO_ERROR) 
 		 			SchedulerLeaveTask(SET_PULSE_WIDTH_AND_DELAY_2);
				else
					SchedulerLeaveTask(SET_HEAD_2_DELAY);
			}
			else
				SchedulerLeaveTask(WAIT_FOR_HEAD_2_OP_DONE);
			break;
         case SET_PULSE_WIDTH_AND_DELAY_2:
			if (PrintDrv_SetPulseWidthAndDelay2(SetPrintingHeadsTaskParams.PulseWidth2, SetPrintingHeadsTaskParams.PulseDelay2) == PRINT_DRV_NO_ERROR)
 		  		SchedulerLeaveTask(WAIT_FOR_PULSE_OP_2_DONE);
			else
				SchedulerLeaveTask(SET_PULSE_WIDTH_AND_DELAY_2);
			break;

		case WAIT_FOR_PULSE_OP_2_DONE:
			Status = PrintDrv_IsSetOperationDone();
			if (Status != PRINT_DRV_OP_NOT_DONE)
			{	
				if (Status == PRINT_DRV_NO_ERROR) 
 		 			SchedulerLeaveTask(SET_POST_PULSER_DELAY_2);
				else
					SchedulerLeaveTask(SET_PULSE_WIDTH_AND_DELAY_2);
			}
			else
		...
		case SET_POST_PULSER_DELAY_2:
			if (PrintDrv_SetPostPulserDelay2(SetPrintingHeadsTaskParams.PostPulserDelay2) == PRINT_DRV_NO_ERROR)
 		  		SchedulerLeaveTask(WAIT_FOR_POST_PULSER_OP_2_DONE);
			else
				SchedulerLeaveTask(SET_POST_PULSER_DELAY_2);
			break;

		case WAIT_FOR_POST_PULSER_OP_2_DONE:
			Status = PrintDrv_IsSetOperationDone();
			if (Status != PRINT_DRV_OP_NOT_DONE)
			{	
				if (Status == PRINT_DRV_NO_ERROR) 
 		 			SchedulerLeaveTask(HEAD_DRIVE_SELECT_TABLE);
				else
					SchedulerLeaveTask(SET_POST_PULSER_DELAY_2);
			}
			else
		...
		case HEAD_DRIVE_SELECT_TABLE:
			if (PrintDrv_HeadDriveSelectTable(SetPrintingHeadsTaskParams.HeadTable) == PRINT_DRV_NO_ERROR)
 		  		SchedulerLeaveTask(WAIT_FOR_HEAD_DRIVE_SELECT_TABLE_DONE);
			else
				SchedulerLeaveTask(HEAD_DRIVE_SELECT_TABLE);
			break;

		case WAIT_FOR_HEAD_DRIVE_SELECT_TABLE_DONE:
			Status = PrintDrv_IsSetOperationDone();
			if (Status != PRINT_DRV_OP_NOT_DONE)
			{	
				if (Status == PRINT_DRV_NO_ERROR) 
