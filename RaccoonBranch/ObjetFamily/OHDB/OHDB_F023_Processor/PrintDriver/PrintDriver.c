...
#define HEAD_1_DELAY_REG_ADDR							0x24
#define HEAD_2_DELAY_REG_ADDR							0x81	  // all the other address were not used by HW(approved by Eldad Hen)


#define DIRECTION_OF_PLOT_REG_ADDR			        	0x75
#define BUMPER_PEG_ENABLE_ADDR            				0x76

#define HEAD_DRIVE_SELECT_REG_ADDR                      0X80 
#define PULSE_WIDTH_2_AND_DELAY_REG_2_ADDRESS			0x82
#define POST_PULSER_DELAY_2_RED_ADDR 					0x83
...
// Module variables
// ================
const BYTE HeadDelaysRegArray[NUM_OF_HEADS_OPTIONS] = {
...
/****************************************************************************
 *
 *  NAME        : PrintDrv_SetPulseWidthAndDelay2 
 *
 *  DESCRIPTION : Set the pulse width and the pulse delay for new head                                        
 *
 ****************************************************************************/
...
// build the msg to be sent to the xilinx
// --------------------------------------
	SetPulseMsg.Address = PULSE_WIDTH_2_AND_DELAY_REG_2_ADDRESS;
	SetPulseMsg.Data = Delay;
	SetPulseMsg.Data = SetPulseMsg.Data << 8;
	SetPulseMsg.Data |= Width;
 ...
 *
 ****************************************************************************/
PRINT_DRV_STATUS PrintDrv_SetHeadDelay(BYTE HeadNumOption, BYTE Delay)
{
	TXilinxMessage xdata HeadDelayMsg;

// build the msg to be sent to the xilinx
// --------------------------------------
	HeadDelayMsg.Address = HeadDelaysRegArray[HeadNumOption];
...
/****************************************************************************
 *
 *  NAME        : PrintDrv_SetPostPulserDelay2 
 *
 *  DESCRIPTION : Set the post pulser delay register for new head                                        
 *
 ****************************************************************************/
...
// build the msg to be sent to the xilinx
// --------------------------------------
	DelayMsg.Address = POST_PULSER_DELAY_2_RED_ADDR;
	DelayMsg.Data = Delay;	
...
/****************************************************************************
 *
 *  NAME        : PrintDrv_HeadDriveSelectTable
 *
 *  DESCRIPTION : Set the head table: '0' represent old head, '1' represent new head                                   
 *
 ****************************************************************************/
PRINT_DRV_STATUS PrintDrv_HeadDriveSelectTable(WORD HeadTable)
{
	TXilinxMessage xdata HeadTableMsg;

// build the msg to be sent to the xilinx
// --------------------------------------
	HeadTableMsg.Address = HEAD_DRIVE_SELECT_REG_ADDR;
	HeadTableMsg.Data = HeadTable;	
 	
// send the mesage to the xilinx 
// ------------------------------
	if (XilinxWrite(&HeadTableMsg) == XILINX_SEND_FAILED)
		return PRINT_DRV_OP_FAIL;
		...
		{	
 					...
 					FireAllTaskParams.TransactionId,TRUE) == EDEN_PROTOCOL_NO_ERROR)
			{
			  FireAllStatus = FIRE_ALL_ACTIVE;
				SchedulerLeaveTask(RESET_STATE_MACHINE_1);
			}
			else
		...
		case WAIT_FOR_READ_DIAG_DONE:
			...
			{	
				if (XilinxStatus == XILINX_SEND_SUCCESS)
				{
				  ...
				  if (XilinxData == 0)
					  SchedulerLeaveTask(RESET_STATE_MACHINE_2);
					else
					{
					  SchedulerLeaveTask(RESET_DIAGNOSTICS_2);
					}
