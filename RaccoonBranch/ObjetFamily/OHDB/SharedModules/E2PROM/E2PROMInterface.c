...
#define E2PROM_BUFFER_LENGTH			20
...
E2PROM_STATUS E2PROMReadBlock(BYTE DeviceNum, WORD Address, BYTE Length)
{
	...
	Message = (BYTE*) malloc ( (E2PROM_READ_INST_LENGTH+Length) * sizeof(BYTE)); 
...
// send the message using the spi
	SpiSend(E2PROM_READ_INST_LENGTH + Length, E2PROMDataIn, E2PROMCallBack);
...
// check if all the bytes are on the same page
	if (((Address + Length)/16) > (Address/16))
...
// prepare the message to be send using the spi
	...
	// send the message using the spi
	SpiSend(E2PROM_WRITE_INST_LENGTH + Length, E2PROMDataIn,E2PROMCallBack);
...
//		else
//TODO:			memcpy (Data, &(E2PROMDataIn[2]), Length); // the tow first byte is junk received after th read insturction
	}
