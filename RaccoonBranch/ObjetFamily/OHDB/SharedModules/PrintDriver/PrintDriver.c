...
/****************************************************************************
 *
 *  NAME        : PrintDrv_SetBumperSensitivity 
 *
 *  DESCRIPTION : Set the bumper sensitivity                                          
 *
 ****************************************************************************/
 
PRINT_DRV_STATUS PrintDrv_SetBumperSensitivity(BYTE Sensitivity)
{
/*	if (PotenmtrWriteValue(Sensitivity) == POTENMTR_NO_ERROR)
		return PRINT_DRV_NO_ERROR;
...
/****************************************************************************
 *
 *  NAME        : PrintDrv_IsSetBumperSensitivityDone 
 *
 *  DESCRIPTION : Is the 'Set Bumper Sensitivity' operation done?                                          
 *
 ****************************************************************************/
 
PRINT_DRV_STATUS PrintDrv_IsSetBumperSensitivityDone()
{
/*	POTENMTR_STATUS Status;

// check if the write to the potentiometer is done
// -----------------------------------------------
	Status = PotenmtrGetWriteStatus();
	if (Status != POTENMTR_BUSY)
	{
		if (Status == POTENMTR_SEND_SUCCESS)
			return PRINT_DRV_NO_ERROR;
 ...
 *
 ****************************************************************************/
void PrintDrv_SetFireAllParams(WORD Frequency, WORD Time, WORD Head, BYTE DestId, BYTE TransactionId)
{
  ...
  WAIT_FOR_READ_DIAG_DONE,
	...
	switch (Arg)
	{
		...
		{	
			...
			if (OHDBEdenProtocolSend((BYTE*)&Msg,sizeof(TAckMsg),FireAllTaskParams.DestId,
		...
		case SET_FREQUENCY:
			...
			Msg.Data = 33333333/(4*FireAllTaskParams.FireAllFrequency);
			if (XilinxWrite(&Msg) == XILINX_NO_ERROR)
		...
		case SET_NUM_OF_FIRES: 		
			...
			Msg.Data = 2000;	// could be any value larger then 2
			if (XilinxWrite(&Msg) == XILINX_NO_ERROR)
				...
				if (Status == PRINT_DRV_NO_ERROR) 
 		 		{
					...
					SchedulerTaskSleep(FireAllTaskParams.FireAllTime);
				}
				else
					SchedulerLeaveTask(GO);
			}
