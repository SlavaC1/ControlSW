...
void TOCBTesterForm::Open(CEdenProtocolEngine *ProtocolEngine)
{
  if (m_ProtocolClient == NULL)
  {
    ...
    m_DisplayThread = new COCB_DisplayThread(m_ProtocolClient,
                                            &m_A2DDisplayEvent,
                                            &m_ActuatorsDisplayEvent,
                                            &m_InputsDisplayEvent,
                                            &m_PowerDisplayEvent,
                                            &m_TrayStatusDisplayEvent,
                                            &m_IsTrayInDisplayEvent,
                                            &m_UVLampsStatusDisplayEvent,
                                            &m_FillingDisplayEvent,
                                            &m_PurgeDisplayEvent,
                                            &m_DoorDisplayEvent,
                                            &m_LiquidTanksDisplayEvent,
                                            &m_LiquidTanksWeightsDisplayEvent);
...
void __fastcall TOCBTesterForm::FormCreate(TObject *Sender)
{
  ...
  SHighThCheckBox->Checked = ParamsMgr->ActiveThermistors[SUPPORT_M3_CHAMBER_THERMISTOR];
...
void TOCBTesterForm::LiquidTanksWeightsHandler(int TransactionId, PVOID Data, unsigned DataLength, TGenericCockie Cockie)
{
  ...
  InstancePtr->m_ModelTank1Weight   = LiquidTanksWeightsMsg->ModelTank1Weight;
  InstancePtr->m_ModelTank2Weight   = LiquidTanksWeightsMsg->ModelTank2Weight;
  InstancePtr->m_ModelTank3Weight   = LiquidTanksWeightsMsg->ModelTank3Weight;

  InstancePtr->m_ModelTank4Weight   = LiquidTanksWeightsMsg->ModelTank4Weight;
  InstancePtr->m_ModelTank5Weight   = LiquidTanksWeightsMsg->ModelTank5Weight;
  InstancePtr->m_ModelTank6Weight   = LiquidTanksWeightsMsg->ModelTank6Weight;

  InstancePtr->m_SupportTank1Weight = LiquidTanksWeightsMsg->SupportTank1Weight;
  InstancePtr->m_SupportTank2Weight = LiquidTanksWeightsMsg->SupportTank2Weight;


  InstancePtr->m_WasteTankWeight    = LiquidTanksWeightsMsg->WasteTankWeight;
...
void TOCBTesterForm::LiquidTanksStatusHandler(int TransactionId, PVOID Data, unsigned DataLength, TGenericCockie Cockie)
{
  ...
  InstancePtr->m_DisplayThread->EnableErrorMessageDisplay(true);
  // (zohar) InstancePtr->m_ProtocolClient->UnInstallReceiveHandler(TransactionId);
  InstancePtr->m_ModelTank1Status   = LiquidTanksStatusMsg->ModelTank1Status;
  InstancePtr->m_ModelTank2Status   = LiquidTanksStatusMsg->ModelTank2Status;
  InstancePtr->m_ModelTank3Status   = LiquidTanksStatusMsg->ModelTank3Status;

  InstancePtr->m_ModelTank4Status   = LiquidTanksStatusMsg->ModelTank4Status;
  InstancePtr->m_ModelTank5Status   = LiquidTanksStatusMsg->ModelTank5Status;
  InstancePtr->m_ModelTank6Status   = LiquidTanksStatusMsg->ModelTank6Status;

  InstancePtr->m_SupportTank1Status = LiquidTanksStatusMsg->SupportTank1Status;
  InstancePtr->m_SupportTank2Status = LiquidTanksStatusMsg->SupportTank2Status;
...
void TOCBTesterForm::FillingReceiveHandler(int TransactionId, PVOID Data, unsigned DataLength, TGenericCockie Cockie)
{
  ...
  InstancePtr->m_SThermistorLevel = FillingMsg->ResinLevel[SUPPORT_M3_CHAMBER_THERMISTOR];
...
void TOCBTesterForm::DoorReceiveHandler(int TransactionId, PVOID Data, unsigned DataLength, TGenericCockie Cockie)
{
  ...
  InstancePtr->m_DisplayThread->EnableErrorMessageDisplay(true);
  // (zohar) InstancePtr->m_ProtocolClient->UnInstallReceiveHandler(TransactionId);
  InstancePtr->m_DoorStatus = DoorMsg->DoorStatus;
...
MESSAGE void TOCBTesterForm::DisplayHeadsFilling(TMessage& Message)
{
  ...
  SThermistorLevelPanel->Caption = IntToStr(m_ChamberLiquidLevel[SUPPORT_M3_CHAMBER_THERMISTOR]);
  ...
  SThermistorLevelPanel->Color   =      ((m_ChamberCurrentPumpState[SUPPORT_M3_CHAMBER_THERMISTOR]) ? clLime : clRed);
}
...
{
  Model1LeftPanel->Caption   = FloatToStrF (m_ModelTank1Weight, ffFixed, 4, 2);
  Model1RightPanel->Caption   = FloatToStrF (m_ModelTank2Weight, ffFixed, 4, 2);
  Model2LeftPanel->Caption   = FloatToStrF (m_ModelTank3Weight, ffFixed, 4, 2);
  Model2RightPanel->Caption = FloatToStrF (m_ModelTank4Weight, ffFixed, 4, 2);
  Model3LeftPanel->Caption = FloatToStrF (m_ModelTank5Weight, ffFixed, 4, 2);
  Model3RightPanel->Caption = FloatToStrF (m_ModelTank6Weight, ffFixed, 4, 2);
  Support1Panel->Caption = FloatToStrF (m_SupportTank1Weight, ffFixed, 4, 2);
  Support2Panel->Caption = FloatToStrF (m_SupportTank2Weight, ffFixed, 4, 2);

  WastePanel->Caption    = FloatToStrF (m_WasteTankWeight, ffFixed, 4, 2);
}
...
{
  if (LIQUID_TANK_IS_EMPTY != m_ModelTank1Status)
  {
	ModelTank1LeftExistEdit->Text = "In";
	ModelTank1LeftExistEdit->Color = clLime;
  } else
  {
	ModelTank1LeftExistEdit->Text = "Out";
	ModelTank1LeftExistEdit->Color = clRed;
  }

  if (LIQUID_TANK_IS_EMPTY != m_ModelTank2Status)
  {
	ModelTank1RightExistEdit->Text = "In";
	ModelTank1RightExistEdit->Color = clLime;
  } else
  {
	ModelTank1RightExistEdit->Text = "Out";
	ModelTank1RightExistEdit->Color = clRed;
  }

  if (LIQUID_TANK_IS_EMPTY != m_ModelTank3Status)
  {
	ModelTank2LeftExistEdit->Text = "In";
	ModelTank2LeftExistEdit->Color = clLime;
  } else
  {
	ModelTank2LeftExistEdit->Text = "Out";
	ModelTank2LeftExistEdit->Color = clRed;
  }
  if (LIQUID_TANK_IS_EMPTY != m_SupportTank1Status)
  {
	SupportTank1ExistEdit->Text = "In";
	SupportTank1ExistEdit->Color = clLime;
  } else
  {
    SupportTank1ExistEdit->Text = "Out";
    SupportTank1ExistEdit->Color = clRed;
  }
  if (LIQUID_TANK_IS_EMPTY != m_SupportTank2Status)
  {
    SupportTank2ExistEdit->Text = "In";
    SupportTank2ExistEdit->Color = clLime;
  } else
  {
    SupportTank2ExistEdit->Text = "Out";
    SupportTank2ExistEdit->Color = clRed;
  }
  if (LIQUID_TANK_IS_EMPTY != m_ModelTank4Status)
  {
	ModelTank2RightExistEdit->Text = "In";
	ModelTank2RightExistEdit->Color = clLime;
  } else
  {
	ModelTank2RightExistEdit->Text = "Out";
	ModelTank2RightExistEdit->Color = clRed;
  }
  if (LIQUID_TANK_IS_EMPTY != m_ModelTank5Status)
  {
	ModelTank3LeftExistEdit->Text = "In";
	ModelTank3LeftExistEdit->Color = clLime;
  } else
  {
	ModelTank3LeftExistEdit->Text = "Out";
	ModelTank3LeftExistEdit->Color = clRed;
  }
  if (LIQUID_TANK_IS_EMPTY != m_ModelTank6Status)
  {
	ModelTank3RightExistEdit->Text = "In";
	ModelTank3RightExistEdit->Color = clLime;
  } else
  {
	ModelTank3RightExistEdit->Text = "Out";
	ModelTank3RightExistEdit->Color = clRed;
  }
}
...
MESSAGE void TOCBTesterForm::DisplayDoorStatus(TMessage& Message)
{
  switch (m_DoorStatus)
  {
    case DOOR_OUTSIDE_UNLOCKED:
      DoorStatusEdit->Text = "Door Outside and Unlocked";
      break;

    case DOOR_INSIDE_UNLOCKED:
      DoorStatusEdit->Text = "Door Inside and Unlocked";
      break;

    case DOOR_OUTSIDE_LOCKED:
      DoorStatusEdit->Text = "Door Outside and Locked";
      break;

    case DOOR_INSIDE_LOCKED:
      DoorStatusEdit->Text = "Door Inside and Locked";
      break;
  }
}
...
{
  InternalVersionEdit->Text = StrToInt(m_InternalVersion);
  ExternalVersionEdit->Text = StrToInt(m_ExternalVersion);
}
...
/*iMPORTANT: The Order is Support, Model, [Model2]*/
  ...
  if (m_IsHeadParamsInUse == false)
  {
    ...
    try
	{
	  ...
	  Message.ResinLowThereshold[SUPPORT_M3_CHAMBER_THERMISTOR]  = StrToInt(SLevelThermistorLowEdit->Text);
	  ...
	  Message.ResinHighThershold[SUPPORT_M3_CHAMBER_THERMISTOR]  = StrToInt(SLevelThermistorHighEdit->Text);
	  ...
	  Message.ActiveThermistors[SUPPORT_M3_CHAMBER_THERMISTOR] = SHighThCheckBox->Checked;
	  ...
	  ParamsMgr->ActiveThermistors[SUPPORT_M3_CHAMBER_THERMISTOR] = SHighThCheckBox->Checked;
