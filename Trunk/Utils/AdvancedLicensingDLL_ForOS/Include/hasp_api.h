/**
 * \mainpage HASP SRM Run-time API
 * \file hasp_api.h HASP SRM Run-time API declarations
 *
 * Aladdin Knowledge Systems Ltd. (C) 1985-2008. All rights reserved.
 *
 * $Id: hasp_api.h,v 1.57 2008/04/15 07:45:23 christoph Exp $
 */
...
/**
 * \brief HASP SRM secure writable memory file
 *
 * File ID for HASP SRM secure writable memory.
 */
#define HASP_FILEID_RW 0xfff4

/**
 * \brief HASP SRM secure read only memory file
 *
 * File ID for HASP SRM secure read only memory.
 */
...
enum hasp_error_codes
{
    ...
    /** Update request not supported by HASP SRM protection key */
    ...
    /** HASP SRM protection key does not support encryption type */
    ...
    /** Communication error between API and local HASP License Manager */
...
/**
 * @defgroup hasp_basic The Basic HASP SRM Run-time API
 *
 * @{
 */
 ...
 *
 * <b>HASP SRM Run-time API Usage Notes</b><br>
 * This function establishes a context to a HASP SRM protection key
 ...
 *                        completed
 *                      <li>HASP_HASP_NOT_FOUND</li> Required HASP SRM
 ...
 *
 * <b>HASP SRM Run-time API Usage Notes</b><br>
 * Encrypts data using the encryption engine in the HASP SRM protection key.
 * The specific session handle determines which HASP SRM protection key and
 * which Feature ID encrypts the data buffer. The encryption key remains
 * in the HASP SRM protection key. If the encryption fails, the buffer
 ...
 *                        in secure channel
 *                      <li>HASP_ENC_NOT_SUPP</li> HASP SRM protection key
 ...
 *
 * <b>HASP SRM Run-time API Usage Notes</b><br>
 * Decrypts data using the decryption engine in the HASP SRM protection key.
 * The specific session handle determines which HASP SRM protection key and
 * which Feature ID decrypts the data buffer. The decryption key remains
 * in the HASP SRM protection key. If the decryption fails, the buffer
 ...
 *                        too short
 *                      <li>HASP_SCHAN_ERR</li> Communication error
 *                        in secure channel
 *                      <li>HASP_ENC_NOT_SUPP</li> HASP SRM protection key
 *                        does not support encryption type
 *                      <li>HASP_BROKEN_SESSION</li> Session has been
...
/**
 * \brief Reads the memory of a HASP SRM protection key.
 *
 * <b>HASP SRM Run-time API Usage Notes</b><br>
...
/**
 * \brief Writes to the memory of a HASP SRM protection key.
 *
 * <b>HASP SRM Run-time API Usage Notes</b><br>
 ...
 *
 * <b>HASP SRM Run-time API Usage Notes</b><br>
 * This function does not work with a HASP SRM login handle.
 *
 * <b>Legacy HASP Remarks</b><br>
...
/**
 * @defgroup hasp_extended Extended HASP SRM functionality
 *
 ...
 *
 * <b>HASP SRM Run-time API Usage Notes</b><br>
 * You do not need to be logged in to a HASP SRM Feature in order to use
 * this function.
 *
...
/**
 * \brief Updates a HASP SRM protection key.
 *
 * This function writes update information. Note that the HASP SRM protection
 * key must be locally connected.
 *
 * The update code contains all necessary data to perform the update on
 * a deployed HASP SRM protection key including:
 * <ul>
 *   <li>The HASP SRM protection key on which the updated information is 
 *      to be written</li>
 *   <li>The necessary Vendor Code that is required to access the HASP 
 *      SRM protection key</li>
 ...
 *                        not supported by HASP SRM protection key
 ...
 *                      <li>HASP_TOO_MANY_KEYS</li> Too many HASP SRM
...
/**
 * \brief Detaches a Product, according to customizable parameters
 *
 * <b>HASP SRM Run-time API Usage Notes</b><br>
 * You do not need to be logged in to a HASP SRM Feature in order to use
 * this function.
 *
 * This function is used to detach a product (i.e. all HASP SRM Features
 * and Memory files which belong to this product) from a HASP SL Protection
 * key.
 *
 * The requisite Vendor Codes are stored in a VendorCodes folder in
 * your system. Without the correct Vendor Code, the function call 
 ...
 *                        does not select a unique Parameter
 *                      <li>HASP_TOO_MANY_USERS</li> Too many users currently
 *                        connected
 *                      <li>HASP_ACCESS_DENIED</li> Request not possible 
 *                        because of ACC restrictions
 ...
 *                        or too short (if an existing attached container is
