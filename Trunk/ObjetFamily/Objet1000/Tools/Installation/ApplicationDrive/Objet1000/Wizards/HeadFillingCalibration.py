...
#
#   Version:  $Rev: 22416 $
#   Date:   $Date: 2015-07-23 08:27:12 +0300 (Thu, 23 Jul 2015) $
#   Path:   $HeadURL: http://srv-ire-svn/svn/ControlSW/Trunk/ObjetFamily/Base/PythonScripts/Wizards/HeadFillingCalibration.py $ 
#
################################################################################################
...
HeadsHeatingPage = StatusWizardPage("Current state",-1,wpNextDisabled | wpPreviousDisabled)
...
PurgePage = StatusWizardPage("Current state",-1,wpNextDisabled | wpPreviousDisabled)
...
if int(Application.MachineType) == OBJET_1000:
	HEAD_FILLING_CALIBRATION_LOW_FACTOR  = 80
	HEAD_FILLING_CALIBRATION_HIGH_FACTOR = 100
else:
...
def OnStart():
  ...
  global PrevPumpsTimeDutyOff
  global PrevPumpsTimeDutyOn
  ...
  PrevPumpsTimeDutyOff = AppParams.PumpsTimeDutyOff
  PrevPumpsTimeDutyOn = AppParams.PumpsTimeDutyOn
  BackEndInterface.EnableConfirmationBypass(PER_MACHINE_MODE_HIGH_SPEED, HIGH_SPEED_MODES_DIR)
  BackEndInterface.EnableConfirmationBypass(PER_MACHINE_MODE_HIGH_QUALITY, HIGH_QUALITY_MODES_DIR)
  BackEndInterface.EnableConfirmationBypass(PER_MACHINE_MODE_HIGH_MIX, HIGH_MIX_MODES_DIR)
  AppParams.PumpsTimeDutyOff = AppParams.HF_PumpsTimeDutyOff
  AppParams.PumpsTimeDutyOn = AppParams.HF_PumpsTimeDutyOn
  ...
  #if int(Application.MachineType) == OBJET_???:
...
def OnEnd():
  ...
  AppParams.PumpsTimeDutyOff = PrevPumpsTimeDutyOff
  AppParams.PumpsTimeDutyOn = PrevPumpsTimeDutyOn
  BackEndInterface.DisableConfirmationBypass(PER_MACHINE_MODE_HIGH_SPEED, HIGH_SPEED_MODES_DIR)
  BackEndInterface.DisableConfirmationBypass(PER_MACHINE_MODE_HIGH_QUALITY, HIGH_QUALITY_MODES_DIR)
  BackEndInterface.DisableConfirmationBypass(PER_MACHINE_MODE_HIGH_MIX, HIGH_MIX_MODES_DIR)
  TurnHeadFilling(OFF)
  TurnHeadHeaters(OFF)
...
def OnPageLeave(Page,LeaveReason):
  ...
  if Page == WelcomePage:
    ...
    if(TrayPlacer.IsTrayInserted() == False): #Tray is out
  ...
  if Page == CheckTrayPage:
    ...
    if(TrayPlacer.IsTrayInserted() == False):
      SetNextPage( CheckTrayPage )
    else:
	  SetNextPage( CloseDoorPage )    	  
