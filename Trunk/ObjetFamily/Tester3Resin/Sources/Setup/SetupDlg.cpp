...
{
	for(int i = 0; i < JS_LOG_TAGS_NUM; i++)
	{
		TCheckBox *chk      = new TCheckBox(LogGroupbox);
		chk->Parent         = LogGroupbox;
		chk->Left           = 20;
		chk->Top            = 30 + 20 * i;
		chk->Checked        = true;
		chk->Caption        = LogTagDescription[i].c_str();
		m_LogTagCheckbox[i] = chk;
	}
}
...
void __fastcall TSetupForm::ComComboBoxDropDown(TObject *Sender)
{
	...
	{
		// Allow any com port, even the same port for different cards. Used for Simulator
...
void __fastcall TSetupForm::EmbeddedRadioGroupClick(TObject *Sender)
{
	...
	{
		case NONE:
			m_SystemParams->EmbeddedConnectionKind = NONE;
		...
		case DIRECT:
			m_SystemParams->EmbeddedConnectionKind = DIRECT;
...
void __fastcall TSetupForm::OCBSimRadioGroupClick(TObject *Sender)
{
	...
	{
		case NONE:
			m_SystemParams->OCBSimConnectionKind = NONE;
			...
			break;
		case DIRECT:
			m_SystemParams->OCBSimConnectionKind = DIRECT;
...
void __fastcall TSetupForm::MCBRadioGroupClick(TObject *Sender)
{
	...
	{
		case NONE:
			m_SystemParams->MCBConnectionKind = NONE;
			...
			break;
		case DIRECT:
			m_SystemParams->MCBConnectionKind = DIRECT;
			if(MCBComboBox->Text != "")
			{
			...
			break;
		case THROUGH_EMBEDDED:
			m_SystemParams->MCBConnectionKind = THROUGH_EMBEDDED;
			MCBComboBox->ItemIndex = -1;  // clear the combo box data
			if(m_SystemParams->MCBComNum != 0)
				UpdateCommList(m_SystemParams->MCBComNum,false);
			break;
...
void __fastcall TSetupForm::LCDRadioGroupClick(TObject *Sender)
{
	...
	{
		case NONE:
			m_SystemParams->LCDConnectionKind = NONE;
		...
		case DIRECT:
			m_SystemParams->LCDConnectionKind = DIRECT;
			if(LCDComboBox->Text != "")
			{
		...
		case THROUGH_EMBEDDED:
			m_SystemParams->LCDConnectionKind = THROUGH_EMBEDDED;
			LCDComboBox->ItemIndex = -1;  // clear the combo box data
			if(m_SystemParams->LCDComNum != 0)
				UpdateCommList(m_SystemParams->LCDComNum,false);
			break;
...
void __fastcall TSetupForm::ContainersRadioGroupClick(TObject *Sender)
{
	...
	{
		case NONE:
			m_SystemParams->ContainersConnectionKind = NONE;
		...
		case DIRECT:
			m_SystemParams->ContainersConnectionKind = DIRECT;
		...
		case THROUGH_EMBEDDED:
			m_SystemParams->ContainersConnectionKind = THROUGH_EMBEDDED;
			ContainersComboBox->ItemIndex = -1;  // clear the combo box data
			if(m_SystemParams->MCBComNum != 0)
				UpdateCommList(m_SystemParams->ContainersComNum,false);
			break;
...
void __fastcall TSetupForm::OCBRadioGroupClick(TObject *Sender)
{
	...
	{
		case NONE:
			m_SystemParams->OCBConnectionKind = NONE;
		...
		case DIRECT:
			m_SystemParams->OCBConnectionKind = DIRECT;
			if(OCBComboBox->Text != "")
			{
		...
		case THROUGH_EMBEDDED:
			m_SystemParams->OCBConnectionKind = THROUGH_EMBEDDED;
			OCBComboBox->ItemIndex = -1;  // clear the combo box data
			if(m_SystemParams->OCBComNum != 0)
				UpdateCommList(m_SystemParams->OCBComNum,false);
			break; 
...
void __fastcall TSetupForm::OHDBRadioGroupClick(TObject *Sender)
{
	...
	{
		case NONE:
			m_SystemParams->OHDBConnectionKind = NONE;
			...
			break;
		case DIRECT:
			m_SystemParams->OHDBConnectionKind = DIRECT;
			...
			break;
		case THROUGH_EMBEDDED:
			m_SystemParams->OHDBConnectionKind = THROUGH_EMBEDDED;
			OHDBComboBox->ItemIndex = -1;  // clear the combo box data
			if(m_SystemParams->OHDBComNum != 0)
				UpdateCommList(m_SystemParams->OHDBComNum,false);
			break;
		case THROUGH_OCB:
			m_SystemParams->OHDBConnectionKind = THROUGH_OCB;
			OHDBComboBox->ItemIndex = -1;  // clear the combo box data
			if(m_SystemParams->OHDBComNum != 0)
				UpdateCommList(m_SystemParams->OHDBComNum,false);
			break;
...
void __fastcall TSetupForm::FormShow(TObject *Sender)
{
	...
	for(int i = 0; i < JS_LOG_TAGS_NUM; i++)
		m_LogTagCheckbox[i]->Checked = m_SystemParams->LogTagsEnable[i];
}
//---------------------------------------------------------------------------
...
void __fastcall TSetupForm::OKBitBtnClick(TObject *Sender)
{
	...
	for(int i = 0; i < JS_LOG_TAGS_NUM; i++)
	{
		CQLog::EnableDisableTag(i, m_LogTagCheckbox[i]->Checked);
		m_SystemParams->LogTagsEnable[i] = m_LogTagCheckbox[i]->Checked;
	}

	m_SystemParams->SaveAll();
}
//---------------------------------------------------------------------------
...
void __fastcall TSetupForm::RFIDRadioGroupClick(TObject *Sender)
{
	switch(RFIDRadioGroup->ItemIndex)
	{
		case NONE:
		{
			m_SystemParams->RFIDConnectionKind = NONE;
		...
		case DIRECT:
		{
			m_SystemParams->RFIDConnectionKind = DIRECT;
