...
#ifdef OCB2
  // For: 	For: SMB0CN, sbit STA  -  SMBUS 0 START FLAG.
  SFRPAGE = SMB0_PAGE;
#endif
	...
	{		
		...
		case READ_MSC:		
			if (FirstSecondMSC == FIRSTMSCCARD) 
			{
				...
				/*if(MSC2_Com_Error == MSC2_NO_SLAVE)  
				SchedulerLeaveTask(WAIT_FOR_BUS_NOT_BUSY);*/					
				TARGET = SECOND_CHIP_SELECT;             // Target the Slave for next SMBus transfer  					              
			}
			else  
			{
				...
				/*if(MSC1_Com_Error == MSC1_NO_SLAVE)  
				SchedulerLeaveTask(WAIT_FOR_BUS_NOT_BUSY);*/				
				TARGET = FIRST_CHIP_SELECT;             // Target the Slave for next SMBus transfer  					      
			}
	...
	//if ISR doesn't reset ISRKeepAliveCheck for 40 cycles, send an error to EM 
...
#else
		SMB0CR = 0xCC;			// Configure SMBug bitrate to 100 kHz
								// 		T_LOW  = (256 - SMB0CR) / SYSCLK
								// 		T_HIGH = (258 - SMB0CR) / SYSCLK + 625ns
#endif

		_SMB0CN = SMB0CN; // for debugging.
		_SMB0CR = SMB0CR; // for debugging.
      ...
      case SMB_MRADDNACK:   
		 ...
		 { 		   
		   ...
		   { 					              // While trying to reach un-existing  or Disconnected	Address.		  
		    ...
		    { 
		     //MSC2_Com_Error = MSC2_NO_SLAVE;
			 I2C_Error_Notifier = MSC2_NO_SLAVE;
			 
			 //Zero the voltages Buffers 
