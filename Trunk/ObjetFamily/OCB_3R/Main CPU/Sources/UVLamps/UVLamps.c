...
#define UV_SAMPLES_THRESHOLD              10 // %
...
/*
  ...
  if ( input > Avg ) {
    if( (input - Avg) <= ((Avg * UVSamplesThreshold) / 100) )
	  return TRUE;
	else
	  return FALSE;
  }
  else if ( Avg > input ) {
    if( (Avg - input) <= ((Avg * UVSamplesThreshold) / 100) )
	  return TRUE;
	else
	  return FALSE;
  }
...
void SampleUVSensorTask(BYTE Arg)
{
  WORD ReadingINT = SpiA2D_GetReading(ANALOG_IN_19);
  WORD ReadingEXT = SpiA2D_GetReading(ANALOG_IN_16);
  
  AverageAddReading(&UVSensorAverageINT, (long)ReadingINT );
  ...
  if( UVSensorAverageINT_Max.NumOfElements == 0 ) {
  //if( UVSensorAverageINT_Max.NumOfElements < UVSensorAverageINT_Max.WindowSize ) {

	// add the reading to the gliding average
	GlidingAverageAddReading(&UVSensorAverageINT_Max, (long)ReadingINT );
	GlidingAverageAddReading(&UVSensorAverageEXT_Max, (long)ReadingEXT );
	  ...
	  // handle finding the Max value of INT sensor

	  if( CheckUVThreshold( ReadingINT /*, UVSensorAverageINT_Max.Average */ ) ) // check if we're within the threshold bounds
	  {
	    // update the Max Delta if needed
	    UpdateUVReadingMaxDeltaIfNeeded( ReadingINT, UVSensorAverageINT_Max.Average, &UVReadingMaxDeltaINT );

		// add this reading to the sum
		UVReadingSumINT += ReadingINT;
	
		// add the reading to the gliding average
		GlidingAverageAddReading(&UVSensorAverageINT_Max, (long)ReadingINT );
		
		// update Max reading if needed
		if( UVSensorAverageINT_Max.Average > UVMaxReadingINT )
		  UVMaxReadingINT = UVSensorAverageINT_Max.Average;
	  }
	
	
	  // handle finding the Max value of EXT sensor
	
	  if( CheckUVThreshold( ReadingEXT /*, UVSensorAverageEXT_Max.Average */ ) ) // check if we're within the threshold bounds
	  {
	    // update the Max Delta if needed
...
BOOL UVLampsSetSamplingParamsEx(BOOL a_RestartSampling, WORD a_WindowSizeForAvg, WORD a_WindowSizeForMax, WORD a_UVSamplesThreshold)
{
  ...
  {
    AverageInitialize(&UVSensorAverageINT, a_WindowSizeForAvg);
	AverageInitialize(&UVSensorAverageEXT, a_WindowSizeForAvg);
    GlidingAverageInitialize(&UVSensorAverageINT_Max, a_WindowSizeForMax);
