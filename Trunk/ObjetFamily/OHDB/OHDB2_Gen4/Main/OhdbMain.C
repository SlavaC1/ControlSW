...
void main (void)
{
	TTaskHandle xdata A2DTaskHandle, HeaterStatusTaskHandle, HeaterControlTaskHandle, ExtA2DTaskHandle, FansReadDataTaskHandle;
	...
	ExtA2DTaskHandle            = SchedulerInstallTask(SpiExtA2D_Task); 
	...
	FansReadDataTaskHandle			= SchedulerInstallTask(FansReadDataTask);
	...
	SchedulerResumeTask(ExtA2DTaskHandle,0);
	...
	SchedulerResumeTask(FansReadDataTaskHandle,0);

    /* configure the watchdog - remark the line below if you are debugging the OHDB */
	WDTCN = 0x07;	
...
void XBR_Init()
{
	...
	// Bit7 of XBR0 enabled. Enable CP0 on P0.6
  	XBR0 |= 0x80;
	
	// Bit0 of XBR1 enabled. Enable CP1 on P0.7
	XBR1 |= 0x01;	
	
	// Bit2 of XBR1 enabled. Enable INT0 on P2.0
	XBR1 |= 0x04;

	// Bit4 of XBR1 enabled. Enable INT1 on P2.1
	XBR1 |= 0x10;	
	
	// Bit7 of XBR1 enable. Enable SYSCK on P2.2
	XBR1 |= 0x80;  
