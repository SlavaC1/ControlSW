...
//DUMMY
	...
	case XILINX_SLAVE_ID:
		...
		LSBIndexData  = SpiTxBuffer[DATA_LSB_INDEX];
		...
		if (Address == GO_REG_ADDRESS && LSBIndexData == ENABLE_PRINTING_COMMAND )  //during printing
			EXTMem_WriteBitsNoLock(XILINX_PRINTING_ADD_OHDB, 1, 0x1);
		else  if (Address == GO_REG_ADDRESS && LSBIndexData != ENABLE_PRINTING_COMMAND )
 ...
 *
 ****************************************************************************/
void SpiSelectSlave(BYTE NewSlaveID, BYTE Select)
{
	SlaveId = 0 ;
	if (CHIP_SELECT == Select && XILINX_SLAVE_ID == NewSlaveID )
	{
