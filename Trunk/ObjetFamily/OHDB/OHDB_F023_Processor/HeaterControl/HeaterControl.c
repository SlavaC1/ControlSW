...
void TurnHeatersOnOff()
{
	...
	EXTMem_Read(P2_ADD_OHDB,&Item );
 	...
 	EXTMem_Read(P1_ADD_OHDB,&Item );
   	EXTMem_Write(P1_ADD_OHDB, (Item & 0xDF) | ((HeatersOnOff & EXTERNAL_HEATER_MASK) >> 7) );
#endif	
#else
	P3 = (HeatersOnOff & HEAD_HEATERS_MASK);
	
// Modify only the 4 upper bits of port 2
// --------------------------------------
	P2 = (P2 & 0x0F) | ((HeatersOnOff & BLOCKS_AND_PREHEATER_MASK) >> 4);

#ifndef OBJET_MACHINE
// Modify only bit 5 of port 1
// ---------------------------
  P1 = (P1 & 0xDF) | ((HeatersOnOff & EXTERNAL_HEATER_MASK) >> 7); 
#endif  
#endif	
...
#ifndef OBJET_MACHINE
   	EXTMem_Read(P1_ADD_OHDB,&Item );
	EXTMem_Write(P1_ADD_OHDB, Item & 0xDF);
#endif
