...
const BYTE SelectSlaveArrayP3[NUM_OF_SPI_SLAVES] = 
{
	SPI_SELECT_E2PROM_1,
	SPI_SELECT_E2PROM_2,
	SPI_SELECT_E2PROM_3,
...
void SpiInit(void)
{
	...
	// Configure P2.5 (address lines) as push pull. For SPI_WR
	P2MDOUT |= 0x40;
	
	// Configure P3.0 P3.1 P3.5 P3.6 P3.7 (address lines) as push pull. For SPI chip selects. 
	P3MDOUT |= 0xE3;
...
void SpiIsr()interrupt 6 using 3
{
	if (SPIF)
	{
		...
		if(CommWithA2D)
		{
			...
			P3 = (P3 & SPI_SELECT_NONE) | SPI_SELECT_NONE;			
		}	
		...
		if (SpiIndex < SpiDataLen)
		{
			if(CommWithA2D)
			{
				...
				// set the NSS to low					    	
			    P3 = (P3 & SPI_SELECT_NONE) | SPI_SELECT_A2D_PROCESSOR;				
			}
...
{
	// Modify only the bits 0, 1, 5, 6, 7 of P3
	if (Select == CHIP_SELECT)
	{	  
		P3 = (P3 & SPI_SELECT_NONE) | SelectSlaveArrayP3[SlaveID];		
	}
	else
	{	
		P3 = (P3 & SPI_SELECT_NONE) | SPI_SELECT_NONE;
