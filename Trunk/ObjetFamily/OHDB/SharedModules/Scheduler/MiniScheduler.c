...
// Constants
// =========
#if (defined C8051F023_CPU || defined C8051F123_CPU)
	#define MAX_NUM_OF_TASKS 72
#else 
	#define MAX_NUM_OF_TASKS 56
#endif
...
// Module variables
// ================
TTaskConrolBlock xdata Tasks[MAX_NUM_OF_TASKS];
...
TTaskHandle SchedulerInstallTask(TTaskPtr TaskFunctionPtr)
{
	...
	if (NumOfTasks >= MAX_NUM_OF_TASKS)
  		return -1;
...
void SchedulerRun()
{
	...
	{
	    /* refresh the watchdog - remark the line below if you are debugging the OHDB */
	    WDTCN = 0xa5;
		...
		{
			if (TimerHasTimeoutExpired(&Tasks[RunningTask].Timer) == TRUE)
			{
				Tasks[RunningTask].State = TASK_RUNNING;
				Tasks[RunningTask].TaskPtr(Tasks[RunningTask].Argument);
				break;
