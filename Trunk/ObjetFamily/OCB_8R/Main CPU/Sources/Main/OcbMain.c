...
void main (void)
{
	...
	TTaskHandle xdata HeaterStatusTaskHandle, HeaterControlTaskHandle;
 	TTaskHandle xdata OHDBMessageDecodeTaskHandle,FansReadDataTaskHandle;
#endif
	...
	//The Ping task is the first to be created in the system, do not create tasks before that
	EdenProtocolInit();
	MessageDecodeInit();
	MSC_Init();
#ifdef OCB_SIMULATOR
