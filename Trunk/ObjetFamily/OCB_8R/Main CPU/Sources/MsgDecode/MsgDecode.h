...
#define VS_SENSOR				SELECT_CPU_ANALOG_IN1 //EladH
#define VCC_SENSOR			    SELECT_CPU_ANALOG_IN2 //EladH
#define V_12_SENSOR				SELECT_CPU_ANALOG_IN3 //EladH
#define V_24_SENSOR				SELECT_CPU_ANALOG_IN4 //EladH
...
#define GET_POWER_SUPPLIES_VALUE_MSG 		    0xA1 //EladH
...
#define ACTIVATE_SIGNAL_TOWER_MSG               0xA3
#define SET_AMBIENT_HEATER_ON_OFF_MSG           0xA4
#define GET_EVACUATION_AIRFLOW_STATUS_MSG       0xA5
#define RESET_NORMALLY_OPEN                     0xA6 // reset the normally open interlock feedback status, using to inverse the HW ( doors & emergency stop butoon
...
#define POWER_SUPPLIES_VALUE_MSG                0xF7 //EladH
#define MSC_STATUS_MSG                          0xF8 //EladH
#define I2C_DEBUG_MSG                           0xF9 //EladH
#define GET_SAFETY_SYSTEM_STATUS_MSG            0xFA // Guy
#define SEND_SAFETY_SYSTEM_STATUS_MSG           0xFB // Guy
#define EVAC_AIRFLOW_STATUS_MSG                 0xFC
...
typedef enum
{
	...
	M5_M6_CHAMBER,
	NUM_OF_CHAMBERS,
	NUM_OF_DM_CHAMBERS = SUPPORT_M7_CHAMBER,
...
#define NUM_OF_EMERGENCY_BUTTONS  2
...
// A TEvacAirFlowStatusMsg message is sent in response to GetEvacuationAirFlowStatus message.
typedef struct {		
	BYTE MsgId;
	WORD AirFlowValue;
	}TEvacAirFlowStatusMsg;	//Elad A
...
typedef struct {
    ...
    WORD V_12Volt;
	WORD V_24Volt;	
    WORD I2CA2DInput[NUM_OF_MSC_CARDS][4];	
	}TPowerSuppliesVoltageMsg; //EladH
...
typedef struct {		
	BYTE MsgId;
	BYTE ActautorsStatus[NUM_OF_ACTUATOR_PORTS];
	}TActuatorsStatusMsg;

typedef struct {		
	BYTE MsgId;
    BYTE InputsStatus[NUM_OF_SENSORS/8]; // Number of sensors is 32 - 8 * 4
	}TInputsStatusMsg;
...
typedef struct {		
	BYTE MsgId;
	BYTE DoorRequest[NUM_OF_DOORS];
    BYTE DoorStatus[NUM_OF_DOORS];
}TDoorStatusMsg;
...
typedef struct {		
	BYTE MsgId;
    BYTE EmergencyButtonsStatus[NUM_OF_EMERGENCY_BUTTONS];
	BYTE ServiceKeyStatus;
}TSafetySystemStatusMsg;
...
typedef struct {		
	...
	{		
		BYTE MsgId;		
		BYTE TankStatus[NUM_OF_LIQUID_TANKS];
	...
	{		
		BYTE MsgId;
		float TankWeight[NUM_OF_LIQUID_TANKS];
