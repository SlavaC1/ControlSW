...
#define MAX_MUN_OF_TASKS		143 //85(OCB)+55(OHDB)

#else

#define MAX_MUN_OF_TASKS		86
...
void SchedulerRun()
{
	...
	for (i = 0; i < NumOfTasks; i++)
	{
		...
		{
			if (TimerHasTimeoutExpired(&Tasks[RunningTask].Timer))
			{
				Tasks[RunningTask].State = TASK_RUNNING;
				Tasks[RunningTask].TaskPtr(Tasks[RunningTask].Argument);
				break;
 ...
 *
 *
 *  DESCRIPTION : Get the argument of a task                                     
 *
 ****************************************************************************/
#ifdef OCB_SIMULATOR
