...
//
  ...
  m_sMachineType = Q2RTApplication->GetMachineTypeAsString().c_str();

  Reg->OpenKey( "SOFTWARE\\Stratasys Geometries Ltd.\\" + m_sMachineType + "\\" + GetVersionStringFormatted(VersionNum->Caption), false);
  ...
  if(PatchStrings->Count == 0)
	PatchesList->Items->Add("None");
  else
  	for(int i = 0; i < PatchStrings->Count; i++)
		PatchesList->Items->Add(PatchStrings->Strings[i]);
...
void TAboutBox::UpdateVersions(void)
{
  ...
  //RFIDDLLLable->Caption = "0.1";//m_BackEnd->GetVersion(VERSION_RFID_DLL_HW).c_str();
  ...
  MSC3HWLabel->Visible = true;
  MSC4HWLabel->Visible = true;
  MSC3Label->Visible = true;
  MSC4Label->Visible = true;
  MSC3HWLabel->Caption   = m_BackEnd->GetVersion(HW_FW_VERSION_MSC3).c_str();
  MSC4HWLabel->Caption   = m_BackEnd->GetVersion(HW_FW_VERSION_MSC4).c_str();
#endif
  }
