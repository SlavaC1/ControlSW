...
#include "WizardImages.h" //GetPageImage
	...
	: TFrame(Owner)
{
	...
	lblEtaProgressBar->Visible = false;
     	TanksSummaryFrame->OpModePanel->Brush->Style = Graphics::bsClear;
}
//---------------------------------------------------------------------------
void TCustomResinSelectSummaryFrame::PopulateMaterialsGrid(CWizardPage *WizardPage)
{
	...
	if(! Tanks.empty())
	{
		for(TTanksVector::iterator i = Tanks.begin(); i != Tanks.end(); ++i)
		{
			...
			int TableIdx = TanksSummaryFrame->TankIndexToTable(i->TankIndex);

			if(i->MarkedForReplacement)
			{
				dynamic_cast<TLabel *>(TanksSummaryFrame->grdTable[TableIdx]->ControlCollection->ControlItems[Col][Row]->Control)->Caption = i->MaterialName.c_str();
			...
			{
				dynamic_cast<TLabel *>(TanksSummaryFrame->grdTable[TableIdx]->ControlCollection->ControlItems[Col][Row]->Control)->Caption = "--------";
	...
	if (m_Page->AreWarningsSuppressed())
	{
		CartridgesRemark->Caption = "";
	} else
	{
		//display a warning if the waste cartridge(s) should be replaced/doesn't have much space left
		int WasteFreeSpace      = m_Page->GetWasteTanksFreeSpace();
		int PrevWasteFreeSpace  = m_Page->GetPrevWasteTanksFreeSpace();
		if (WasteFreeSpace != PrevWasteFreeSpace) {
			QString remark;
			switch (WasteFreeSpace) {
				case SUMMARY_WASTE_WEIGHT_OK:
					remark = QString("");
					break;
				/*case SUMMARY_WASTE_WEIGHT_FULL:
					remark = ERR_WASTE_CARTRIDGE_FULL;
					break;*/
				case SUMMARY_WASTE_WEIGHT_NA:
					remark = ERR_WASTE_CARTRIDGE_NA;
					break;
				case SUMMARY_WASTE_WEIGHT_UNSTABLE:
					remark = ERR_WASTE_CARTRIDGE_WAIT;
					break;
				default: remark = QFormatStr(ERR_WASTE_CARTRIDGE_LOW_LIMIT.c_str(),WasteFreeSpace).c_str();
			}
			CartridgesRemark->Caption = remark.c_str();
		}
	}
}
//---------------------------------------------------------------------------
...
void TCustomResinSelectSummaryFrame::SetTankStatus(int TankIndex, TTankStatus Status, int NeededWeight)
{
	...
	int TableIdx = TanksSummaryFrame->TankIndexToTable(TankIndex);
	QString ToolTip;

	TImage *Image = dynamic_cast<TImage *>(TanksSummaryFrame->grdTable[TableIdx]->ControlCollection->ControlItems[Col][Row]->Control);
	...
	switch(Status)
	{
		...
		case STATUS_EXPIRED:
			...
			break;
		case STATUS_STABILIZATION_TIME:
			ToolTip = HINT_WAITING;
			break;
		case STATUS_EMPTY:
		default:
			return;
...
//---------------------------------------------------------------------------
void TCustomResinSelectSummaryFrame::WizardFormSizeExpand(const QString pageGridImage)
{
	 const bool showPageSideImg = false;

	 WizardForm->GetUIFrame()->ReplaceViewerFrame(this, showPageSideImg /* ShowImage */);
	 TanksSummaryFrame->ResizeCabinetsInfo(
		TanksSummaryFrame->TanksGroupBox->Height, TanksSummaryFrame->TanksGroupBox->Width,
		pageGridImage /*If we have a side image (showPageSideImg), we don't need the table's in-between image(s), and vice versa */);
}
//---------------------------------------------------------------------------
...
__fastcall CCustomResinSelectSummaryPageViewer::CCustomResinSelectSummaryPageViewer()
{
  ...
  m_CustomResinSelectSummaryWizFrame = NULL ;
}
...
void CCustomResinSelectSummaryPageViewer::Prepare(TWinControl *PageWindow, CWizardPage *WizardPage)
{
   ...
   const QString innerImage = GetPageImage(MRW_GRID_CABINET_IDENTIFY);
   m_CustomResinSelectSummaryWizFrame->WizardFormSizeExpand(innerImage);
}
 //---------------------------------------------------------------------------
