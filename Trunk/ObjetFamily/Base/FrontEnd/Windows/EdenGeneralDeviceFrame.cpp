  ...
  : TFrame(Owner),m_UvLampsMask(0)
{
  ...
  {
	 TrayHeaterGroupBox->Visible = true;
  }

  if(ParamsMgr->EvacuationIndicatorEnabled)
  {
	 EvacuationIndicatorGroupBox->Visible = true;
  }
}
//---------------------------------------------------------------------------
...
void __fastcall TGeneralDeviceFrame::PowerOnActionExecute(TObject *Sender)
{
  ...
  try
  {
	CBackEndInterface::Instance()->TurnPower(true);

	// Get the RFID Instance:
    CRFIDBase* RFIDInstance = CRFID::Instance();

	if(RFIDInstance->WaitForReaderPowerUp(0) != true)
	  CQLog::Write(LOG_TAG_RFID,"Error waiting to RF Reader PowerUp.");
	if(RFIDInstance->WaitForReaderPowerUp(1) != true)
	  CQLog::Write(LOG_TAG_RFID,"Error waiting to RF Reader PowerUp.");
    ...
    BackEnd->AllowTankInsertionAndRemovalSignals();
    BackEnd->GetLiquidTankInsertedStatus();

    BackEnd->ResumeOCBStatusThread();
    BackEnd->ResumeHeadStatusThread();

    // retransmit OCB configuration  TODO: if OHDB SW reset button is implemented it should do the same code segment
    if (Q_NO_ERROR != BackEnd->WaitForOhdbHwDrvToLoad())
    {
        CQLog::Write(LOG_TAG_GENERAL, "Error waiting for OHDB HW Driver to load");
    }
    if (Q_NO_ERROR != BackEnd->ResetOhdbHwDrv())
    {
        CQLog::Write(LOG_TAG_GENERAL, "Error resetting OHDB HW Driver");
    }
    BackEnd->SetDefaultParamsToOCB();
...
void __fastcall TGeneralDeviceFrame::PowerOffActionExecute(TObject *Sender)
{
  ...
  try
  {
    // Since OHDB power supply is used commonly with the RFID Reader Power Supply,
    // simulate *tanks removal* to avoid RFID read exceptions when turning power Off:
	...
	CQLog::Write(LOG_TAG_RFID,"SendTanksRemovalSignals.");
	BackEnd->SendTanksRemovalSignals();
	CQLog::Write(LOG_TAG_RFID,"IgnoreTankInsertionAndRemovalSignals.");
	BackEnd->IgnoreTankInsertionAndRemovalSignals();
	
	CRFIDBase* RFIDInstance = CRFID::Instance();
	CQLog::Write(LOG_TAG_RFID,"RFIDInstance->ReaderStopAndClear(0).");
	RFIDInstance->ReaderStopAndClear(0); //Reader number = 0
	CQLog::Write(LOG_TAG_RFID,"RFIDInstance->ReaderStopAndClear(1).");
	RFIDInstance->ReaderStopAndClear(1); //Reader number = 1
    
    // Turn Off power supply:
    CBackEndInterface::Instance()->TurnPower(false);
    CBackEndInterface::Instance()->WaitForPowerOnOff();
...
void TGeneralDeviceFrame::HandleStatusUpdateMessage(TMessage &Message)
{
  ...
  switch(Message.WParam)
  {
	...
	case FE_CURRENT_EVAC_AIRFLOW_STATUS:
		 //change the panel's color according to Message.LParam (<0,0,>0)
		 if (Message.LParam < 0) {
			Speed_CurrAirFlowPanel->Color = clRed;
		 } else
			Speed_CurrAirFlowPanel->Color = (Message.LParam) ? clLime : clYellow;
		 break;

	case FE_CURRENT_EVAC_AIRFLOW_SPEED:
	{
		 float NewValue = *((float *)& Message.LParam);
		 Speed_CurrAirFlowPanel->Caption = Format("%.2f%",ARRAYOFCONST((NewValue)));
		 break;
	}
	
	case FE_CURRENT_EVAC_AIRFLOW_IN_A2D:
		 A2D_CurrAirFlowPanel->Caption = IntToStr(Message.LParam);
		 break;
    ...
    case FE_CURRENT_ROLLER_STATUS:
         ...
         else
         {
            ...
            RollerSpeedPanel->Caption = "0";
         }
         break;

    case FE_CURRENT_ROLLER_SPEED:
         RollerSpeedPanel->Caption = IntToStr(Message.LParam);
         break;
