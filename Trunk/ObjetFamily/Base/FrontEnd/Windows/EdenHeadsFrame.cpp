  ...
  : TFrame(Owner)
{
  ...
  for(int i = 0; i < TOTAL_NUMBER_OF_HEATERS; i++)
  {
	 ...
	 if((i%2 != 0)&&(i<TOTAL_NUMBER_OF_HEADS))
	   HeadReqTempEdits[i]->Enabled = false;
     #endif
	 sortTops[i]                              = HeadReqTempEdits[i]->Top;
	 HeadReqTempEditsSaved[i]                 = 0;
  }
...
#ifdef OBJET_MACHINE_KESHET
 const int ChamberPhisicalOffsetArray[NUM_OF_DM_CHAMBERS_THERMISTORS] = {TYPE_CHAMBER_SUPPORT, TYPE_CHAMBER_MODEL7, TYPE_CHAMBER_MODEL6, TYPE_CHAMBER_MODEL5, TYPE_CHAMBER_MODEL4, TYPE_CHAMBER_MODEL3, TYPE_CHAMBER_MODEL2, TYPE_CHAMBER_MODEL1};
#else
  ...
  //Labels
	 ...
	 {
	   if(i == NUM_OF_DM_CHAMBERS_THERMISTORS) //Support
			MaterialLevelLabelArray[i]->Left            =  SpaceX + (SpaceX + PanelWidth)*(i-(NUM_OF_DM_CHAMBERS_THERMISTORS-1));
	   else
			MaterialLevelLabelArray[i]->Left            = SpaceX + (SpaceX + PanelWidth)*(NUM_OF_DM_CHAMBERS_THERMISTORS+5-i);
	 //  MaterialLevelLabelArray[i]->Left            = SpaceX + (SpaceX + PanelWidth)*ChamberPhisicalOffsetArray[i];
...
void __fastcall THeadsFrame::SaveParameterButtonClick(TObject *Sender)
{
  try
  {
	...
	for(int i = 0; i < TOTAL_NUMBER_OF_HEATERS; i++)
	{
		if((i%2 != 0)&&(i<TOTAL_NUMBER_OF_HEADS))
		{
		HeadReqTempEdits[i]->Text =  HeadReqTempEdits[i-1]->Text;
		}
	}
