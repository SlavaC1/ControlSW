        ...
        : TForm(Owner)
{
    ...
    m_ParamsMgr = NULL;
	m_bCommOK = false;
	m_WiperHeightRestore = 0;
	m_YAxisHomeCountRestore = 0;
}
//---------------------------------------------------------------------------
	...
	try
	{
		...
		for(int i = 0; i < n ; i++) {
			...
			if( new_percentage > ProgressBar1->Position )
			{
				...
				QString sProgress = QFormatStr("Downloading MCB firmware file... [%d\%]", ProgressBar1->Position);
...
void __fastcall TMCBDiagForm::WriteButtonClick(TObject *Sender)
{
  ...
  try
  {
	 ...
	 int Value = StrToInt(DiagValueEdit->Text);
