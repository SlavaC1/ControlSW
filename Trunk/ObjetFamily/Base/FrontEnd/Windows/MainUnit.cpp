...
void TMainForm::CloseMessageDlg()
{
    ...
    /* Assumption: No other SW will be around to generate windows titled "Information" */
    MessageFormHDL = FindWindow(NULL/*"TMessageForm"*/, "Information");
    ...
    {
	   CQLog::Write(LOG_TAG_GENERAL, "Error: Can't find notification window to close");
    }
}
//--------------------------------------------------------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void TMainForm::SetupDynamicShortcutsForAutomation()
{
	if (Q2RTApplication->GetQATestsAllowed())
	{
		//Allow executing the reserved Automation script
		RunAutomationScriptAction->Enabled = true;    	

		//We add the shortcuts to the TAction objects themselves in order for them to work.
		//Adding the shortcuts to the TMenuItem objects will only present (GUI-wise) the shortcut.
		TShiftState extraKeys = TShiftState() << ssCtrl << ssShift;
		//Available letter for above mentioned TShiftState: J,K,M,N,Q,V,X,Y,Z
		GetUIFrame()->OpenGeneralDevicesAction->ShortCut =
			ShortCut(Word('G'), extraKeys); //Maintenance :: General Devices
		GetUIFrame()->OpenActuatorsAndSensorsAction->ShortCut =
			ShortCut(Word('A'), extraKeys); //Maintenance :: Actuators & Sensors
		OpenParamsDialogAction->ShortCut =
			ShortCut(Word('P'), extraKeys); //Maintenance :: Parameter Manager
		GetUIFrame()->OpenHeadsControlAction->ShortCut =
			ShortCut(Word('H'), extraKeys); //Maintenance :: Heads Control
		GetUIFrame()->EnterStandbyAction->ShortCut =
			ShortCut(Word('S'), extraKeys); //Maintenance :: Enter Standby
		GetUIFrame()->ExitStandbyAction->ShortCut =
			ShortCut(Word('B'), extraKeys); //Maintenance :: Exit Standby
		GetUIFrame()->MachineActionsAction->ShortCut =
			ShortCut(Word('I'), extraKeys); //Maintenance :: History :: Machine Operations
		GetUIFrame()->MaintenanceCountersAction->ShortCut =
			ShortCut(Word('C'), extraKeys); //Options :: Maintenance Counters...
		// === shortcuts for Wizards ===
		GetUIFrame()->OpenHeadFillingWizardAction->ShortCut =
			ShortCut(Word('F'), extraKeys); //Maintenance :: Heads Filling Calibration wizard
		GetUIFrame()->OpenResinReplacementWizardAction_Noi->ShortCut =
			ShortCut(Word('R'), extraKeys); //Options :: MRW
		GetUIFrame()->CleanHeadsWizardAction->ShortCut =
			ShortCut(Word('E'), extraKeys); //Options :: Head Cleaning Wizard
		GetUIFrame()->CleanWiperWizardAction->ShortCut =
			ShortCut(Word('W'), extraKeys); //Options :: Wiper Cleaning Wizard
		GetUIFrame()->UVReplacementWizard->ShortCut =
			ShortCut(Word('U'), extraKeys); //Options :: UV Lamp Replacement Wizard
		GetUIFrame()->LoadCellCalibrationAction->ShortCut =
			ShortCut(Word('L'), extraKeys); //Options :: Load Cell Calibration Wizard
		GetUIFrame()->ShutdownAction->ShortCut =
			ShortCut(Word('D'), extraKeys); //Options :: Shutdown Wizard
		GetUIFrame()->RollerTiltWizardAction->ShortCut =
			ShortCut(Word('O'), extraKeys); //Maintenance :: Roller Tilt Wizard
		GetUIFrame()->OpenTrayPointsWizardAction->ShortCut =
			ShortCut(Word('T'), extraKeys); //Maintenance :: Tray Calibration Wizard
	}
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void TMainForm::CreateAutomatedTestingSupportFile()
{
	if (Q2RTApplication->GetQATestsAllowed())
	{
		const string AUTOMATION_SUPPORT_FILE_DLM_LISTS = ";";

		CQStringList* strAllInfoList;
		QString supportFilePath = QConvertPath(
			Q2RTApplication->AppFilePath.Value() + LOAD_STRING(IDS_CONFIGS_DIRNAME)
			+ LOAD_STRING(IDS_AUTOMATION_SUPPORT_FILE_NAME));
		CBackEndInterface* backEnd = CBackEndInterface::Instance();

		TPackagesList          PackagesList         = backEnd->GetLicensedPackagesList();
		TLicensedMaterialsList MaterialsList        = backEnd->GetLicensedMaterialsList();
		TLicensedMaterialsList DigitalMaterialsList = backEnd->GetLicensedDigitalMaterialsList();

		try {
			try {
				strAllInfoList = new CQStringList();

				strAllInfoList->Add("[HASP]");

				QString aKey = "Packages=";
				if(! PackagesList.empty())
				{
					for (TPackagesList::iterator it = PackagesList.begin() ; it != PackagesList.end() ; ++it)
						aKey += (it->PackageName.c_str() + AUTOMATION_SUPPORT_FILE_DLM_LISTS);

					DEL_LAST_X_CHARS(aKey,1);
				}
				strAllInfoList->Add(aKey.c_str());

				aKey = "Materials=";
				if(! MaterialsList.empty())
				{
					for (TLicensedMaterialsList::iterator it = MaterialsList.begin() ; it != MaterialsList.end() ; ++it)
						aKey += (it->MaterialName.c_str() + AUTOMATION_SUPPORT_FILE_DLM_LISTS);

					DEL_LAST_X_CHARS(aKey,1);
				}
				strAllInfoList->Add(aKey.c_str());

				aKey = "DigitalMaterials=";
				...
				strAllInfoList->SaveToFile(supportFilePath);
			} catch (...) {
				CQLog::Write(LOG_TAG_GENERAL,"Warning: could not create (automation) support file");
			}
		}
		__finally
		{
			if (strAllInfoList)
				delete strAllInfoList;
		}
	}
}
//---------------------------------------------------------------------------
...
bool TMainForm::SetMaintenanceMode(bool NewMode)
{
  ...
  MainUIFrame->PFFPrintJobAction->Visible = false;
  if (ParamMgr->PFFEnable)
  {
	MainUIFrame->PFFPrintJobAction->Visible = NewMode;
  }
...
void TMainForm::RefreshScriptsMenu(void)
{
  ...
  ScriptsMenuItem->Visible = (! ScriptList.empty());
}
//---------------------------------------------------------------------------
...
void TMainForm::UpdateParameterManager(int NewLevel)
{
  ...
  m_ParamsDlg->AddParamsContainer(CAppParams::Instance(), NewLevel, false, NULL, m_ValidPassword);
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::MainUIFrameShutdownActionExecute(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::TrayRemovalPosition1Click(TObject *Sender)
{
  MainUIFrame->DoTrayUpperPositionActionExecute(Sender);

}
//---------------------------------------------------------------------------
void __fastcall TMainForm::GotoTrayLowerPosition1Click(TObject *Sender)
{
  MainUIFrame->DoTrayLowerPositionActionExecute(Sender);
	
}
//---------------------------------------------------------------------------
void __fastcall TMainForm::Wipe2Click(TObject *Sender)
{
  MainUIFrame->DoWipeActionExecute(Sender);
	
}
//---------------------------------------------------------------------------
void __fastcall TMainForm::ZCalibration1Click(TObject *Sender)
{
  MainUIFrame->OpenZStartCalibrationWizardActionExecute(Sender);
}
//---------------------------------------------------------------------------
...
void TMainForm::HandleSliceImageUpdateMessage(TMessage &Message)
{
  ...
  int i,j = 0;
  //Display support bmps
  for(i = FIRST_SUPPORT_CHAMBER_INDEX; i < LAST_SUPPORT_CHAMBER_INDEX; i++)
  {
	 m_ImageViewForm[j]->DisplayBMP(ParamMgr->TypesArrayPerChamber[i].c_str(),
									   j,
  ...
  //Display model bmps
  for(i = FIRST_MODEL_CHAMBER_INDEX; i < LAST_MODEL_CHAMBER_INDEX; i++)
  {
     //skip support if exists
	 if(CBackEndInterface::Instance()->IsSupportMaterial(ParamMgr->TypesArrayPerChamber[i]))
	   continue;
	 //skip chambers with the same resin
	 if(strcmp(ParamMgr->TypesArrayPerChamber[i-1].c_str(),ParamMgr->TypesArrayPerChamber[i].c_str())!=0)
	 {
	   m_ImageViewForm[j]->DisplayBMP(ParamMgr->TypesArrayPerChamber[i].c_str(),
									   j,
									   CBackEndInterface::RequestBMPAndLock,
...
void __fastcall TMainForm::FormCreate(TObject *Sender)
{
  ...
  RefreshModesDisplay();
  CreateAutomatedTestingSupportFile(); // Provide assistance for QA
  RefreshScriptsMenu();
...
void __fastcall TMainForm::FormShow(TObject *Sender)
{
  ...
  if ((ChooseMaintenanceModePopupMenu->Items->Count - CHOOSE_MAINTENANCE_ITEMS_START_COUNT) == 1)
  {
   ...
   {
	 OpenTrayPointsWizardAction1->Visible = True;
   }
   else
	OpenTrayPointsWizardAction1->Visible = False;
...
//   FactoryShutdownWizardItem->Caption = LOAD_STRING(IDS_FSW_WIZARD); // deprecated

   GetUIFrame()->GetMaintenanceStatusFrame()->SetLabelsPosition();

	// For now, relevant for Objet1000 machines only
	if(CAppParams::Instance()->DualWasteEnabled)
	{
		GetUIFrame()->GetMaintenanceStatusFrame()->HideWasteBar();
		GetUIFrame()->MainStatusFrame->ShowDoubleWasteTanks();
	}

	GetUIFrame()->GetMaintenanceStatusFrame()->RefreshEvacuationVisibility();
	SetupDynamicShortcutsForAutomation(); //Assist QA's Automation process by assigning more key shortcuts
}
 //---------------------------------------------------------------------------
...
void __fastcall TMainForm::ControlEnableDisableEventHandler(TObject *Sender,int ControlID,bool Value)
{
  switch(ControlID)
  {
    ...
    case FE_HASP_UI_CONTROLS_STATUS:
    {
	  ...
	  MainUIFrame->PFFPrintJobAction->Enabled       	  = Value;
...
//      MainUIFrame->FactoryShutdownWizardAction->Enabled   = Value; // deprecated
...
void __fastcall TMainForm::OpenServiceNotesActionExecute(TObject *Sender)
{
  ...
  QString Content  = QFormatStr(LOAD_STRING(IDS_HISTORY_RESTRICTIONS), "Service Notes");
...
void __fastcall TMainForm::OpenHaspToolActionExecute(TObject *Sender)
{
  ...
  QMonitor.NotificationMessageWaitOk("After updating the license, you need to close and re-open the printer-control application.");
  ExternalAppExecute(FileName.c_str(), "");
}
//---------------------------------------------------------------------------
 void __fastcall TMainForm::RunAutomationScriptActionExecute(TObject *Sender)
{
	if (Q2RTApplication->GetQATestsAllowed())
	{
		QString supportScriptPath = QConvertPath(
			Q2RTApplication->AppFilePath.Value() + "Configs\\"
			+ LOAD_STRING(IDS_AUTOMATION_SUPPORT_SCRIPT_NAME));

		if (FileExists(supportScriptPath.c_str()))
		{
			try {
				QPythonIntegratorDM->ExecFile(supportScriptPath.c_str());
			} catch (...) {
			}
		}
	}
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::OpenFansSpeedActionExecute(TObject *Sender)
{
   FansSpeedDlg->Show();
}
//---------------------------------------------------------------------------
...
/*******************************************************************************************************************/
void __fastcall TMainForm::UVCalibrationGNR2Click(TObject *Sender)
{
  MainUIFrame->UVCalibrationGNRWizardExecute(Sender);
    
}
//---------------------------------------------------------------------------

//void __fastcall TMainForm::FactoryShutdownWizardItemClick(TObject *Sender)
//{
//  MainUIFrame->FactoryShutdownWizardActionExecute(Sender);
//}
...
{
	CAppParams      * ParamsMgr = CAppParams::Instance();
	if (ParamsMgr->UpsSafetyEnabled.Value()) {
...
void __fastcall TMainForm::PFF1Click(TObject *Sender)
{
  MainUIFrame->PFFPrintJobActionExecute(Sender);

}
//---------------------------------------------------------------------------

void TMainForm::SetFormLocationVisible(TForm* form, int marginX, int marginY)
{
  /* This technique doesn't enumerates the windows above (on top) the new form's
   * z-order (ref: win32 api call to GetNextWindow), but actually moves
   * the new form to a visible/semi-visible location for the user. */
  TPoint absoluteAppLocation = TPoint(0, 0);
  absoluteAppLocation = MainForm->ClientToScreen(absoluteAppLocation);

  TPoint relativeNewSubformLocation = TPoint(
	  (absoluteAppLocation.x + MainForm->Width ) - form->Width  + marginX ,
	  (absoluteAppLocation.y + MainForm->Height) - form->Height + marginY
  );
  //this conversion should be added only when the new form is a bounded child of the MainForm (typically: it isn't)
  //relativeNewSubformLocation = AboutBox->ScreenToClient(relativeNewSubformLocation);

  //check if the new coordinates will make the new form partially visible.
  //If so - then the MainForm is not screen-centered and thus put it in the center of screen.
  if ((relativeNewSubformLocation.x >= 0) && (relativeNewSubformLocation.x + form->Width <= Screen->Width ) &&
	  (relativeNewSubformLocation.y >= 0) && (relativeNewSubformLocation.y + form->Height<= Screen->Height))
  {
	  form->Left = relativeNewSubformLocation.x;
	  form->Top  = relativeNewSubformLocation.y;
	  form->Position = poDefaultSizeOnly;
  }
  else
  {
