...
const int HW_FW_VERSION_MSC3  = 13;
const int HW_FW_VERSION_MSC4  = 14;
#endif
...
class CBackEndInterface : public CQComponent {
  ...
  public:
   ...
   void PFFPrintJob(char* FolderPath);	
#endif
	...
	void GotoTrayLowerPosition(void);

	void GotoHeadInspectionPosition(void);
 	TQErrCode GotoHeadInspectionPositionUnderWiperCalibration();
        TQErrCode GotoPurgePositionEX(bool Blocking , T_AxesTable AxesTable  );
	// Goto purge positon
    ...
    TQErrCode WaitForOhdbHwDrvToLoad();

    // Reset the FPGA of the OHDB
    TQErrCode ResetOhdbHwDrv();
	...
	BYTE GetNumOfDoor();   //return num of doors

	// Check if door is closed
	TQErrCode CheckIfDoorIsClosed(void);

	// return the correct window for the wizard ("Cover" or "Doors")
	QString GetDoorMessage(void);

	QString GetDoorStatusMessage(void);
	...
	// Get the current evacuation airflow parameter
	int GetCurrentEvacAirFlowParameter(); //Not implemented

	// Get the current ambient temperature parameter
    int GetCurrentAmbientTemperatureParameter(); //Not implemented
    ...
    void TurnHeadHeaters(bool On, int* Temperature = NULL, int NumOfHeads = 0, const bool& bKeepHeatingEvenOpenOrShortCircuit = false);

    //During HOW application ignore head error (short/open circuit)
    void HeadHeatersSetIgnoreError(const bool& bIgnoreError);  // the function set the value - ignore error or not
    //During HOW application ignore head error (short/open circuit)
    bool HeadHeatersIgnoreError(); // the function return the value - ignore error or not

    // Are the temperatures of the heads Ok
    bool AreHeadsTemperaturesOk();

    void PrintHeatersTemperaturesToLog();
    ...
    void MoveMotorToZTrayOutPosition();
	  ...
	  // Move the motor of Z axis down -  (if objet 1000 , the tray will move till the pins position, and not to wheels position)
    ...
    DEFINE_METHOD(CBackEndInterface,TQErrCode,MoveZAxisDownToLowestPosition);

    // Move the motor of Z axis down
    DEFINE_METHOD(CBackEndInterface,TQErrCode,MoveZAxisDownWiperWizard);
    ...
    int MotorGetAxisRightLimit(TMotorAxis Axis);

    // Get the current status of "passed the left limit"
    int MotorGetAxisLeftLimit(TMotorAxis Axis);
    ...
    // Get the remaining free space in resin weight from the waste cartridge(s) altogether
    int GetRemainingWasteWeight();

	//Turn on/off waste mechanism
	DEFINE_METHOD_1(CBackEndInterface,TQErrCode,ActivateWaste,bool);
	DEFINE_METHOD_1(CBackEndInterface,TQErrCode,ActivateRollerAndPurgeWaste,bool);
	bool      IsWasteActive();
	...
	void BEI_WaitUntilTrayInserted();
 ...
 //	void DisableConfirmationBypass(const QString ModeName, QString Dir);
   ...
   DEFINE_METHOD_2(CBackEndInterface,TQErrCode,EnableConfirmationBypass,QString /*ModeName*/, QString /*Directory Name*/);
   DEFINE_METHOD_2(CBackEndInterface,TQErrCode,DisableConfirmationBypass,QString /*ModeName*/, QString /*Directory Name*/);
	...
	bool IsMicroSwitchInserted(int Tank);
	...
	void WriteToHOWHistoryFile(QString Str);
    ...
    QString GetContainerRFTagID(TTankIndex CartridgeID);
    ...
    bool IsTankWeightOk(int Tank, bool CheckHysteresys = false);
...
//OBJET_MACHINE feature
	...
	DEFINE_METHOD_4(CBackEndInterface,TQErrCode,ActivateSignalTower,BYTE,BYTE,BYTE,bool);
	BYTE GetSignalTowerLightState(BYTE light);

	void SendOHDBCleanHeadWizardStartedNotification();
	void SendOHDBCleanHeadWizardEndedNotification();

	void SetSleepingChannels();
};
