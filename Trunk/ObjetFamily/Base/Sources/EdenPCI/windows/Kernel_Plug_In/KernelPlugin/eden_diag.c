...
*  File: eden_diag.c
*
*  Sample user-mode diagnostics application for accessing EDEN
...
/* --------------------------------------------------
    EDEN configuration registers information
   -------------------------------------------------- */
/* Configuration registers information array */
const WDC_REG gEDEN_CfgRegs[] = {
...
#define EDEN_CFG_REGS_NUM sizeof(gEDEN_CfgRegs) / sizeof(WDC_REG)
/* TODO: For read-only or write-only registers, change the direction field of
         the relevant registers in gEDEN_CfgRegs to WDC_READ or WDC_WRITE. */
/* NOTE: You can define additional configuration registers in gEDEN_CfgRegs. */
const WDC_REG *gpEDEN_CfgRegs = gEDEN_CfgRegs;

/* -----------------------------------------------
    EDEN run-time registers information
   ----------------------------------------------- */
/* Run-time registers information array */
/* const WDC_REG gEDEN_Regs[]; */
const WDC_REG *gpEDEN_Regs = NULL;
/* TODO: You can remove the comment from the gEDEN_Regs array declaration and
         fill the array with run-time registers information for your device,
         in which case be sure to set gpEDEN_Regs to point to gEDEN_Regs. */
#define EDEN_REGS_NUM 0
...
static void DiagIntHandler(WDC_DEVICE_HANDLE hDev, EDEN_INT_RESULT *pIntResult);
...
int main(void)
{
    ...
    printf("\n");
    printf("EDEN diagnostic utility.\n");
    ...
    {
        EDEN_ERR("eden_diag: Failed to initialize the EDEN library: %s",
            EDEN_GetLastErr());
        return dwStatus;
    }

    /* Find and open a EDEN device (by default ID) */
    if (EDEN_DEFAULT_VENDOR_ID)
        hDev = DeviceFindAndOpen(EDEN_DEFAULT_VENDOR_ID, EDEN_DEFAULT_DEVICE_ID);
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
        EDEN_ERR("eden_diag: Failed to uninit the EDEN library: %s", EDEN_GetLastErr());
...
static void MenuMain(WDC_DEVICE_HANDLE *phDev)
{
    ...
    {
        printf("\n");
        printf("EDEN main menu\n");
        printf("--------------\n");
        printf("%d. Scan PCI bus\n", MENU_MAIN_SCAN_PCI_BUS);
        printf("%d. Find and open a EDEN device\n", MENU_MAIN_FIND_AND_OPEN);
        if (*phDev)
        {
        ...
        case MENU_MAIN_SCAN_PCI_BUS: /* Scan PCI bus */
            ...
            break;
        case MENU_MAIN_FIND_AND_OPEN: /* Find and open a EDEN device */
            if (*phDev)
                DeviceClose(*phDev);
        ...
        case MENU_MAIN_RW_ADDR: /* Read/write memory and I/O addresses */
            ...
            break;
        case MENU_MAIN_RW_CFG_SPACE: /* Read/Write the EDEN configuration space */
...
static BOOL DeviceFind(DWORD dwVendorId, DWORD dwDeviceId, WD_PCI_SLOT *pSlot)
{
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        EDEN_ERR("DeviceFind: Failed scanning the PCI bus.\n"
    ...
    if (!dwNumDevices)
    {
        EDEN_ERR("No matching device was found for search criteria "
...
/* Open a handle to a EDEN device */
static WDC_DEVICE_HANDLE DeviceOpen(const WD_PCI_SLOT *pSlot)
{
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        EDEN_ERR("DeviceOpen: Failed retrieving the device's resources information.\n"
            "Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
        return NULL;
    }
    ...
    hDev = EDEN_DeviceOpen(&deviceInfo);
    if (!hDev)
    {
        EDEN_ERR("DeviceOpen: Failed opening a handle to the device: %s",
            EDEN_GetLastErr());
        return NULL;
    }
    ...
    dwStatus = WDC_CallKerPlug(hDev, KP_EDEN_MSG_VERSION, &kpVer, &dwKPResult);

    printf("\n");
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        EDEN_ERR("Failed sending a 'Get Version' message (0x%x) to the Kernel PlugIn driver (%s).\n"
            "Error 0x%lx - %s\n",
            KP_EDEN_MSG_VERSION, KP_EDEN_DRIVER_NAME, dwStatus, Stat2Str(dwStatus));
    }
    else if (KP_EDEN_STATUS_OK != dwKPResult)
    {
        EDEN_ERR("Kernel PlugIn 'Get Version' message (0x%x) failed. "
"Kernel PlugIn status: 0x%lx\n", KP_EDEN_MSG_VERSION, dwKPResult);
    }
    else
    {
        printf("Using %s Kernel-PlugIn driver version %ld.%02ld - %s\n",
            KP_EDEN_DRIVER_NAME, kpVer.dwVer/100, kpVer.dwVer%100, kpVer.cVer);
    }
    return hDev;
}

/* Close handle to a EDEN device */
static void DeviceClose(WDC_DEVICE_HANDLE hDev)
{
    if (!hDev)
        return;

    if (!EDEN_DeviceClose(hDev))
    {
        EDEN_ERR("DeviceClose: Failed closing EDEN device: %s",
            EDEN_GetLastErr());
    }
}
...
static void MenuReadWriteAddr(WDC_DEVICE_HANDLE hDev)
{
    ...
    {
        DWORD dwNumAddrSpaces = EDEN_GetNumAddrSpaces(hDev);
        ...
        {
            EDEN_ERR("MenuReadWriteAddr: Error - no active address spaces found\n");
...
static void SetAddrSpace(WDC_DEVICE_HANDLE hDev, PDWORD pdwAddrSpace)
{
    DWORD dwAddrSpace;
    DWORD dwNumAddrSpaces = EDEN_GetNumAddrSpaces(hDev);
    EDEN_ADDR_SPACE_INFO addrSpaceInfo;
    ...
    for (dwAddrSpace = 0; dwAddrSpace < dwNumAddrSpaces; dwAddrSpace++)
    {
        ...
        if (!EDEN_GetAddrSpaceInfo(hDev, &addrSpaceInfo))
        {
            EDEN_ERR("SetAddrSpace: Error - Failed to get address space information: %s",
                EDEN_GetLastErr());
            return;
        }
...
static void MenuReadWriteCfgSpace(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        ...
        if (EDEN_CFG_REGS_NUM)
        {
            printf("\n");
            printf("Configuration registers:\n");
            printf("------------------------\n");
            WDC_DIAG_RegsInfoPrint(gpEDEN_CfgRegs, EDEN_CFG_REGS_NUM,
        ...
        if (DIAG_INPUT_FAIL == DIAG_GetMenuOption(&option,
            EDEN_CFG_REGS_NUM ? MENU_RW_CFG_SPACE_WRITE_REG :
            MENU_RW_CFG_SPACE_WRITE_OFFSET))
        {
            continue;
        }
        ...
        case MENU_RW_CFG_SPACE_READ_ALL_REGS:
            WDC_DIAG_ReadRegsAll(hDev, gpEDEN_CfgRegs, EDEN_CFG_REGS_NUM, TRUE);
            break;
        case MENU_RW_CFG_SPACE_READ_REG:  /* Read from a configuration register */
            WDC_DIAG_ReadWriteReg(hDev, gpEDEN_CfgRegs, EDEN_CFG_REGS_NUM, WDC_READ, TRUE);
            break;
        case MENU_RW_CFG_SPACE_WRITE_REG: /* Write to a configuration register */
            WDC_DIAG_ReadWriteReg(hDev, gpEDEN_CfgRegs, EDEN_CFG_REGS_NUM, WDC_WRITE, TRUE);
            break;
        }
...
static void MenuReadWriteRegs(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        /* Display pre-defined registers' information */
        printf("\n");
        printf("EDEN run-time registers:\n");
        printf("--------------------------\n");   
        WDC_DIAG_RegsInfoPrint(gpEDEN_Regs, EDEN_REGS_NUM, WDC_DIAG_REG_PRINT_ALL);

        printf("\n");
        printf("Read/write the EDEN run-time registers\n");
        printf("-----------------------------------------\n");
        ...
        case MENU_RW_REGS_READ_ALL:
            WDC_DIAG_ReadRegsAll(hDev, gpEDEN_Regs, EDEN_REGS_NUM, FALSE);
            break;
        case MENU_RW_REGS_READ_REG:  /* Read from a register */
            WDC_DIAG_ReadWriteReg(hDev, gpEDEN_Regs, EDEN_REGS_NUM, WDC_READ, FALSE);
            break;
        case MENU_RW_REGS_WRITE_REG: /* Write to a register */
            WDC_DIAG_ReadWriteReg(hDev, gpEDEN_Regs, EDEN_REGS_NUM, WDC_WRITE, FALSE);
            break;
        }
...
static void MenuInterrupts(WDC_DEVICE_HANDLE hDev)
{
    ...
    if (dwIntOptions & INTERRUPT_LEVEL_SENSITIVE)
    {
        /* TODO: You can remove this message after you have modified the
           implementation of EDEN_IntEnable() in eden_lib.c to 
        ...
        printf("Before enabling the interrupts, %s first modify the source "
           "code of EDEN_IntEnable(), in the file eden_lib.c, to "
    ...
    do
    {
        fIntEnable = !EDEN_IntIsEnabled(hDev);
        ...
        case MENU_INT_ENABLE_DISABLE: /* Enable/disable interrupts */
            if (fIntEnable)
            {
                DWORD dwStatus = EDEN_IntEnable(hDev, DiagIntHandler);

                if (WD_STATUS_SUCCESS == dwStatus)
                    printf("Interrupts enabled\n");
                else
                {
                    EDEN_ERR("Failed enabling interrupts. Error 0x%lx - %s\n",
                       dwStatus, Stat2Str(dwStatus));
                }
            }
            else
            {
                if (WD_STATUS_SUCCESS == EDEN_IntDisable(hDev))
                    printf("Interrupts disabled\n");
                else
                    EDEN_ERR("Failed disabling interrupts: %s", EDEN_GetLastErr());
            }
            break;
        }
...
static void MenuEvents(WDC_DEVICE_HANDLE hDev)
{
    ...
    do
    {
        fRegister = !EDEN_EventIsRegistered(hDev);
        ...
        case MENU_EVENTS_REGISTER_UNREGISTER: /* Register/unregister events */
            if (fRegister)
            {
                if (WD_STATUS_SUCCESS == EDEN_EventRegister(hDev, DiagEventHandler))
                    printf("Events registered\n");
                else
                    EDEN_ERR("Failed to register events. Last error:\n%s", EDEN_GetLastErr());
            }
            else
            {
                if (WD_STATUS_SUCCESS == EDEN_EventUnregister(hDev))
                    printf("Events unregistered\n");
                else
                    EDEN_ERR("Failed to unregister events. Last Error:\n%s", EDEN_GetLastErr());
            }
            break;
        }
