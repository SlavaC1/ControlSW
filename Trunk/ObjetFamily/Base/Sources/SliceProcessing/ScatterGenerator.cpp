...
int  CScatterGenerator::s_LargeSliceOnSmallTray = 0;
...
void CScatterGenerator::Init()
{
  ...
  //ModelHeadsNum - how many heads are printing per model
  int ModelHeadsNum         = GetHeadsNumPerMaterial();
  int PrintRes_Y            = ParamsMgr->DPI_InYAxis;
  //how many nozzles are used per material
  g_NozzlesInAllHeads       = NOZZLES_PER_HEAD * ModelHeadsNum;
  //the gap between the nozzles in Y
  g_NozzlesGapInPixels      = PrintRes_Y / (LOGICAL_HEAD_PRINT_Y_DPI * ModelHeadsNum);
  ...
  if ((s_TotalTrayHeight % s_HeadPrintWidth) >= s_TrayRearOffset)
    s_LargeSliceOnSmallTray = s_LargeSliceMaxSize;
}
                                                 ...
                                                 int& LowestScatter,int& HighestScatter)
{
      ...
      m_FirstSliceSizeType = sstSmall;
        ...
        if(ModelMaxHeight300DPI <= s_MediumSliceMaxSize)
        {
          m_FirstSliceSizeType = sstMedium;
            ...
            if(ModelMaxHeight300DPI <= s_LargeSliceOnSmallTray)
			{
			  m_FirstSliceSizeType = sstLargeOnRelativeSmallTray;
			} else
				{
              m_FirstSliceSizeType = sstLarge;
				}
  ...
  switch(m_FirstSliceSizeType)
  {
    ...
    case sstMedium: case sstLargeOnRelativeSmallTray:
      if (sstLargeOnRelativeSmallTray == m_FirstSliceSizeType)
        m_TrayYOffset = 0; // in sstLargeOnRelativeSmallTray trays we start from top of tray (as in sstLarge)
      else
        m_TrayYOffset = s_TrayRearOffset * DPIFactor;
	  ...
	  if (sstLargeOnRelativeSmallTray == m_FirstSliceSizeType)
	  {
   		  HighestScatter = min((long)HighestScatter, (long)((s_HeadPrintWidth - (s_TotalTrayHeight % s_HeadPrintWidth) - s_InterlaceFrontDelta - s_EvenOddDelta) / g_NozzlesGapInPixels));
	  }
      break;
...
int CScatterGenerator::GetLargeOnSmallTraySliceMaxSize()
{
   return  s_LargeSliceOnSmallTray;
}
