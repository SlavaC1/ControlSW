...
class CLayerProcess : public CQThread {
  private:
    ...
    THeadMapEntry m_HeadMapArr_1200DPI[NUM_OF_LOGICAL_HEADS][NOZZLES_PER_HEAD];
    ...
    int CalcResolutionBitShiftFactor(float SliceResolution);

	PBYTE GenerateOneTravelDataWrapped(int CurrentY, int NoOfFires, CSlice *Slice, PBYTE LayerBuffer, bool Direction);
	PBYTE GenerateOneTravelData       (int CurrentY, int NoOfFires, CSlice *Slice, PBYTE LayerBuffer, bool Direction);
 ...
 //	int CalcEndOfPlot(int StartOfPlot, int SliceWidth, float SliceRes, int LastHeadOffset_1200DPI, int AdvanceFire, int InitialEmptyFires);
	...
	/* Collecting data for stipulator. Initializes only according to "dumpPassToFile" flag at FillLayerBidirection300 */
	StipulatorData* m_stipulatorData;
...
public:
  ...
  CScatterTester(CLayerProcess* LayerProcObj, const QString& Name,bool RosterRegistration)
   : m_LayerProcess(LayerProcObj), CQComponent(Name,RosterRegistration)
   {
    ...
    m_MinMotorLocation = 0 ;
    memset(&m_EmulatedModelStyle, 0 , sizeof(TEmulatedModelStyle));
    m_EndHeight = 0;
    m_StartHeight = 0;
    m_MachineSequencer  = NULL;
    m_Scatter = NULL;
    m_Slice = NULL;
    m_Layer  = NULL;
  ...
  void		CheckLayer(int NoOfPasses, int YScatterOffset);
  TQErrCode CleanUp();
