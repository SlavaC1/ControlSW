...
class CActuatorBase : public CQComponent{
  ...
  protected:
    ...
    void PurgeBathSensorCalc(bool CurrRead);
    int GetRSSOnValue(void); //RSS, itamar
	int GetRSSOffValue(void);//RSS, itamar
	char* GetActuatorName(int ActuatorID);
    ...
    CSensorStabilizer m_PurgeBathSensorStabilizer;
  ...
  public:
    ...
    //     bool GetStabilizedPurgeBathSensorStatus(); returns "is full"
    DEFINE_METHOD(CActuatorBase,bool,GetStabilizedPurgeBathSensorStatus);
	...
	DEFINE_V_METHOD_2(CActuatorBase,TQErrCode,SetAmbientHeaterOnOff,bool,BYTE)=0;
...
class CActuator : public CActuatorBase {
 ...
 private:
  ...
  static void AmbientHeaterOnOffResponse(int TransactionId,PVOID Data,
									unsigned DataLength,TGenericCockie Cockie);
...
public:
	...
	TQErrCode SetAmbientHeaterOnOff(bool OnOff, /* TAmbientHeaterSelector */ BYTE Heater);
};
