...
//Members
	...
	bool m_HeadsInsertedWrong[TOTAL_NUMBER_OF_HEADS_HEATERS/2];
    ...
    bool m_HeadsToBeReplaced          [TOTAL_NUMBER_OF_HEADS_HEATERS/2];
...
//Methods
    ...
    void InitHeadsToBeReplacedArray();

    void SetIndexes(int VacuumLeakagePageIndex, int PurgePageIndex);
    void HeadsHeating1PageEnter          (CWizardPage* WizardPage);
    void HeadsHeatingCycle(const int TimeOut=DEFAULT_TIMEOUT,int* HeadsTemperatures=NULL, bool IncludePreHeater=true,  const bool& bKeepHeatingEvenOpenOrShortCircuit = false);
    void HeadsOnlyHeatingCycle(const int TimeOut=DEFAULT_TIMEOUT,int* HeadsTemperatures=NULL, const bool& bKeepHeatingEvenOpenOrShortCircuit = false);
    void SelectedHeadsHeatingCycle(const int TimeOut=DEFAULT_TIMEOUT, int* HeadsTemperatures=NULL , const bool& bKeepHeatingEvenOpenOrShortCircuit = false);
