...
{
  m_RepeatTest = false;
  m_PrevTestPatternXPos= 0 ;
  m_PrevTestPatternYPos = 0;
  ...
  {
    CRadioGroupWizardPage *pPage = new CRadioGroupWizardPage(LOAD_STRING(IDS_ARE_RESULTS_SATISFYING),QUESTION_IMAGE_ID, wpPreviousDisabled);
    pPage->Strings.Add(LOAD_STRING(IDS_NO_REPEAT));
    pPage->Strings.Add(LOAD_STRING(IDS_YES_FINISH));
...
void CPrintingPositionWizard::PageLeave(CWizardPage *WizardPage, TWizardPageLeaveReason LeaveReason)
{
  ...
  switch(WizardPage->GetPageNumber())
  {
    ...
    case ppPositionCorrection:
         ...
         break;
    case ppCloseDoor:
         EnableAllAxesAndHome(NULL,BLOCKING,FORCE_HOME); //Home all Axis
         break;
