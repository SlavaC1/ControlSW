...
void CRollerBase::UpdateBumperImpactCounter ()
{
   m_MutexDataBase.WaitFor();
	  m_BumperImpactCounter ++;
   m_MutexDataBase.Release();
}
//This procedure get the value of Roller on off of OHDB(8051).
...
int CRollerBase::GetBumperImpactCounter (void)
{
   int ImpactCounter;

   m_MutexDataBase.WaitFor();
	  ImpactCounter = m_BumperImpactCounter;
   m_MutexDataBase.Release();
...
TQErrCode CRollerBase::ResetBumperImpactCounter (void)
{
   m_MutexDataBase.WaitFor();
...
void CRoller::NotificationBumperMessage (int TransactionId,PVOID Data,unsigned DataLength,TGenericCockie Cockie)
{
   ...
   if(DataLength != sizeof(TOHDBBumperImpactDetected))
	...
	if(BackEnd->IsWizardRunning() == false)   /*print in case we are not in wizard*/
	{
		CQLog::Write(LOG_TAG_GENERAL,"!!!Bumper Notification!!!");
		long CurrentY = (int)Motor->GetAxisLocation(AXIS_Y);
		long CurrentX = (int)Motor->GetAxisLocation(AXIS_X);
		long CurrentZ = (int)Motor->GetAxisLocation(AXIS_Z);
			CQLog::Write(LOG_TAG_GENERAL,"!!!Position X = %d, Y = %d, Z = %d", CurrentX,CurrentY,CurrentZ);
		InstancePtr->UpdateBumperImpactCounter();
		CQLog::Write(LOG_TAG_GENERAL,"!!!The number of bumper impact is %d", InstancePtr->GetBumperImpactCounter());
		if(InstancePtr->GetBumperImpactCounter() >= InstancePtr->m_ParamsMgr->BumperImpactCounterFilter)
			{
...
void CRollerBase::SetPrevRollerSpeed(int prevRollerSpeed)
{
	m_OrgRollerSpeed = prevRollerSpeed;
}
int CRollerBase::GetPrevRollerSpeed()
{
	return  m_OrgRollerSpeed;
}
