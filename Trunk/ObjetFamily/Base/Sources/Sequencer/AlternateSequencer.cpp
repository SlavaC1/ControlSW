...
// ---------------
   ...
   while( PassInLayer >= 0 &&
          (unsigned)PassInLayer < Layer->GetNoOfPasses() )
      {
...
TQErrCode CMachineSequencer::Double300DpiAlternate600PrintSequencer(CLayer *Layer)
{
         ...
         )
   {
      if(PassIsNotEmpty)
      {
         ...
         Location = m_SequencerBlock.KeepZLocation +
                    CONFIG_ConvertZumToStep(m_SequencerBlock.CurrentLayerHeightInUM - m_ParamsMgr->Z_MoveToStartNewLayer_um);
         ...
         if ((Err = CheckEndOfPass()) != Q_NO_ERROR)
         {
             ...
             if(Err != Q2RT_FIFO_NOT_EMPTY_ERROR)
             {
               CQLog::Write(LOG_TAG_PRINT,"Error during checking OHDB for end pass.:Slice%d.",Layer->GetSliceNumber());
               return Err;
             }
             else
             {
