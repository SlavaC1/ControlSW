...
extern const int XILINX_REG_PRINT_VOLTAGE       = 0x07;
extern const WORD XILINX_PRINTING_ENABLED       = 0x0001;
...
class CMachineSequencer : public CQSequencer {
  private:
   ...
   int 	 m_PurgeIntervalsToPerformBrushWipe;
...
// Smart Roller vvvvvvvvvv
   bool  IsCurrSliceAccumulating() const;
// Smart Roller ^^^^^^^^^^
	...
	TQErrCode RunPrintingDuringPatternTest(bool bPrintTheEvenHeads);  
    void LogOHDB_FIFOStatus();
  public:
    // Constructor
    CMachineSequencer(const QString& Name);

    // Destructor
    virtual ~CMachineSequencer(void);
	...
	CTrayBase* GetTrayHeaterInstance();
	CTrayBase* GetTrayPlacerInstance();
	...
	// Return a pointer to the fans instance
	CFansBase* GetFansInstance();

	CSignalTower* GetSignalTower();
    ...
    TQErrCode GotoTrayLowerPositionSequencer(void);
	...
	TQErrCode ActivateSignalTower(BYTE Red, BYTE Green, BYTE Yellow, bool ValidateState = true);
	...
	TQErrCode WriteToHOWHistoryFile(QString);
	TQErrCode OpenHOWLampsHistoryFile();
	TQErrCode CloseHOWLampsHistoryFile();
	...
	int GetCurrentTemperatureCelcius();
	void SetOHDBAmbientParams();
	int GetPrevHighLimit();
	void SetPrevHighLimit(int prevHighLimit);
	int GetPrevRollerSpeed();
	void SetPrevRollerSpeed(int prevRollerSpeed);
};
