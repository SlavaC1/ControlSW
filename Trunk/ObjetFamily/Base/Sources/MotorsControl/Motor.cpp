...
{   
   ...
   {
      bool LastMode = m_Axis[Axis]->GetMotorMode();
...
{
   //does not need calibration. (1400 steps is a ball-park number that ensures the basket is clearly far from the heads)
   if (AvoidContactWithBasket)
	  return GoWaitAbsolutePositionSequence(AXIS_T, 1400);
...
TQErrCode CMotorsDummy::SetMotorEnableDisable(TMotorAxis Axis,bool Mode)
{
   ...
   TQErrCode Err = CMotorsBase::SetMotorEnableDisable(Axis, Mode);
