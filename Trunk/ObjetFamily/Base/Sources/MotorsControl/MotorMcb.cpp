...
#define OBJET1000_TIME_OUT_IN_SECONDS 240
...
TQErrCode CMotors::GoToAbsolutePosition(TMotorAxis Axis, TMotorPosition Position,TMotorUnits Units, bool IgnoreBumperImpact)
{
   ...
   if( MotorBase->IsMotorMoving(Axis) )
   {
	...
	if ((Err = MotorBase->CheckForEndOfMovement(Axis,Axis == AXIS_Z ? 150 : 10)) != Q_NO_ERROR)
      return Err;
   }
...
TQErrCode CMotors::WaitForEndOfMovement(TMotorAxis Axis,int ExtraTimeWaitingSec)
{
   ...
   switch(Axis)
      ...
      case AXIS_X:
		 ...
		 {
		   	ExtraTimeWaitingSec *= 2; // 4 min only for objet100, Axis z
		 }
         // Wait for Notify message.
	  ...
	  case AXIS_Z:
		   if( (Q2RTApplication->GetMachineType()) == (mtObjet1000 ))
		   {
				ExtraTimeWaitingSec = OBJET1000_TIME_OUT_IN_SECONDS; // 4 min only for objet100, Axis z
		   }
