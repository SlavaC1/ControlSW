...
private:
  ...
  bool m_IsHistoryFileEnabled;
  bool m_EchoLogToHistory;
...
protected:
  ...
  unsigned StartAsyncHeatingCycle(const int NumberOfHeadHeaters, bool IncludePreHeater=true, int* HeadsTemperatures=NULL, const bool& bKeepHeatingEvenOpenOrShortCircuit = false);
  void EndAsyncHeatingCycle(unsigned StartTime, unsigned TimeOut);
  void HeadsHeatingCycle(const int TimeOut = DEFAULT_TIMEOUT,const int NumberOfHeadHeaters = TOTAL_NUMBER_OF_HEATERS,bool IncludePreHeater = true,int* HeadsTemperatures=NULL, const bool& bKeepHeatingEvenOpenOrShortCircuit = false);
  void HeadsFillingCycle(unsigned int FillingTimeout);
  void HeadsDrainingCycle (CWizardPage *Page, unsigned int DrainingTime, bool CheckingEmpty=false,bool wipe=true, bool bHomeT = true);
...
public:
	...
	void XY_AxisGoHomePosition_Z_AxisGoHeadInspectionPosition(T_AxesTable a_AxesTable = NULL, bool Blocking = true, bool ForceHome = false, bool bIsModelOnTheTray = false);

	CHistoryFile *HistoryFile;
	void EnableHistoryFile();
    void EchoLogToHistory();
