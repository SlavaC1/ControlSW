...
protected:
   static CLinearInterpolator <int> m_HeadA2DToCelcius;
   static CLinearInterpolator <int> m_HeadCelciusToA2D;
public:
...
class CBlockTemperature : public CTemperature {
protected:
   static CLinearInterpolator <int> m_BlockA2DToCelcius;
   static CLinearInterpolator <int> m_BlockCelciusToA2D;
public:
   static void LoadTemperatureTable(const QString& FileName);
   int ConvertCelciusToA2D(int);
   int ConvertA2DToCelcius(int);
...
class CQSingleHeadHeater : public CQComponent {
  ...
  private:
    ...
    CBlockTemperature  m_CurrentBlockTemperature;
    CBlockTemperature  m_SetBlockTemperature ;
    CBlockTemperature  m_SetBlockStandbyTempearture;
    ...
    void UpdateCurrentTemperatureA2D(int, const TTemperatureType& temperatureType = HEAD_TEMPERATURE);
    void UpdateSetTemperatureA2D(int, const TTemperatureType& temperatureType = HEAD_TEMPERATURE);
    void UpdateStandbyTemperatureA2D(int, const TTemperatureType& temperatureType = HEAD_TEMPERATURE);
    void UpdateIfWorkingHead(bool);

    //bool GetTemperatureOk(void);
    int GetCurrentTemperatureInCelcius(const TTemperatureType& temperatureType = HEAD_TEMPERATURE);
    int GetCurrentTemperatureInA2D(const TTemperatureType& temperatureType = HEAD_TEMPERATURE);
    int GetSetTemperatureInA2D(const TTemperatureType& temperatureType = HEAD_TEMPERATURE);
    int GetSetTemperatureInCelcius(const TTemperatureType& temperatureType = HEAD_TEMPERATURE);
    int GetStandbyTemperatureInA2D(const TTemperatureType& temperatureType = HEAD_TEMPERATURE);
    int GetStandbyTemperatureInCelcius(const TTemperatureType& temperatureType = HEAD_TEMPERATURE);
...
private:
	...
	float m_3_3_V_PowerSupply;
	float m_1_2_V_PowerSupply;
  ...
  protected:
    ...
    bool m_BlockTableConversionExist;
    ...
    TTemperatureType SetTemperatureType(const int& iHeadNum);
    ...
    void UpdatePowerSupply(float Vpp,float V24,float VDD,float VCC, float V_3_3, float V_1_2);
    ...
    bool m_IgnoreErrorDuringHOW; // the parameter is true only during HOW (we want to ignore the error message)
  public:
    // Constructor
    ...
    DEFINE_V_METHOD_1(CHeadHeatersBase,TQErrCode,SetHeatersTemperature, bool)=0;
    ...
    virtual TQErrCode SetHeatersTemperature(THeaterElementsTable, const bool& bKeepHeatingEvenOpenOrShortCircuit = false) =0;
    ...
    DEFINE_METHOD(CHeadHeatersBase,float,Get_A2D_3_3_PowerSupply);
    DEFINE_METHOD(CHeadHeatersBase,float,Get_A2D_1_2_PowerSupply);
    DEFINE_METHOD(CHeadHeatersBase,bool,IsVppPowerSupplyLegal);
    DEFINE_METHOD(CHeadHeatersBase,bool,IsVDDPowerSupplyLegal);

    DEFINE_V_METHOD_1(CHeadHeatersBase,float,GetPowerSupply,int);
    DEFINE_V_METHOD_1(CHeadHeatersBase,bool,IsPowerSupplyLegal,int);
    DEFINE_V_METHOD_1(CHeadHeatersBase,float,GetMinVoltagesVal,int);
    DEFINE_V_METHOD_1(CHeadHeatersBase,float,GetMaxVoltagesVal,int);
    ...
    int CalculatePotValue(float ReqVoltage);
    float EstimateHeadVoltage(int PotVal);
    bool Get_Ignore_Error();
    void SetIgnoreError(const bool& bIgnoreError);

	DEFINE_V_METHOD(CHeadHeatersBase,TQErrCode,SendHeadsCleaningWizardStartedNotification)=0;
	DEFINE_V_METHOD(CHeadHeatersBase,TQErrCode,SendHeadsCleaningWizardEndedNotification)=0;
};
...
class CHeadHeaters : public CHeadHeatersBase {
 ...
 private:
   ...
   bool m_flagHeadCleanWizardStartedUnderUse;
   bool m_flagHeadCleanWizardEndedUnderUse;
   ...
   static void HeadsCleanWizardStartedAckResponse(int TransactionId, PVOID Data, unsigned DataLength, TGenericCockie Cockie);

   static void HeadsCleanWizardEndedAckResponse(int TransactionId, PVOID Data, unsigned DataLength, TGenericCockie Cockie);
...
public:
    ...
    TQErrCode SetHeatersTemperature( bool bKeepHeatingEvenOpenOrShortCircuit = false);

    TQErrCode SetHeatersTemperature(THeaterElementsTable, const bool& bKeepHeatingEvenOpenOrShortCircuit = false);
	...
	TQErrCode SendHeadsCleaningWizardStartedNotification();
	TQErrCode SendHeadsCleaningWizardEndedNotification();
};
