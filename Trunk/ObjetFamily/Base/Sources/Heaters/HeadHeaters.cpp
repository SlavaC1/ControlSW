...
//------------------------------------
// Static members
CLinearInterpolator<int> CHeadTemperature::m_HeadA2DToCelcius;
CLinearInterpolator<int> CHeadTemperature::m_HeadCelciusToA2D;
CLinearInterpolator<int> CBlockTemperature::m_BlockA2DToCelcius;
CLinearInterpolator<int> CBlockTemperature::m_BlockCelciusToA2D;
...
{
    m_HeadA2DToCelcius.LoadFromFile(FileName);
    m_HeadCelciusToA2D.LoadInverseFromFile(FileName);
}

int CHeadTemperature::ConvertCelciusToA2D(int value)
{
    return (m_HeadCelciusToA2D.Interpolate(value));
}

int CHeadTemperature::ConvertA2DToCelcius(int value)
{
    return (m_HeadA2DToCelcius.Interpolate(value));
}

void CBlockTemperature::LoadTemperatureTable(const QString& FileName)
{
    m_BlockA2DToCelcius.LoadFromFile(FileName);
    m_BlockCelciusToA2D.LoadInverseFromFile(FileName);
}

int CBlockTemperature::ConvertCelciusToA2D(int value)
{
    return (m_BlockCelciusToA2D.Interpolate(value));
}

int CBlockTemperature::ConvertA2DToCelcius(int value)
{
    return (m_BlockA2DToCelcius.Interpolate(value));
}
...
CQSingleHeadHeater::CQSingleHeadHeater(const QString& Name,int Index) : CQComponent(Name)
{
    ...
    TTemperatureType currentTemperatureType = HEAD_TEMPERATURE;
    if((m_ParamsMgr->BlockTableConversionExist )&&(Index> HEAD_7))
    {
         currentTemperatureType  = BLOCK_TEMPERATURE;
    }
    UpdateCurrentTemperatureA2D(GetParameter(),currentTemperatureType );
}
...
int CQSingleHeadHeater::GetCurrentTemperatureInCelcius(const TTemperatureType& temperatureType)
{
    int Temperature;
    if(HEAD_TEMPERATURE == temperatureType)
    {
        m_MutexSingleHead.WaitFor();
    ...
    {
       m_MutexSingleHead.WaitFor();
       Temperature = m_CurrentBlockTemperature.CelciusValue();
       m_MutexSingleHead.Release();
    }
    return (Temperature);
}

int CQSingleHeadHeater::GetCurrentTemperatureInA2D(const TTemperatureType& temperatureType)
{
    ...
    {
       m_MutexSingleHead.WaitFor();
       Temperature = m_CurrentBlockTemperature.A2DValue();
       m_MutexSingleHead.Release();
    }
    return (Temperature);
}

void CQSingleHeadHeater::UpdateCurrentTemperatureA2D(int TA2D, const TTemperatureType& temperatureType )
{
	...
	int BlockLowThreshold  = ParamsMgr->BlockTemperatureLowThershold;
	int BlockHighThreshold = ParamsMgr->BlockTemperatureHighThershold;
    ...
    {
        m_MutexSingleHead.WaitFor();
        m_CurrentBlockTemperature.AssignFromA2D(TA2D);

		if (m_CurrentBlockTemperature < (m_SetBlockTemperature.A2DValue() - BlockHighThreshold))
			m_TemperatureOk = HEAD_HOT;
		else if(m_CurrentBlockTemperature > (m_SetBlockTemperature.A2DValue() + BlockLowThreshold))
			m_TemperatureOk = HEAD_COLD;
        else
            m_TemperatureOk = HEAD_TEMPERATURE_OK;
...
int CQSingleHeadHeater::GetSetTemperatureInCelcius( const TTemperatureType& temperatureType)
{
    int Temperature;
    if(HEAD_TEMPERATURE == temperatureType)
    {
    ...
    {
        m_MutexSingleHead.WaitFor();
        Temperature = m_SetBlockTemperature.CelciusValue();
        m_MutexSingleHead.Release();
    }
    return Temperature;
}

int CQSingleHeadHeater::GetSetTemperatureInA2D( const TTemperatureType& temperatureType)
{
    int Temperature;
    if(HEAD_TEMPERATURE == temperatureType)
    {
    ...
    {
         m_MutexSingleHead.WaitFor();
        Temperature = m_SetBlockTemperature.A2DValue();
        m_MutexSingleHead.Release();
    }
    return Temperature;
}

void CQSingleHeadHeater::UpdateSetTemperatureA2D(int TA2D, const TTemperatureType& temperatureType)
{
  if(HEAD_TEMPERATURE == temperatureType)
  {
        m_MutexSingleHead.WaitFor();
  ...
  {
     m_MutexSingleHead.WaitFor();
     m_SetBlockTemperature.AssignFromA2D(TA2D);
     m_MutexSingleHead.Release();

       //Refresh temperature status
      UpdateCurrentTemperatureA2D(m_CurrentBlockTemperature.A2DValue(), BLOCK_TEMPERATURE);
  }
}

int CQSingleHeadHeater::GetStandbyTemperatureInCelcius(const TTemperatureType& temperatureType)
{
    int Temperature;
    if(HEAD_TEMPERATURE == temperatureType)
    {
    ...
    {
        m_MutexSingleHead.WaitFor();
        Temperature = m_SetBlockStandbyTempearture.CelciusValue();
        m_MutexSingleHead.Release();
    }
    return (Temperature);
}

int CQSingleHeadHeater::GetStandbyTemperatureInA2D(const TTemperatureType& temperatureType)
{
    int Temperature;
    if(HEAD_TEMPERATURE == temperatureType)
    {
    ...
    {
       m_MutexSingleHead.WaitFor();
       Temperature = m_SetBlockStandbyTempearture.A2DValue();
       m_MutexSingleHead.Release();
    }
    return (Temperature);
}

void CQSingleHeadHeater::UpdateStandbyTemperatureA2D(int TA2D, const TTemperatureType& temperatureType)
{
    if(HEAD_TEMPERATURE == temperatureType)
    {
        m_MutexSingleHead.WaitFor();
    ...
    {
        m_MutexSingleHead.WaitFor();
        m_SetBlockStandbyTempearture.AssignFromA2D(TA2D);
        m_MutexSingleHead.Release();
    }
}
...
CHeadHeatersBase::CHeadHeatersBase(const QString& Name) : CQComponent(Name)
{
    int i;
     m_ParamsMgr = CAppParams::Instance();
     m_BlockTableConversionExist = m_ParamsMgr->BlockTableConversionExist ;
    ...
    try
    {
        ...
        CQLog::Write(LOG_TAG_HEAD_HEATERS, "HeadHeater.txt file was downloaded.");
        if(m_BlockTableConversionExist)
        {
           CBlockTemperature::LoadTemperatureTable(Q2RTApplication->AppFilePath.Value() + LOAD_STRING(IDS_CONFIGS_DIRNAME) + "BlockHeaters.txt");
           CQLog::Write(LOG_TAG_HEAD_HEATERS, "BlocksHeater.txt file was downloaded");
        }
    }
    ...
    INIT_METHOD(CHeadHeatersBase,Get_A2D_3_3_PowerSupply);
    INIT_METHOD(CHeadHeatersBase,Get_A2D_1_2_PowerSupply);

    INIT_METHOD(CHeadHeatersBase,GetMinVoltagesVal);
    INIT_METHOD(CHeadHeatersBase,GetMaxVoltagesVal);
    INIT_METHOD(CHeadHeatersBase,IsPowerSupplyLegal);
    INIT_METHOD(CHeadHeatersBase,GetPowerSupply);
	...
	INIT_METHOD(CHeadHeatersBase,SendHeadsCleaningWizardStartedNotification);
	INIT_METHOD(CHeadHeatersBase,SendHeadsCleaningWizardEndedNotification);
...
TTemperatureType CHeadHeatersBase::SetTemperatureType(const int& iHeadNum)
{
    TTemperatureType currentTemperatureType = HEAD_TEMPERATURE;
    if((m_BlockTableConversionExist )&&(iHeadNum> HEAD_7))
    {
       currentTemperatureType  = BLOCK_TEMPERATURE;
    }
...
CHeadHeaters::CHeadHeaters(const QString& Name): CHeadHeatersBase(Name)
{
	...
	m_flagHeadCleanWizardStartedUnderUse = 0;
	m_flagHeadCleanWizardEndedUnderUse = 0;
...
void CHeadHeatersBase::UpdatePowerSupply(float Vpp, float V24, float VDD, float VCC, float V_3_3, float V_1_2)
{
    m_MutexDataBase.WaitFor();
	...
	m_3_3_V_PowerSupply    = V_3_3;
	m_1_2_V_PowerSupply    = V_1_2;
...
bool CHeadHeatersBase::IsPowerSupplyLegal(int index)
{
    bool legal = true;
    float value = GetPowerSupply(index);
    if ( -1 == value )
    {
	   CQLog::Write(LOG_TAG_HEAD_HEATERS,"Power Supply Legal failed because the index=%d is not valid",index);
	   legal = false;
    }
    if( value < MinPowerSupplyValue[index] || value > MaxPowerSupplyValue[index] )
    {    legal = false;}
    return legal;
}

float CHeadHeatersBase::GetPowerSupply(int index)
{
    float value;
    switch (index)
    {
        case VDD_POWER_SUPPLY:
        {
           return GetVDDPowerSupply();
        }
        case VPP_POWER_SUPPLY:
        {
            return GetVppPowerSupply();
        }
        case HEATER_24_POWER_SUPPLY:
        {
            return Get24VPowerSupply();

        }
        case VCC_POWER_SUPPLY:
        {
           return GetVCCPowerSupply();
        }
        case A2D_3_3_POWER_SUPPLY:
        {
             return Get_A2D_3_3_PowerSupply();
        }
        case A2D_1_2_POWER_SUPPLY:
        {
          return Get_A2D_1_2_PowerSupply();
        }
    }

     CQLog::Write(LOG_TAG_HEAD_HEATERS,"Eroor: Get PowerSupply failed because the index=%d is not valid",index);
     return -1;
}
...
float CHeadHeatersBase::Get_A2D_3_3_PowerSupply(void)
{
    float A2D_3_3;

    m_MutexDataBase.WaitFor();
    A2D_3_3 = m_3_3_V_PowerSupply;
    m_MutexDataBase.Release();

    return A2D_3_3;
}

float CHeadHeatersBase::Get_A2D_1_2_PowerSupply(void)
{
    float A2D_1_2;

    m_MutexDataBase.WaitFor();
    A2D_1_2 = m_1_2_V_PowerSupply;
    m_MutexDataBase.Release();

    return A2D_1_2;
}

float CHeadHeatersBase::GetMinVoltagesVal(int index)
{
   return MinPowerSupplyValue[index];
}
float CHeadHeatersBase::GetMaxVoltagesVal(int index)
{
   return MaxPowerSupplyValue[index];
}
...
TQErrCode CHeadHeatersBase::DebugDisplayHeadStatus(void)
{
    ...
    for(int i = 0; i < TOTAL_NUMBER_OF_HEATERS; i++)
    {
      TTemperatureType currentTemperatureType = SetTemperatureType(i);
      Output += QIntToStr (m_Heaters[i]->GetCurrentTemperatureInCelcius(currentTemperatureType));
      Output += " ";
    }
    ...
    {
       TTemperatureType currentTemperatureType = SetTemperatureType(i);
       Output += QIntToStr (m_Heaters[i]->GetCurrentTemperatureInA2D(currentTemperatureType));
        Output += " ";
    }
...
TQErrCode CHeadHeatersBase::DisplayHeadStatus(void)
{
    ...
    for(i=0; i < TOTAL_NUMBER_OF_HEATERS; i++)
    {
		...
		CQLog::Write(LOG_TAG_HEAD_HEATERS, "%s =   %d      %d    %d     ",
					 ...
					 m_Heaters[i]->GetCurrentTemperatureInA2D(currentTemperatureType),
					 m_Heaters[i]->GetSetTemperatureInA2D(currentTemperatureType),
                     m_Heaters[i]->GetStandbyTemperatureInA2D(currentTemperatureType));
    }
...
TQErrCode CHeadHeaters::SetHeaterOnOff(bool OnOff,WORD HeatersMask)
{
    ...
    if (m_FlagSetHeaterOnOffComUnderUse)
        throw EHeadHeater(Q2RT_HEADHEATER_SET_ONOFF_SEND_2MSGS_ERROR);
...
int CHeadHeatersBase::CalculatePotValue(float ReqVoltage)
{
	return CONVERT_HEAD_VOLTAGE_TO_POT_VALUE(ReqVoltage);
}
//---------------------------------------------------------------------------

float CHeadHeatersBase::EstimateHeadVoltage(int PotVal)
{
    return CONVERT_HEAD_POT_VALUE_TO_VOLTS(PotVal);
}
...
TQErrCode CHeadHeatersBase::CheckThermistorsIntegrity(int Head,int A2DValue)
{
    ...
    if(A2DValue < THERMISTOR_SHORT_VALUE)
    {
        ...
        if(Get_Ignore_Error())
        {
             ...
             CQLog::Write(LOG_TAG_HEAD_HEATERS,"Heads Heater %s, Thermistor short (%d) but application keep heating during HOW",
    ...
    if(A2DValue > THERMISTOR_OPEN_VALUE)
    {
         ...
         if(Get_Ignore_Error())
         {
         	...
         	CQLog::Write(LOG_TAG_HEAD_HEATERS,"Heads Heater %s, Thermistor open (%d) but application keep heating during HOW",
                         GetHeaterStr(Head).c_str(),
                         A2DValue);
         }
         else
...
TQErrCode CHeadHeatersBase::CheckThermistorsStatus(bool DuringPrint)
{
    ...
    for(i = 0; i < TOTAL_NUMBER_OF_HEATERS; i++)
    {
        ...
        {
            TTemperatureType currentTemperatureType = SetTemperatureType(i);
            Err = CheckThermistorsIntegrity(i,m_Heaters[i]->GetCurrentTemperatureInA2D(currentTemperatureType));
         }
        //Statistics
        if(DuringPrint)
 ...
 bool CHeadHeatersBase::Get_Ignore_Error()
{
     CQLog::Write(LOG_TAG_HEAD_HEATERS, "Gt ignore error %d", (int)m_IgnoreErrorDuringHOW);
     return m_IgnoreErrorDuringHOW;
}

void CHeadHeatersBase::SetIgnoreError(const bool& bIgnoreError)
{

    m_IgnoreErrorDuringHOW = bIgnoreError;
    CQLog::Write(LOG_TAG_HEAD_HEATERS, "Set ignore error %d", (int)m_IgnoreErrorDuringHOW);
}
    ...
    for(int i=0; i<TOTAL_NUMBER_OF_HEATERS; i++)
    {
        if(m_Heaters[i]->GetIfHeadWorking())
        {
              ...
              CQLog::Write(LOG_TAG_HEAD_HEATERS,"HeadTemperature %s: Set(%d A/D = %d°C), Current(%d A/D = %d°C)",
                          GetHeaterStr(i).c_str(),
                         m_Heaters[i]->GetSetTemperatureInA2D(currentTemperatureType),
                         m_Heaters[i]->GetSetTemperatureInCelcius(currentTemperatureType),
                         m_Heaters[i]->GetCurrentTemperatureInA2D(currentTemperatureType),
                         m_Heaters[i]->GetCurrentTemperatureInCelcius(currentTemperatureType));
...
TQErrCode CHeadHeaters::SetHeatersTemperature(bool bKeepHeatingEvenOpenOrShortCircuit)
{
    SetHeatersTemperature(m_HeatersSetTemperatures, bKeepHeatingEvenOpenOrShortCircuit);
    return Q_NO_ERROR;
}

TQErrCode CHeadHeaters::SetHeatersTemperature(THeaterElementsTable HeaterSetTable, const bool& bKeepHeatingEvenOpenOrShortCircuit)
{
    // Verify if we are already not performing this command
	...
	for(i=0; i<TOTAL_NUMBER_OF_HEATERS ; i++)
	{
		...
		CQLog::Write(LOG_TAG_HEAD_HEATERS,"SendMsg.SetElements.HeaterTable = %d",SendMsg.SetElements.HeaterTable[i]);
	}
	...
	SendMsg.BlockLowThreshold = m_ParamsMgr->BlockTemperatureLowThershold;
	SendMsg.BlockHighThreshold = m_ParamsMgr->BlockTemperatureHighThershold;
	SendMsg.HeatingRate   = m_ParamsMgr->HeatingRate;
	SendMsg.KeepHeatingEvenOpenOrShortCircuit= (BYTE)bKeepHeatingEvenOpenOrShortCircuit;
	SendMsg.HeatingWatchdogSamplingInterval = m_ParamsMgr->HeatingWatchdogSamplingInterval;
    m_SetHeaterTemperature_AckOk=false;
    ...
    {
        TTemperatureType currentTemperatureType = SetTemperatureType(i);
        m_Heaters[i]->UpdateSetTemperatureA2D(HeaterSetTable[i], currentTemperatureType);
    }
...
TQErrCode CHeadHeaters::GetHeatersStatus(void)
{
    ...
    {
        TTemperatureType currentTemperatureType = SetTemperatureType(i);
        CurrentValue = m_Heaters[i]->GetCurrentTemperatureInA2D(currentTemperatureType);
        ...
        FrontEndInterface->UpdateStatus(FE_HEAD_HEATER_CELSIUS_BASE+i,
                                        static_cast<int>(m_Heaters[i]->GetCurrentTemperatureInCelcius(currentTemperatureType)));

        if(!m_Heaters[i]->GetIfHeadWorking())
			continue;
			
		CQLog::Write(LOG_TAG_HEAD_HEATERS, "Current temp %s =   %d ",
					 GetHeaterStr(i).c_str(),
					 m_Heaters[i ]->GetCurrentTemperatureInA2D(currentTemperatureType));
        ...
        if(Err != Q_NO_ERROR)
        {
                ...
                if(IsHeaterOn()/* && !m_HeatersOutOfLimits[i]*/) // m_HeatersOutOfLimits remembers status and thus stops message for next on/off
                {
                    QString OpenShort = "";
                    ...
                    if(Err == Q2RT_HEAD_HEATER_THERMISTOR_OPEN)
                        OpenShort = "open";
                    if(!Get_Ignore_Error()) //During HOW, application ignore open circuit/ short circuit
                    {
                    ...
                    {
                        CQLog::Write(LOG_TAG_HEAD_HEATERS, "During HOW application ignores %s circuit in circuit number %d", OpenShort, i);
                    }
                     m_HeatersOutOfLimits[i] = true;
                }
        }
        ...
        {
            FrontEndInterface->UpdateStatus(FE_HEAD_HEATERS_STATUS_BASE+i,static_cast<int>(m_Heaters[i]->GetIfTemperatureOk()));
        }
    }
        ...
        TGenericCockie Cockie)
{
    ...
    for(i=0; i<TOTAL_NUMBER_OF_HEATERS; i++)
    {
          TTemperatureType currentTemperatureType = InstancePtr->SetTemperatureType(i);
         InstancePtr->m_Heaters.Lookup(i)->UpdateCurrentTemperatureA2D((static_cast<int>(StatusMsg->CurrentElements.HeaterTable[i])),(currentTemperatureType));
...
TQErrCode CHeadHeatersBase::VerifyIfTemperatureOk(void)
{
    ...
    {
        CQLog::Write(LOG_TAG_GENERAL,"VerifyIfTemperatureOk() calls WaitForHeadsTemperatureOK()");
...
TQErrCode CHeadHeaters::SetStandbyTemperature(THeaterElementsTable HeaterTable)
{
    ...
    {
         TTemperatureType currentTemperatureType = SetTemperatureType(i);
        m_Heaters[i]->UpdateStandbyTemperatureA2D(HeaterTable[i], currentTemperatureType);
    }
    m_FlagSetHeaterStandbyComUnderUse=false;
    return (Q_NO_ERROR);
}
        ...
        TGenericCockie Cockie)
{
    ...
    for(int i = 0 ,Value = 0 ; i<TOTAL_NUMBER_OF_HEATERS; i++)
    {
        TTemperatureType currentTemperatureType = InstancePtr->SetTemperatureType(i);
        Value = static_cast<int>(StatusMsg->StandbyElements.HeaterTable[i]);
        InstancePtr->m_Heaters.Lookup(i)->UpdateStandbyTemperatureA2D(Value, currentTemperatureType);
        ...
        TGenericCockie Cockie)
{
	...
	float Vpp,Vcc,Vdd,_24V, V_3_3, V_1_2;
	...
	V_3_3 = CONVERT_3_3_A2D_TO_VOLT((float)PSMsg->V_3_3_Voltage);
	V_1_2 = CONVERT_1_2_A2D_TO_VOLT((float)PSMsg->V_1_2_Voltage);
	...
	FrontEndInterface->UpdateStatus(FE_CURRENT_OHDB_A2D_3_3_V_POWER_SUPPLIERS_STATUS,PSMsg->V_3_3_Voltage);
	FrontEndInterface->UpdateStatus(FE_CURRENT_OHDB_A2D_1_2_V_POWER_SUPPLIERS_STATUS,PSMsg->V_1_2_Voltage);

	InstancePtr->UpdatePowerSupply(Vpp,_24V,Vdd,Vcc,V_3_3,V_1_2);
	...
	FrontEndInterface->UpdateStatus(FE_CURRENT_OHDB_3_3_V_POWER_SUPPLIERS_STATUS,V_3_3);
	FrontEndInterface->UpdateStatus(FE_CURRENT_OHDB_1_2_V_POWER_SUPPLIERS_STATUS,V_1_2);
...
{
   TTemperatureType currentTemperatureType = SetTemperatureType(HeadNumber);
    return m_Heaters[HeadNumber]->GetCurrentTemperatureInA2D(currentTemperatureType);
}
...
void CHeadHeaters::NotificationHeadsTemperatureError(int TransactionId, PVOID Data, unsigned DataLength, TGenericCockie Cockie)
{
    QString msgString;
    ...
    {
		InstancePtr->AckToOhdbNotification(OHDB_HEADS_TEMPERATURE_ERROR, TransactionId, ACK_STATUS_FAILURE, Cockie);
        InstancePtr->m_ErrorHandlerClient->ReportError("Notification Error message:", 0, (int)ErrorMsg->MessageID);
        return;
    }

	InstancePtr->AckToOhdbNotification(OHDB_HEADS_TEMPERATURE_ERROR, TransactionId, ACK_STATUS_SUCCESS, Cockie);

	// Heaters watch dog
	//-------------------------------------------------------- 
	QString str = QFormatStr("Invalid heating rate %d [AD/sec] in heater %s for period of %d seconds. Turning heating off.", ErrorMsg->ErrorHeatingRate,
																															 GetHeaterStr(ErrorMsg->ErrorHead).c_str(),
																															 InstancePtr->m_ParamsMgr->HeatingWatchdogSamplingInterval.Value());
	FrontEndInterface->CancelWizard(str.c_str());                                           // Cancel the running wizard
	InstancePtr->m_ErrorHandlerClient->ReportError(str.c_str(), Q2RT_INVALID_HEATING_RATE);	// Stop printing	
	CQLog::Write(LOG_TAG_HEAD_HEATERS, str.c_str());
	//--------------------------------------------------------

	if(CBackEndInterface::Instance()->HeadHeatersIgnoreError())
		msgString = "\"During HOW application ignores short/open circuit in thermistor: ";
	else
		msgString = "\"Heads Temperature Error: ";

	for(int i = 0; i < TOTAL_NUMBER_OF_HEATERS; i++)
	{
		msgString += QIntToStr (ErrorMsg->Temperatures[i]);
		msgString += " ";
	}
	CQLog::Write(LOG_TAG_HEAD_HEATERS, msgString);
}
    ...
    CHeadHeatersBase(Name)
{
	UpdatePowerSupply( (40.0  * 4095.0 / (21.0 * 2.4)), // Vpp
					   ...
					   (3.3   * 4095.0 / (1.66 * 2.4)), // 3.3 V
					   (1.2   * 4095.0 /  2.4       )); // 1.2 V
...
TQErrCode CHeadHeatersDummy::SetHeatersTemperature(bool bKeepHeatingEvenOpenOrShortCircuit)
{
    SetHeatersTemperature(m_HeatersSetTemperatures, bKeepHeatingEvenOpenOrShortCircuit);
    return (Q_NO_ERROR);
}

TQErrCode CHeadHeatersDummy::SetHeatersTemperature(THeaterElementsTable HeaterElementsTable, const bool& bKeepHeatingEvenOpenOrShortCircuit)
{
    int  i;
    WORD j, HeatersMask = 0;
    ...
    for(i=0; i<TOTAL_NUMBER_OF_HEATERS; i++)
    {
        ...
        {
             TTemperatureType currentTemperatureType = SetTemperatureType(i);
             m_Heaters[i]->UpdateSetTemperatureA2D(HeaterElementsTable[i], currentTemperatureType);
             m_Heaters[i]->UpdateCurrentTemperatureA2D(HeaterElementsTable[i], currentTemperatureType);
             FrontEndInterface->UpdateStatus(FE_HEAD_HEATERS_STATUS_BASE+i,HEAD_TEMPERATURE_OK);
...
TQErrCode CHeadHeatersDummy::GetHeatersStatus(void)
{
    ...
    for(i=0; i<TOTAL_NUMBER_OF_HEATERS; i++)
    {
        ...
        if(m_Heaters[i]->GetIfHeadWorking())
        {
            CurrentA2DValue = m_Heaters[i]->GetCurrentTemperatureInA2D(currentTemperatureType);
...
bool CHeadHeaters::AreHeadsTemperaturesOk()
{
    ...
    for(i = 0; i < TOTAL_NUMBER_OF_HEATERS; i++)
    {
		...
		int status = m_Heaters[i]->GetIfTemperatureOk();
		if(status != HEAD_TEMPERATURE_OK)
		{
			CQLog::Write(LOG_TAG_HEAD_HEATERS, QFormatStr("AreHeadsTemperaturesOk, heater: %d, status: %s", i, ((status == 0) ? "Cold" : "Hot")));
		    return false;
		}
    }
    return true;
...
TQErrCode CHeadHeaters::SetAndMeasureHeadVoltage(int HeadNumber, float ReqVoltage)
{
	...
	CQLog::Write(LOG_TAG_HEAD_HEATERS,"ReqVolt %.2f", ReqVoltage);
	QString VoltageErrorStr = "Voltage error in ";
    int min_limit = m_ParamsMgr->HSW_MinVoltageArray[0].Value();
    int max_limit = m_ParamsMgr->HSW_MaxVoltageArray[0].Value();
    CHECK_REQUESTED_VOLTAGE_LIMITS(ReqVoltage, EHeadHeater,min_limit,max_limit);

    // Calculate the potentiometer value
    int PotValue = CalculatePotValue(ReqVoltage);
    ...
    while (!m_CancelFlag)
    {
        ...
        float voltage_tolerance  = 2 * fabs((EstimateHeadVoltage(PotValue) -  EstimateHeadVoltage(PotValue + 1)));
...
TQErrCode CHeadHeaters::SendHeadsCleaningWizardStartedNotification(void)
{
    // Verify if we are already not performing this command
	if (m_flagHeadCleanWizardStartedUnderUse)
		throw EHeadHeater("HeadHeater:HeadsCleanWizardStarted:two message send in the same time");

	m_flagHeadCleanWizardStartedUnderUse=true;

	TOHDBHeadscleaningStartedMessage Msg;

	Msg.MessageID = static_cast<BYTE>(OHDB_HEADS_CLEANING_WIZARD_STARTED);

    // Send a wizard has started notification
	if (m_OHDBClient->SendInstallWaitReply(&Msg,
										   sizeof(TOHDBHeadscleaningStartedMessage),
										   HeadsCleanWizardStartedAckResponse,
										   reinterpret_cast<TGenericCockie>(this)) != QLib::wrSignaled)
	{
		CQLog::Write(LOG_TAG_HEAD_HEATERS,"OHDB didn't get ack for \"HeadsCleanWizardStarted\" message");
		m_flagHeadCleanWizardStartedUnderUse=false;
        throw EHeadHeater("OHDB didn't get ack for HeadsCleanWizardStarted message");
	}
...
void CHeadHeaters::HeadsCleanWizardStartedAckResponse(int TransactionId, PVOID Data, unsigned DataLength, TGenericCockie Cockie)
{
	TOHDBHeadscleaningStartedResponse *HeadsCleaningWizardStartedMsg = static_cast<TOHDBHeadscleaningStartedResponse *>(Data);

    //Verify size of message
	if(DataLength != sizeof(TOHDBHeadscleaningStartedResponse))
	{
		FrontEndInterface->NotificationMessage("Recieved \"HeadsCleaningWizardStarted\" message with incorrect length");
		CQLog::Write(LOG_TAG_HEAD_HEATERS,"Recieved \"HeadsCleaningWizardStarted\" message with incorrect length");
        return;
    }

	if (static_cast<int>(HeadsCleaningWizardStartedMsg->MessageID) != OHDB_CLEAN_HEADS_WIZ_START_NOTIFIED)
	{
		FrontEndInterface->NotificationMessage("Recieved \"HeadsCleaningWizardStarted\" message with incorrect message ID");
		CQLog::Write(LOG_TAG_HEAD_HEATERS,"Recieved \"HeadsCleaningWizardStarted\" message with incorrect message ID (0x%X)", HeadsCleaningWizardStartedMsg->MessageID);
		return;
	}
}

TQErrCode CHeadHeaters::SendHeadsCleaningWizardEndedNotification(void)
{
    // Verify if we are already not performing this command
	if (m_flagHeadCleanWizardEndedUnderUse)
		throw EHeadHeater("HeadHeater:HeadsCleanWizardEnded:two message send in the same time");

	m_flagHeadCleanWizardEndedUnderUse=true;

	TOHDBHeadscleaningEndedMessage Msg;

	Msg.MessageID = static_cast<BYTE>(OHDB_HEADS_CLEANING_WIZARD_ENDED);

    // Send a wizard has started notification
	if (m_OHDBClient->SendInstallWaitReply(&Msg,
										   sizeof(TOHDBHeadscleaningEndedMessage),
										   HeadsCleanWizardEndedAckResponse,
										   reinterpret_cast<TGenericCockie>(this)) != QLib::wrSignaled)
	{
		CQLog::Write(LOG_TAG_HEAD_HEATERS,"OHDB didn't get ack for \"HeadsCleanWizardEnded\" message");
		m_flagHeadCleanWizardEndedUnderUse=false;
		throw EHeadHeater("OHDB didn't get ack for HeadsCleanWizardEnded message");
	}
...
void CHeadHeaters::HeadsCleanWizardEndedAckResponse(int TransactionId, PVOID Data, unsigned DataLength, TGenericCockie Cockie)
{
	TOHDBHeadscleaningEndedResponse *HeadsCleaningWizardEndedMsg = static_cast<TOHDBHeadscleaningEndedResponse *>(Data);

	//Verify size of message
	if(DataLength != sizeof(TOHDBHeadscleaningEndedResponse))
    {
		FrontEndInterface->NotificationMessage("Recieved \"HeadsCleaningWizardEnded\" message with incorrect length");
        CQLog::Write(LOG_TAG_HEAD_HEATERS,"Recieved \"HeadsCleaningWizardEnded\" message with incorrect length");
        return;
    }

	if (static_cast<int>(HeadsCleaningWizardEndedMsg->MessageID) != OHDB_CLEAN_HEADS_WIZ_END_NOTIFIED)
	{
		FrontEndInterface->NotificationMessage("Recieved \"HeadsCleaningWizardEnded\" message with incorrect message ID");
		CQLog::Write(LOG_TAG_HEAD_HEATERS,"Recieved \"HeadsCleaningWizardEnded\" message with incorrect message ID (0x%X)", HeadsCleaningWizardEndedMsg->MessageID);
        return;
	}
}

TQErrCode CHeadHeatersDummy::SendHeadsCleaningWizardStartedNotification()
{
	return Q_NO_ERROR;
}

TQErrCode CHeadHeatersDummy::SendHeadsCleaningWizardEndedNotification()
{
	return Q_NO_ERROR;
}
