...
class CTrayBase : public CQComponent {
 ...
 protected:
  ...
  public:
	...
	DEFINE_V_METHOD_1(CTrayBase,TQErrCode,SetTrayOnOff,bool){return Q_NO_ERROR;}

	//This procedure is called in order to wait for the Tray in Temperature to turn on
	DEFINE_V_METHOD(CTrayBase,TQErrCode,WaitForTrayInTemperature){return Q_NO_ERROR;}

	//This procedure send Set tray temperature message to OCB.
	DEFINE_V_METHOD_3(CTrayBase,TQErrCode,SetTrayTemperatureMsg,int,int,int){return Q_NO_ERROR;}
	...
	DEFINE_V_METHOD(CTrayBase,TQErrCode,GetTrayStatus){return Q_NO_ERROR;}     // Find out the current tray temperature

	//This procedure send Get status message to OCB.
	DEFINE_V_METHOD(CTrayBase,TQErrCode,GetTrayInsertedStatus){return Q_NO_ERROR;}

    bool CheckTrayTemperatureStatus(void); // use error handler to display error
    TQErrCode IsTemperatureValid(void);

	DEFINE_V_METHOD(CTrayBase,bool,IsTrayInserted);
...
class CTrayPlacer : public CTrayBase
{
private:
...
public:
	...
	CTrayPlacer(const QString& Name);
	~CTrayPlacer();
...
class CTrayPlacerDummy : public CTrayBase
{
private:
public:

	CTrayPlacerDummy(const QString& Name);
	~CTrayPlacerDummy(){}
...
class CTrayHeaterDummy : public CTrayBase
{
public:
    // Constructor
	CTrayHeaterDummy(const QString& Name);

    // Destructor
    ~CTrayHeaterDummy(void);
