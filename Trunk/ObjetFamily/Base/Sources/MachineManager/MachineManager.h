...
//OBJET_MACHINE
	...
	bool m_RunMRW; //Run Resin Replacement Wizard
...
--------------------------------------*/
	...
	TMachineState GotoTrayLowerPositionPhase(void);
	...
	bool RunMRWAfterSleepingChannelsAlert();

	int FindMaxSleepingChannelTime();

	CFilePrintJob*        m_LocalFilePrintJob;
	CDoubleFilePrintJob*  m_DoubleFilePrintJob;

	CPFFSliceQueueThread* m_pPFFSliceQueueThread;
	CPFFPrintJob*   m_PFFPrintJob;
...
public:
	...
	void SetRollerSpeedAccordingToFlexibleParams(CFlexibleParameters& FlexParamsMsg);
	void RevertHeadsAmbientTemp();
	void RevertRollerSpeed();
	...
	DEFINE_METHOD_1(CMachineManager,TQErrCode,AddPFFPrintJob,QString);
...
//OBJET_MACHINE
	...
	DEFINE_V_METHOD_7(CMachineManager,TQErrCode,Load4LocalPrintJob,
	 ...
	 DEFINE_V_METHOD_10(CMachineManager,TQErrCode,Load8LocalPrintJob,
	 QString ,QString ,QString ,
		QString,
		QString ,
		QString ,
	...
	TQErrCode ExecutePFFPrintJob(char* FolderPath);
 //	TQErrCode AddPFFPrintJob(QString FolderPath);
	...
	DEFINE_METHOD(CMachineManager,TQErrCode,GotoTrayLowerPosition);
