...
TQErrCode CPower::SetPowerOnOff(bool OnOff)
{
    ...
    if (m_OCBClient->SendInstallWaitReply(&OnOffMsg, sizeof(TOCBSetPowerOnOffMessage), OnOffAckResponse,
                                          reinterpret_cast<TGenericCockie>(this), QSecondsToTicks(OCB_POWER_ON_TIMEOUT_IN_SEC)) != QLib::wrSignaled)
    {
...
TQErrCode CPower::WaitForPowerOnOff(void)
{
    ...
    if(CAppParams::Instance()->SimulatorMode == true)
        WaitResult = m_SyncEventPoweredOnOff.WaitFor(QSecondsToTicks(OCB_POWER_ON_TIMEOUT_IN_SEC*10 +
                     max((const int)OnDelayParameter,(const int)OffDelayParameter)));
    else
        WaitResult = m_SyncEventPoweredOnOff.WaitFor(QSecondsToTicks(OCB_POWER_ON_TIMEOUT_IN_SEC +
                     max((const int)OnDelayParameter,(const int)OffDelayParameter)));
        ...
        unsigned DataLength,TGenericCockie Cockie)
{
	...
	float MSCVoltages[NUM_OF_MSC_CARDS][mscReadingsNum];

	int msg = FE_CURRENT_MSC1_I2C_ANALOG_IN_24_STATUS;

	for(int i = 0; i < NUM_OF_MSC_CARDS; i++)
    {
		for(int j  = 0; j < mscReadingsNum; j++)
			MSCVoltages[i][j] = StatusMsg->I2CA2DInput[i][j];

		InstancePtr->ConverteNCheckA2DMSCSuppliersValue(MSCVoltages[i][mscReading_24V],
														MSCVoltages[i][mscReading_7V],
														MSCVoltages[i][mscReading_5V],
														MSCVoltages[i][mscReading_3_3V], i); 

		for(int j  = 0; j < mscReadingsNum; j++, msg++)
		{
			FrontEndInterface->UpdateStatus(msg, StatusMsg->I2CA2DInput[i][j]);
			FrontEndInterface->UpdateStatus(msg + mscReadingsNum, MSCVoltages[i][j]);
		}
