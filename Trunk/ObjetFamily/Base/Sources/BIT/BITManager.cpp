...
bool CBITManager::PrepareForTest(TTestAttributes TestAttributes)
{
  ...
  {
    CTrayBase *Tray = MachineSequencer->GetTrayPlacerInstance();
  ...
  {
    CTrayBase *TrayHeater = MachineSequencer->GetTrayHeaterInstance();
   ...
   for(unsigned int i = 0;i< m_GroupsList.size();i++)
    ...
    for(unsigned int i = 0;i< m_GroupsList.size();i++)
	   ...
	   for(unsigned int i = 0;i< m_GroupsList.size();i++)
	  {
	   ...
	   for(unsigned int i = 0;i< m_GroupsList.size();i++)
	  {
		if(m_GroupsList[i]->Name() == "EvacuationSensor" )
		{
			tmpList.push_back(m_GroupsList[i]);
			i = m_GroupsList.size();
			break;
