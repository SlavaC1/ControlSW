...
QString CTankBatchNoTable::GetLastBatchNo() const
{
  QString tmp = "";
  if(m_nextBatchNoIndex==0)
    return tmp;
...
QString CTankBatchNoTable::GetResinType() const
  {
	QString tmp = "";
	if(m_nextBatchNoIndex==0)
      return tmp;
...
QString CTankBatchNoTable::GetBatchNo(int index) const
{
  QString tmp = "";
...
void CTankBatchNoTable::PrintBatchNoTable()
{
  if(m_nextBatchNoIndex==0)
    return;
  CQLog::Write(LOG_TAG_OBJET_MACHINE,"Stratasys: Resin type: %s", m_resinType.c_str());
  for(int i=0;i<m_nextBatchNoIndex;i++)
    CQLog::Write(LOG_TAG_OBJET_MACHINE,"Stratasys: BatchNumber %d: %s", i,batchNoArr[i].c_str());
...
void CBatchNoTable::AddTankBatchNumber(int tankIndex,char * batchNo, QString resinType)
{
  if(m_isPrinting)
  {
	  ...
	  CQLog::Write(LOG_TAG_OBJET_MACHINE,"Stratasys: Batch number:%s Resin type: %s, Tank index: %d", batchNo, resinType.c_str(),tankIndex);
   }
}
