...
#include "Q2RTApplication.h"  //For QA purposes
#include "QUtils.h"
...
/////////////////// debug /////////////////////
extern void OutputToComLogFile(const char * outputStr, const char dir, const int comId);
/////////////////// debug /////////////////////

unsigned int WAIT_FOR_REPLY_RETRIES = 4; // = 1 inital transaction + 3 retries.

// Maximum and minimum number of transactions for outgoing messages
...
// ------------------------------------------------------------------
void CEdenProtocolClient::SetConstants()
{
#ifndef TESTER
  //force-change our constant, without removing their write-protection
  if (Q2RTApplication->GetQATestsAllowed())
  {
	 CQLog::Write(LOG_TAG_GENERAL,"Info: [QA] increased number of comm retries");
	 //changing a constant
	 //*((unsigned int*)( &WAIT_FOR_REPLY_RETRIES )) = 10; // = 1 inital transaction + 9 retries.
	 WAIT_FOR_REPLY_RETRIES = 10; // = 1 inital transaction + 9 retries.
  }
#endif  
}
...

{
  ...
  do {
     ...
     {                               // "communication timed out"
       OutputToComLogFile(QFormatStr("Message 01 %02x %02x %02x no reply", m_DestID&0xff, TransactionID&0xff, MessageID&0xff).c_str(), '!', m_ProtocolEngine->GetComPort()->GetComNum());
       CQLog::Write(
           LOG_TAG_GENERAL,
           ...
           MessageID,
           TransactionID,
           QGetTicks() - CurrentInfoBlock->SendTimeStamp, // Receive time stamp is meaninless here
           ClientTimeDiff);
     }
	 ...
	 if (
              (WaitResult != QLib::wrSignaled) &&
             ((RetryCount + 1) < WAIT_FOR_REPLY_RETRIES)
        )
     {
       ...
       if (WaitResult == QLib::wrSignaled)
       {
         CQLog::Write(
           ...
           CurrentInfoBlock->ReceiveTimeStamp - CurrentInfoBlock->SendTimeStamp);
               ...
               (WaitResult != QLib::wrSignaled)
               && (++RetryCount < WAIT_FOR_REPLY_RETRIES)
             );
