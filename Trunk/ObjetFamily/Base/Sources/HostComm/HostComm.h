...
#undef DEFINE_COMMAND
  ...
  unsigned long           m_Max_Incoming_Data_Size;
  long                    m_Max_Incoming_Resinn_Data_Size;
  ...
  int ClearResinRequestedData(ResinPrinterDataType& ResinRequestedData);
   TCommErrorCodes CheckObjetStudioParams(const BYTE& byPrintMode, const short& sNumberOfAdditionalModelResins);
...
public:
   ...
   virtual ~CHostComm(void);
    ...
    bool IsPaused();    
    static TPrintStopReason ErrCodeToHostError(TQErrCode ErrCode);
