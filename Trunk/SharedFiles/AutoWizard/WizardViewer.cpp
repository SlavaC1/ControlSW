...
bool TWizardViewerFrame::ShowPage(CWizardPage *WizardPage)
{
	...
	{
		CQLog::Write(LOG_TAG_GENERAL, QFormatStr("Self closing page: %d. Pressing Done button", WizardPage->GetPageNumber()));
...
void TWizardViewerFrame::SetDefaultFrameLayout()
{
	...
	// Please Wait message upon wizard completion/cancellation
	//No need to do the following:
	//	PleaseWaitLabel->Width = 493; PleaseWaitLabel->Top = 111; PleaseWaitLabel->Left = 11;
	//as long as we keep these properties:
	//Anchors = [akLeft,akTop,akRight,akBottom] //stretching out
	//Layout = tlCenter //Misc section's Vertical Alignment setting
}
...
void TWizardViewerFrame::PrepareCheckboxesPage(CCheckBoxWizardPage *PagePtr)
{
	...
	{
		//This line will count the checkboxes according to the mask
		//EnableDisableNext(~(0xFFFFFFFF << PagePtr->Strings.Count()) == PagePtr->GetChecksMask());
		//This line will count the checkboxes according to their visible state (checked/unchecked)
		EnableDisableNext(CountChecked() == PagePtr->Strings.Count());
	}
	else if(PagePtr->GetAttributes() & wpNextOneChecked)
		//This line will count the checkboxes according to the mask
		//EnableDisableNext(0 != PagePtr->GetChecksMask());
		//This line will count the checkboxes according to their visible state (checked/unchecked)
		EnableDisableNext(0 != CountChecked());
	else if(PagePtr->GetAttributes() & wpNextDisabled)
	...
	{
		if(dynamic_cast<TWinControl &>(*CheckBoxesContainer->Controls[0]).CanFocus())   //It verify window is visible and enable. otherwise, we can Crash !!
		{
			dynamic_cast<TWinControl &>(*CheckBoxesContainer->Controls[0]).SetFocus();
		}
		else
		{
			CQLog::Write(LOG_TAG_GENERAL, QFormatStr("Warning: Failed window.SetFocus() , because the window is invisible or disabled"));
		}
	}
	SetDefaultFrameLayout();
}
...
void TWizardViewerFrame::PrepareRadioGroupPage(CRadioGroupWizardPage *PagePtr)
{
	...
	const int MaxRadioGroupHeight = 0.85f * ((float)(RadioGroupPage->Height - RadioGroup->Top));
	...
	if(ChkPage)
	{
		...
		else
		{
			...
			if(ChkPage->GetAttributes() & wpNextWhenSelected)
				...
				//This line will count the checkboxes according to their visible state (checked/unchecked)
				EnableDisableNext(CountChecked() == ChkPage->Strings.Count());
			else
...
void TWizardViewerFrame::ReplaceViewerFrame(TFrame* CustomizedFrame, bool ShowImage)
{
	...
	int leftStartPointMainPanel = ((ShowImage) ? this->PageImage->Width + PANEL_MARGIN : 0) + PANEL_MARGIN;
	this->Width  = CustomizedFrame->Width + leftStartPointMainPanel;
	...
	//update 'Please Wait' notification upon wizard cancellation/completion
	//to be at the center of the MainPanel (w/o the PageImage).
	//	PleaseWaitLabel->Width = CustomizedFrame->Width - PANEL_MARGIN;
	//	PleaseWaitLabel->Top = (CustomizedFrame->Height - this->PleaseWaitLabel->Height) / 2;
	//	PleaseWaitLabel->Left = leftStartPointMainPanel;
	//The above mentioned settings are obsolete as long as we keep these properties:
	//PleaseWaitLabel->Anchors = [akLeft,akTop,akRight,akBottom] //stretching out
	//PleaseWaitLabel->Layout = tlCenter //Vertical Alignment setting
...
unsigned int TWizardViewerFrame::CountChecked() const
{
	TComponent * MyComponent;
	TCheckBox  * CheckBox;
	...
	for(int i = 0; i < CheckBoxesContainer->ControlCount; i++)
	{
		...
		// Check if we're tring to handle the MoreCheckBox itself.
		if(-1 == CheckBox->Tag)
			continue;
