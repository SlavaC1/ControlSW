...
CWizardPage *CAutoWizard::GetPreviousPage(void)
{
  ...
  if(m_History.size() > ((unsigned)(IsCurrentIgnored ? 0 : 1)))
    {
...
void CAutoWizard::GotoPage(int PageNumber)
{
  SetNextPage(PageNumber);
    TWizardUIControlType LeaveReason = ((GetNextPage()->GetAttributes() & wpCancelPage) ? wuiCancelNext : wuiGotoNext);
  if(m_WizardUICallback)
...
{
  SetNextPage(Page);
  TWizardUIControlType LeaveReason = ((Page->GetAttributes() & wpCancelPage) ? wuiCancelNext : wuiGotoNext);
  if(m_WizardUICallback)
    (*m_WizardUICallback)(LeaveReason,0,m_WizardUICockie);
}
...
static void ShowQException(EQException ExceptionObject,const QString& WizardName)
{
  CQLog::Write(LOG_TAG_GENERAL, QFormatStr("Wizard '%s' throw exception: '%s' , (Code %d)",WizardName.c_str(),
								ExceptionObject.GetErrorMsg().c_str(),ExceptionObject.GetErrorCode()));
  QMonitor.ErrorMessageWaitOk(QFormatStr("%s error\n%s ",WizardName.c_str(), ExceptionObject.GetErrorMsg().c_str()),ORIGIN_WIZARD_PAGE);
}
...
static void ShowUnexpectedException(const QString& WizardName)
{
  QMonitor.ErrorMessageWaitOk(QFormatStr("Wizard '%s' threw an unexpected exception",WizardName.c_str()),ORIGIN_WIZARD_PAGE);
}
